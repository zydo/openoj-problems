...
- New id / title / slug: 1778 / Shortest Route in a Hidden Maze / `shortest-route-in-a-hidden-maze`
...
- Figures: none
- Gates: verify ✓ (7/7 languages, 19/19 cases each) · compatibility ✓ (manual proof below) · stale ✓ · overlap ✓ · check: **known harness lag** (see notes) · sandbox pending (batch)

### Notes

- **`adapt_gates.py` compatibility FAILs on every sharded interactive bundle, including the exemplar.** The gate's verify subprocess passes `problems-adapt/<key>` (flat) and `verify_solution.py`'s `/`-branch never re-resolves through the shard, so `_assembly_sources` sees no `provided/` and the oracle class is missing: `module 'openoj_solution' has no attribute 'MazeController'`. Running `adapt_gates.py 0227_search-hidden-sorted-sequence --source 0702_...` reproduces the identical signature today. Fix is central: pass the shard-qualified path (or make verify's resolver shard-aware). Meanwhile I ran the gate's exact staging (problem.json-derived renames, word-boundary `re.sub`) against the shard-qualified path: both source solutions pass **19/19**.
- **`gen_starters.py` needs two central additions**: `INTERACTIVE_ORACLES["MazeController"] = {"python": "MazeController", "java": "MazeController", "parameter": "maze"}` (without it `check_bundle` fails `starter generation failed: Unsupported interactive oracle: MazeController`), and interactive generation for the five typed languages — the same gap that makes the exemplar 0227 report 5 stray starters. My py/java starters are byte-shaped to the generator's output for when the table entry lands; the other five follow the exemplar exactly.
