...
- New id / title / slug: 337 / Non-Adjacent Loot in a Tree / `non-adjacent-loot-in-a-tree`
...
- Statement rewritten from spec: yes — burglary scenario dropped; framed as the family vocabulary introduced by `0198_maximum-non-adjacent-loot` (nodes hold values, chosen nodes must not be a parent and child, choosing nothing allowed)
...
- Family: `house-robber` — part I `0198_maximum-non-adjacent-loot` already on disk; this is part III. `0213_house-robber-ii` (part II) is not adapted yet and should be titled to match (e.g. *Maximum Non-Adjacent Loot, Circular*). Part IV `2560_house-robber-iv` adapted in this chunk as `2560_non-adjacent-loot-under-a-cap`
