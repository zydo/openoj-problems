...
Scan nums2 left to right while keeping a stack of values still waiting for their next greater element.

### Hint 2

When the current value exceeds the stack top, it is the next greater element for every value popped.

### Hint 3

Store value to next greater element in a hash map, then answer every nums1 query in O(1).

### Hint 4

Values left on the stack at the end have no next greater element; map them to -1.
