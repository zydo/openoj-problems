...
Assume the frog is initially on the first stone and assume the first jump must
be 1 unit.
...
Track, for every stone, the set of jump sizes that can land on it — positions alone are not enough state.

### Hint 2

From a stone reached with last jump k, only stones at distance k-1, k, or k+1 ahead are candidates; use a hash map from position to index to find them.

### Hint 3

The jump size never exceeds n, so the total state space is O(n^2), which fits the constraints.
