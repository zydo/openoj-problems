...
`i`-th index represents the exclusive time for the function with ID `i`.
...
Process the logs in order and keep a stack of the currently running function calls.

### Hint 2

A start log suspends the function on top of the stack: add the elapsed time since it last started or resumed to its exclusive time.

### Hint 3

An end log closes the call on top of the stack; the call ran from its start through the end of the timestamp, so it accumulates end - start + 1, and its parent resumes at timestamp + 1.
