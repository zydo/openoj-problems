 ...
 * @return {number}
 */
var topPathProduct = function (grid) {
    const MOD = 1000000007n;
    const m = grid.length;
    const n = grid[0].length;
    let maxRow = new Array(n).fill(0n);
    let minRow = new Array(n).fill(0n);
    ...
    for (let i = 1; i < m; i++) {
        const newMax = new Array(n).fill(0n);
        const newMin = new Array(n).fill(0n);
