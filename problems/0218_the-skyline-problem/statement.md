...
Sweep a vertical line from left to right over the building edges; at each x-coordinate the skyline height is the tallest building that is still active there.

### Hint 2

At the same x, process start events before end events so adjacent buildings hand off the contour without a spurious dip to 0.

### Hint 3

Keep active heights in a max-heap and lazily discard entries whose right edge has already passed; the heap top is the current skyline height.

### Hint 4

Record a key point only when the current height differs from the previous one, because consecutive equal heights must be merged.
