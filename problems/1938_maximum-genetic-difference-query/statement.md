...
Use a trie to store all the genetic values on the path from a node to the root.

### Hint 2

Dynamically add values with a DFS: insert a node's value when entering it and remove it when leaving.

### Hint 3

For each query, walk the trie greedily from the most significant bit to maximize the XOR.
