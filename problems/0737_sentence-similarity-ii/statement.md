...
similar, return `true` if `sentence1` and `sentence2` are similar, or `false`
...
Notice that a word is always similar to itself, and the similarity relation is
transitive: for example, if the words `a` and `b` are similar, and the words
`b` and `c` are similar, then `a` and `c` are similar.
...
Consider the graph where each pair in similarPairs is an edge: two words are similar if they are the same word or are in the same connected component of this graph.

### Hint 2

Union-find lets you merge similar pairs and then test whether two words share a root in near-constant time.

### Hint 3

Return false immediately if the two sentences have different lengths.
