...
A word can share another word's slot exactly when it is a suffix of that word ("me" fits inside "time#").

### Hint 2

Only keep words that are not a proper suffix of some other word; each kept word contributes len(word) + 1 characters.

### Hint 3

Equivalently, insert every word reversed into a trie and count the trie's leaf path lengths.
