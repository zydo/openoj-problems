function bestTreePurchases(n: number, price: number[], reward: number[], edges: number[][], budget: number): number {
    ...
    for (let idx = n - 1; idx >= 0; idx--) {
        ...
        const costFull = price[u];
        const costDisc = Math.floor(price[u] / 2);
        const profitFull = reward[u] - costFull;
        const profitDisc = reward[u] - costDisc;
        for (let b = 0; b <= budget; b++) {
            if (b >= costFull) {
