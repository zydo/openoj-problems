...
Compute the prefix XOR for every index; the XOR of subarray [left, right] is 0 exactly when the prefix XORs at both ends are equal.

### Hint 3

Iterate from left to right keeping a hash table counting how often each prefix XOR value has appeared.

### Hint 4

Add the count of the current prefix XOR to the answer, then record it (including the initial 0 before the array starts).
