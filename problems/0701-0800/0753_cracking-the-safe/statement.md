...
We can think of this problem as the problem of finding an Euler path (a path visiting every edge exactly once) on the following graph: there are k^(n-1) nodes with each node having k edges. It turns out this graph always has an Eulerian circuit (a path starting where it ends).
