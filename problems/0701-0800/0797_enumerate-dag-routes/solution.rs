impl Solution {
    pub fn enumerate_dag_routes(graph: Vec<Vec<i32>>) -> Vec<Vec<i32>> {
        ...
        // copy and stop.
        fn dfs(g: &Vec<Vec<usize>>, target: usize, node: usize, path: &mut Vec<i32>, paths: &mut Vec<Vec<i32>>) {
            if node == target {
