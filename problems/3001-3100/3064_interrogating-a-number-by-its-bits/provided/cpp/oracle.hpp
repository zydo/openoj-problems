// Problem-provided oracle (MaskedNumber), C++ side. Compiled into the
...
class MaskedNumber {
public:
    MaskedNumber(const OjValue& n, long long budget) : budget_(budget) {
        if (n.kind != OjValue::Int) throw std::runtime_error("MaskedNumber n must be an integer");
        n_ = n.integer;
    }

    int commonSetBits(int num) {
        if (budget_ <= 0) throw std::runtime_error("MaskedNumber query budget exhausted");
        budget_ -= 1;
        int count = 0;
