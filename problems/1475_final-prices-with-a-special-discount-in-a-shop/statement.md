...
For each item, scan forward for the first j with prices[j] <= prices[i] and subtract it; a brute-force double loop works within the limits.

### Hint 2

A monotonic (non-decreasing) stack of indices whose discount is still pending processes the whole array in one pass.
