...
where `position[i]` is the starting mile of the `i`th car and `speed[i]` is
the speed of the `i`th car in miles per hour.
...
For each car compute the time it would take to reach the target driving alone.

### Hint 3

Walking from the car nearest the target backwards, a car merges into the fleet ahead exactly when its alone-time is not greater than that fleet's arrival time.

### Hint 4

A car that would catch up exactly at the target still joins that fleet, so only a strictly greater time starts a new fleet.
