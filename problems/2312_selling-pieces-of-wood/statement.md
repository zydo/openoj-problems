...
Return the maximum money you can earn after cutting an `m x n` piece of
wood.
...
Output: 19
Explanation: One possible scenario consists of:
...
Consider all actions available on a piece of wood of size h x w: sell it whole (if a price exists), cut it vertically into h x w1 and h x w2 with w1 + w2 = w, or cut it horizontally into h1 x w and h2 x w with h1 + h2 = h.

### Hint 2

Each cut breaks the piece into strictly smaller subproblems, so a DP over (height, width) works.

### Hint 3

dp[h][w] = max(price[h][w], best vertical cut, best horizontal cut); the answer is dp[m][n].
