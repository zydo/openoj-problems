...
type HiddenFormula struct {
	functionId int
	budget     int64
}

// NewHiddenFormula builds the oracle from the case's construction values
// (one integer: the hidden formula's id) and the query budget.
func NewHiddenFormula(construction []any, budget int64) *HiddenFormula {
	id, ok := construction[0].(int64)
	if !ok {
		panic("Oracle function_id must be an integer")
	}
	return &HiddenFormula{functionId: int(id), budget: budget}
}

// F returns some positive integer evaluate(x, y) for two positive integers x and y
// based on a formula.
func (custom *HiddenFormula) F(x int, y int) int {
	if custom.budget <= 0 {
