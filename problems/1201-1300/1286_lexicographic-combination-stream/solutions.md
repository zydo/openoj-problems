# Solutions — Lexicographic Combination Stream

## Bitmask enumeration with a final sort

`letters.length <= 15` means at most `2^15` subsets — small enough to
materialize everything up front. Walk the masks in increasing order,
keep those whose popcount equals `length`, and read each kept
...
`n = letters.length`, `O(2^n · k)` space; then `O(1)` per
`next()`/`hasNext()`.
