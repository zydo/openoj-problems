...
`Z`, and a number `n`. Each CPU interval can be idle or allow the completion
of one task. Tasks can be completed in any order, but there's a constraint:
...
For every cycle, find the most frequent letter that can be placed in this cycle; after placing it, decrease that letter's frequency by one.

### Hint 3

A priority queue (max-heap on remaining count) is a natural way to always pick the most frequent available letter.
