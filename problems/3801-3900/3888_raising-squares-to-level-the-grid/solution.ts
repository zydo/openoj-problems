function levelGrid(grid: number[][], k: number): number {
    ...
    const pa: number[][] = Array.from({ length: m + 1 }, () => new Array<number>(n + 1).fill(0));
    const pb: number[][] = Array.from({ length: m + 1 }, () => new Array<number>(n + 1).fill(0));
    let hasFixed = false;
    ...
    function rect(p: number[][], r1: number, r2: number, c1: number, c2: number): number {
