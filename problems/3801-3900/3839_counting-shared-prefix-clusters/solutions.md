# Solutions — Counting Shared-Prefix Clusters
...
collapses every cluster into an equivalence class of the "first `k`
characters" map, so a cluster is nothing more than a single k-prefix taken by
several words, and "every pair is k-matched" holds exactly when the
...
cluster of at least two words, and prefixes seen once are
