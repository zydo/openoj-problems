...
// cost sum stays below budget and every capacity sum is at most 2e5 — far
// inside both 2^31 and the 2^53 exact range of doubles. Sort the machines
// by cost with capacities aligned; every affordable pair is then reachable
// from its dearer machine with a prefix of cheaper partners, so a prefix
// maximum of capacities answers "best partner" in constant time per
// machine.
function maxAffordableCapacity(costs: number[], capacity: number[], budget: number): number {
    const machines: [number, number][] = costs
        .map((cost, i) => [cost, capacity[i]] as [number, number])
        .sort((a, b) => a[0] - b[0]);
    const n = machines.length;
    const sortedCosts: number[] = machines.map((m) => m[0]);
    const prefMax: number[] = new Array(n).fill(0);
    let run = 0;
    for (let i = 0; i < n; ++i) {
