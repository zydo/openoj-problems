# Solutions — Gcd Totals From Opposite Ends

## Running max, sort, two-pointer pairing

The construction of `b` is a one-pass running maximum: at index i the
value is `gcd(nums[i], peak)` where `peak` is the largest element seen so
...
Once `b` is sorted, the pairing rule — smallest unpaired with largest
unpaired — is exactly a two-pointer walk from the two ends of the
