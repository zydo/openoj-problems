...
class Solution {
  public:
    int countXorSlices(vector<int> &nums, int target1, int target2) {
        // nums[i] and both targets are at most 1e5 < 2^17, and XOR never
        // widens a value, so every prefix XOR and every bucket key
        // p ^ target stays below 2^17. Counts are reduced modulo 1e9 + 7
        // at every bucket write, so each stored count is below 1e9 + 7 and
        // any pre-reduction sum below 2^31; the buckets are long long
        // anyway.
        ...
        vector<long long> endsT1(1 << 17, 0), endsT2(1 << 17, 0);
        endsT2[0] = 1;
        int p = 0;
        long long curT1 = 0, curT2 = 0;
        for (int x : nums) {
