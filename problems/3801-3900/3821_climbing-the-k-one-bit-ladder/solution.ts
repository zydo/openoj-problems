function nthWithKSetBits(n: number, k: number): number {
    ...
    // but bitwise operators coerce to 32 bits and would corrupt any
    // value past 2^31, so bits are set by adding exact powers of two.
    const C: number[][] = Array.from({ length: 51 }, () => new Array(51).fill(0));
    for (let i = 0; i <= 50; i++) {
        C[i][0] = 1;
        for (let j = 1; j <= i; j++) {
