...
`trips[i] = [numPassengersi, fromi, toi]` indicates that the `i`th trip has
...
For each trip, record a pickup event of +passengers at from and a dropoff event of -passengers at to.

### Hint 2

Sweep the events in location order, tracking the number of passengers on board.

### Hint 3

Passengers dropped off at a location free their seats for pickups at that same location.
