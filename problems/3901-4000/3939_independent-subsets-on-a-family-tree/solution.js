 ...
 * @return {number}
 */
var countIndependentSubsets = function (parent, nums, k) {
    const modulus = 1000000007;
    const n = parent.length;
    const children = Array.from({ length: n }, () => []);
    for (let i = 1; i < n; i++) children[parent[i]].push(i);

    const dp0 = Array.from({ length: n }, () => new Array(k).fill(0));
    const dp1 = Array.from({ length: n }, () => new Array(k).fill(0));
    for (let node = n - 1; node >= 0; node--) {
        dp0[node][0] = 1;
        dp1[node][nums[node] % k] = 1;
        for (const child of children[node]) {
            const merged0 = new Array(k).fill(0);
            const merged1 = new Array(k).fill(0);
