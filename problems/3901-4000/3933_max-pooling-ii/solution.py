...
class Solution:
    def maxPoolII(self, grid: List[List[int]]) -> int:
        rows, columns = len(grid), len(grid[0])
        positions = [[] for _ in range(201)]
        for row in range(rows):
            for column in range(columns):
                if grid[row][column] != 0:
                    positions[grid[row][column]].append((row, column))
        ...
        for value in range(1, 201):
            ...
            for row in range(rows):
                running = 0
                for column in range(columns):
                    running += int(grid[row][column] > value)
                ...
                for corner_row in (row - value, row + value):
                    for corner_column in (column - value, column + value):
                        if (
                            ...
                            and grid[corner_row][corner_column] > value
                        ):
                            greater -= 1
                if greater == 0:
                    answer += 1
