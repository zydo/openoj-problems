# Solutions — Gated Climbs To The Shared Ancestor

The solution runs binary lifting with card-transition matrix products.

## Binary lifting matrices

Represent each node by a two-color transition matrix and store its product for every power-of-two upward jump. Binary lifting finds each LCA and composes both climbs without traversing them one edge at a time.
