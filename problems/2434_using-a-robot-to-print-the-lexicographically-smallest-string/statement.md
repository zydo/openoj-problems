...
string `t`. Apply one of the following operations until `s` and `t` are both
empty:
...
If there are some characters 'a' in the string, they can be written on paper before anything else.
...
After the robot writes every 'a' on paper, the same holds for other characters 'b', 'c', and so on.
