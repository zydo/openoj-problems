...
numbers from either end of the array (i.e., `nums[0]` or
`nums[nums.length - 1]`), which reduces the size of the array by `1`. The
player adds the chosen number to their score. The game ends when there are no
...
and 7. No matter which number player 2 chooses, player 1 can choose 233.
...
Model the game as the score difference the player about to move can secure on the remaining subarray.

### Hint 2

Define diff(i, j) as the best (current player's score minus opponent's score) on nums[i..j]; it equals max(nums[i] - diff(i + 1, j), nums[j] - diff(i, j - 1)).

### Hint 3

Player 1 wins exactly when diff(0, n - 1) >= 0, because ties still count as a win for player 1.

### Hint 4

With n <= 20, memoizing over the O(n^2) subarrays is instant.
