...
type FlattenedElement = number | any[];

function flattenToDepth(arr: FlattenedElement[], depth: number): FlattenedElement[] {
    const result: FlattenedElement[] = [];
    const stack: [FlattenedElement, number][] = [];
    for (let index = arr.length - 1; index >= 0; index--) {
        stack.push([arr[index], 0]);
    }
    while (stack.length > 0) {
        const entry = stack.pop() as [FlattenedElement, number];
        const value = entry[0];
        const at = entry[1];
...
class Solution {
    run(flattenToDepthCase: FlattenToDepthCase): void {
        flattenToDepthCase.drive(flattenToDepth);
    }
}
