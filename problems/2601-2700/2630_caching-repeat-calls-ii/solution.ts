class Solution {
    runMemoized(replayCacheCase: ReplayCacheCase): string {
        // Cache calls through a tree whose spine walks the argument list one
        ...
        // object numbered 5.
        const branchSymbols = new WeakMap<object | ((...args: any[]) => any), symbol>();
        const referenceKey = (value: object): symbol => {
        ...
        const isByReference = (value: unknown): value is object =>
            value !== null && (typeof value === "object" || typeof value === "function");

        const leafMarker = Symbol("complete");

        const cacheCalls = (fn: (...args: any[]) => any) => {
            const root = new Map<unknown, any>();
        ...
        let calls = 0;
        const countedFn = (...callArgs: any[]): any => {
            calls += 1;
            return replayCacheCase.fn(...callArgs);
        };
        const cached = cacheCalls(countedFn);

        const rows: { val: unknown; calls: number }[] = [];
        for (const argumentList of replayCacheCase.buildInputs()) {
            const val = cached(...argumentList);
            rows.push({ val, calls });
        }
