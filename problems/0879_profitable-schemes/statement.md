...
commit. The `i`th crime generates a `profit[i]` and requires `group[i]`
...
This is a 0-1 knapsack over crimes with two budgets: members (at most n) and profit (at least minProfit).

### Hint 2

Cap the profit dimension at minProfit — once the threshold is reached, extra profit changes nothing.

### Hint 3

Loop crimes on the outside and iterate members/profit downward so each crime is used at most once; take every count modulo 10^9 + 7.
