# Solutions — Run-Limited Binary Arrays II

## Block DP with rolling window sums

A run-limited array is an alternating sequence of blocks — runs of equal bits
whose lengths are all at most `limit` — so `dp[x][y][d]`, the number of
run-limited arrays with exactly `x` zeros, `y` ones, and last bit `d`, extends by
