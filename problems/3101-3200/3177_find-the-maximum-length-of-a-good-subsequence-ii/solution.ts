function maximumLength(nums: number[], k: number): number {
    ...
    for (const x of nums) {
        ...
        remapped.push(mapping.get(x) as number);
    }
    ...
    const best1: number[] = new Array(k + 1).fill(0); // max over v of dp[j][v]
    const val1: number[] = new Array(k + 1).fill(-1); // argmax
    const best2: number[] = new Array(k + 1).fill(0); // second max over v != val1

    for (const x of remapped) {
        const cand: number[] = new Array(k + 1).fill(0);
        for (let j = 0; j <= k; j++) {
