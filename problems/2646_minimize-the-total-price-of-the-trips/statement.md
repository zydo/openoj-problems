...
The final answer is the sum of price[i] * freq[i], where freq[i] is the number of times node i is visited across all trips and price[i] is the final (possibly halved) price.

### Hint 2

To compute freq[i], run a DFS or BFS for each trip and increment every node on the path between start and end.

### Hint 3

Finally use tree dynamic programming: let dp(v, 0/1) denote the minimum total price with node v's price not halved or halved.
