...
somewhere in the array. The hash map asks each position's question directly
— has my partner gone by? — and needs only one look at the data. The sort
buys ordered ground instead: once positions are arranged by value, two
pointers can converge from the ends, and each comparison rules out one
position for good.
