...
Two ways of finding where two root-to-target chains meet. One is a single
...
to say is the meeting point. The other runs the tree backwards: record every
node's parent in one walk, then the chains become list walks from each target
upward, and the deepest shared ancestor is the first entry of one chain seen
from the other.
