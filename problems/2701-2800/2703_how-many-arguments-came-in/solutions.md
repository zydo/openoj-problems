# Solutions — How Many Arguments Came In

The judged quantity is call arity: however many values a case spreads into
`countArguments`, answer with their number. Both offered languages expose
...
objects, or absent altogether (`countArguments()` still sees an empty
argument list of length 0). Declaring `function countArguments() { return
...
`countArguments(...callArgsCase.args)` reproduces the variadic driving
...
call — `function countArguments(...args) { return args.length; }`. The
