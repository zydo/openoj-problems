...
// buildObject, and structurally compares the returned object against the
...
interface Solver {
    buildObject(keysArr: unknown[], valuesArr: unknown[]): unknown;
}
...
class PairingCase {
    ...
    // invoke the solution's buildObject, remember the structural verdict.
    check(solution: Solver): void {
        const [keysArr, valuesArr] = this.buildInputs();
        const actual = solution.buildObject(keysArr, valuesArr);
