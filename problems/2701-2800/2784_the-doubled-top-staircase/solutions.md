# Solutions — The Doubled-Top Staircase

## Sort against the constructed base

The maximum element pins the problem down. If `nums` is a rearrangement of some `base[m]`, its largest value must be `m` itself: every entry of `base[m]` is at most `m`, and `m` occurs in it twice. So once the maximum is read off there are no candidate arrays left to consider — only one `base` could possibly match, and checking it is a pure multiset comparison.

That check is two comparisons. A `base[m]` always has length `m + 1`, so an array whose length disagrees with `max + 1` (Example 4, or a lone `[200]`) is rejected before any work. Otherwise, sort a copy of `nums` and compare it against the literally constructed `[1, 2, ..., m - 1, m, m]`: the ascending values `1` through `m - 1`, then two copies of `m`. Any defect shows up as a mismatch — a missing middle value, a duplicated non-maximum, or extra elements beyond the implied length.
