impl Solution {
    pub fn day_after(date: String) -> String {
        ...
        let (mut next_year, mut next_month, mut day_after) = (year, month, day + 1);
        if day_after > month_lengths[next_month - 1] {
            day_after = 1;
            next_month += 1;
            if next_month > 12 {
                next_month = 1;
        ...
        format!("{:04}-{:02}-{:02}", next_year, next_month, day_after)
    }
}
