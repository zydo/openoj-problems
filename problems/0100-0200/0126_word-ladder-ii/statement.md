...
Run a BFS from beginWord to determine the shortest distance from beginWord to every reachable word in wordList.

### Hint 2

An edge between two words can appear on a shortest path only when their distances differ by exactly 1.

### Hint 3

After the BFS, reconstruct all shortest sequences with a DFS that only follows edges increasing the distance toward endWord.
