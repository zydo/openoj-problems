# Solutions — Loot The Tree, Keep Every Path Alive

Every value is at least 1, so a root-to-leaf path sums to zero exactly
when every node on it was zeroed. Staying sound therefore means: the
...
`dp[0]`, since the root's obligation is precisely the statement's soundness
condition.
