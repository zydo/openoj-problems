...
// deep.
function bestHarvest(edges: number[][], coins: number[], k: number): number {
    ...
    const s: number[][] = Array.from({ length: n }, () => new Array(16).fill(0));
    const dp: number[][] = Array.from({ length: n }, () => new Array(16).fill(0));
    for (let i = n - 1; i >= 0; i--) {
        const v = order[i];
        const c = coins[v];
        for (let t = 0; t < 15; t++) {
