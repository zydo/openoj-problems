...
If at any point in time there are no enqueued tasks, the CPU must wait until the smallest enqueue time of a not-yet-processed task.

### Hint 2

Use a min-heap to choose the task with the smallest processing time (smallest index on ties) from all the currently enqueued tasks.
