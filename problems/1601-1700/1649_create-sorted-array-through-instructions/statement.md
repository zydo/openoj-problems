...
from the elements in `instructions`. You start with an empty array `nums`. For
each element from left to right in `instructions`, insert it into `nums`. The
...
Return _the **total** cost to insert all elements from_ `instructions` _into_
...
If you know the position in which you will insert the i-th element, you can find the minimum cost to insert it.
