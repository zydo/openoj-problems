...
Output: "Alice"
Explanation: Alice must choose all the three piles at the first move to win and leave Bob with a negative score.
...
Output: "Tie"
Explanation: Alice cannot win this game. She can end the game in a draw if she decides to choose all the first three piles, otherwise she will lose.
```
...
Model the game as a minimax DP: process the row from the end and let dp[i] be the best score difference (current player minus opponent) achievable from pile i onward.

### Hint 2

From position i the player takes 1, 2, or 3 piles: dp[i] = max(take_j - dp[j]) over the reachable next positions.

### Hint 3

Compare dp[0] with 0 to decide between "Alice", "Bob", and "Tie".
