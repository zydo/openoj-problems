...
days. On the `i`th day, the tree grows `apples[i]` apples that will rot
after `days[i]` days, that is on day `i + days[i]` the apples will be rotten
and cannot be eaten. On some days, the apple tree does not grow any apples,
...
Given two integer arrays `apples` and `days` of length `n`, return the
...
It is optimal to finish the apples that will rot first before those that will rot last.

### Hint 2

You need a structure to keep the apples sorted by their finish time, and you can keep eating after day n while fresh apples remain.
