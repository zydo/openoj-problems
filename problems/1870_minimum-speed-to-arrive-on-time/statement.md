...
between each train ride. For example, if the 1st train ride takes 1.5 hours,
you must wait for an additional 0.5 hours before you can depart on the 2nd
...
Given a fixed speed, the total travel time is the sum of ceil(dist[i] / speed) for every ride except the last, plus dist[n-1] / speed for the final ride.

### Hint 2

If a given speed gets you there on time, every faster speed does too — the feasibility predicate is monotone, so binary search the speed.
