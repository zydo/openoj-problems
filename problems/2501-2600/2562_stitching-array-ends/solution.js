 ...
 * @return {number}
 */
var stitchTotal = function (nums) {
    // Two pointers eat the array from both ends; every round folds
    // first * 10^digits(last) + last into the running value. This is
    // exactly concat(first, last) without any string round-trip. Even at
    // the constraint maximum (~500 rounds of 5-digit concatenations) the
    // total stays under 5*10^10 << 2^53, so plain numbers are exact.
    let answer = 0;
    let left = 0;
