# Solutions — Binary Strings with Regular Runs

Any regular string decomposes uniquely into runs of equal characters, and each
run's size must be a positive multiple of its character's group. So peel off
the final run: what precedes it is either nothing or a shorter regular string
...
Let `dp[i]` be the number of regular strings of length exactly `i`, with
`dp[0] = 1` for the empty string. A length-`i` regular string is obtained from a
regular string of length `i − oneGroup` by appending a block of 1's, or from one
of length `i − zeroGroup` by appending a block of 0's, and every regular string
