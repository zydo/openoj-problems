# Solutions — Fewest Deletions to Pair the Array

Deletions only shift the tail leftward, so any well-paired array reachable
from `nums` is a subsequence — and the greedy pairing order is preserved: element
