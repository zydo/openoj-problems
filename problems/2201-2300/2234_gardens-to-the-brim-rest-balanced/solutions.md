# Solutions — Gardens to the Brim, the Rest Balanced

## Try every complete-count; water-fill the rest with what's left

If k gardens end up finished, an optimal choice finishes the k gardens that
already have the most flowers — sorted descending, finishing them is as
cheap as any other choice and leaves the largest budget. So sort ascending,
and sweep k = 0…n: the cost to add one more finished garden comes straight
off a suffix walk, spending `target − flowers[i]` per newly finished garden
...
above target, that split is dominated by finishing everything for free, so
