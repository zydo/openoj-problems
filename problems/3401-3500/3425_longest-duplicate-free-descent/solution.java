...
class Solution {
    ...
    public int[] longestDuplicateFreeDescent(int[][] edges, int[] nums) {
        int n = nums.length;
        ...
        while (!st.isEmpty()) {
            ...
            if (ev[4] == 1) {
                ...
                startDepth[0] = startRestore.remove(startRestore.size() - 1);
                continue;
            }
            // Enter node u.
            distPath.add(d);
            ...
            startRestore.add(startDepth[0]);
            if (prevLast >= startDepth[0]) {
                startDepth[0] = prevLast + 1;
            }
            last.put(val, depth);
            int length = d - distPath.get(startDepth[0]);
            int nodes = depth - startDepth[0] + 1;
            if (length > bestLen) {
                bestLen = length;
                bestNodes = nodes;
