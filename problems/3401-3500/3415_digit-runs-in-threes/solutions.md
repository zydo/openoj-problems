# Solutions — Digit Runs In Threes

A SKU qualifies when its label contains at least one maximal run of
digits and every such run has length exactly three. That reading is the
statement's whole trap — a label may carry several digit runs, but each
one must be three digits long — so `447x808y` qualifies while `447x8088y`
and `Batch2024Rev` do not.
...
Each pattern is a linear scan over the label, evaluated once per row;
`ORDER BY sku_id` finishes the contract. The judge runs SQLite, which
matches `GLOB` classes character-wise, so unicode labels behave as
expected — a multibyte letter is a non-digit.

**Complexity:** `O(N·L)` time for `N` labels of length at most `L` (a dozen
