# Solutions — Largest Piece Of A Shared Word

The pile ends up holding every piece of every distinct split of `word`
...
pile's contents into something scannable: a length cap, and the fact that
...
`numFriends` is 1, where the pile holds exactly the whole word — every
string of at most that length really does appear in the pile: lay it at
