...
// C(m+1,2)*C(n+1,2) ~ 2.5e11 — past 32 bits but safely below 2^53, so
// Number arithmetic stays exact.
function countTidySubmatrices(grid: number[][], k: number): number {
    const m = grid.length;
    const n = grid[0].length;
    const stackVal: number[][] = Array.from({ length: n }, () => []);
    const stackWid: number[][] = Array.from({ length: n }, () => []);
