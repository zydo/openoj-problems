...
Binary search on the answer. Write a function possible(capacity) which returns true if and only if the packages can be shipped within days days at that capacity.

### Hint 2

Feasibility is monotone: if a capacity works, every larger capacity works too, so binary search between max(weights) and sum(weights).

### Hint 3

The feasibility check is a single greedy left-to-right pass that opens a new day whenever the next package would overflow the ship.
