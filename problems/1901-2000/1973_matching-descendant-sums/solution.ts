 ...
 * they are kept in 64-bit space (a JS number is exact below 2^53).
 * @param {TreeNode} root
 * @return {number}
 */
function countMatchingNodes(root: TreeNode | null): number {
    ...
    const subtree = new Map<TreeNode, number>();
    let count = 0;
