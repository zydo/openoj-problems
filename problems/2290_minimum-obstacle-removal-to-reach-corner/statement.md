...
You can move up, down, left, or right from and to any cell that is not an
obstacle.
...
Model the grid as a graph where cells are nodes and edges connect adjacent cells. Edges into obstacle cells have cost 1; all other edges have cost 0.

### Hint 2

Use 0-1 BFS with a deque (or Dijkstra's algorithm) to find the minimum-cost path from (0, 0) to (m - 1, n - 1).
