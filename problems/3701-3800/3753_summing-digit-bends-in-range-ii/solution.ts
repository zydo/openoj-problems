function totalBends(num1: number, num2: number): number {
    ...
    // exact.
    const digitsOf = function (n: number): number[] {
        const digits: number[] = [];
        while (n > 0) {
            digits.push(n % 10);
            n = Math.floor(n / 10);
    ...
    const f = function (input: number): number {
        if (input <= 0) {
            return 0;
        }
