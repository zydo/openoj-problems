# Solutions — Counting Inversion-Free Windows

## Count Within Nondecreasing Runs

A subarray is inversion-free exactly when it reads non-decreasing, so the property is a
property of runs: cut `nums` into its maximal non-decreasing segments and no
inversion-free subarray ever crosses a boundary between two of them. For one end
index `e`, the starts that yield an inversion-free subarray are exactly
...
`base[e] = e - left[e] + 1` counts every inversion-free subarray ending at `e`
