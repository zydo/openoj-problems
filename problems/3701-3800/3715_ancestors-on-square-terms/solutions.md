# Solutions — Ancestors On Square Terms

## Square-Free Residues with Tree Aggregation

Multiplying two integers yields a perfect square precisely when they share
the same square-free residue: strip every prime's even powers and whatever
remains must coincide. Let `M = max(nums)`. A smallest-prime-factor sieve up
to `M` makes extracting each residue cheap — walk the value down through its
...
ancestors carry an equal residue.

One depth-first pass answers all nodes at once. Carry a frequency table
indexed by residue value; entering a node first adds its residue's current
...
same-residue siblings sit adjacent in processing order but are not ancestors
of one another.
