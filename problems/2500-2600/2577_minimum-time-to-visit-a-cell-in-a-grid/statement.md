...
Try using an algorithm that finds shortest paths on a graph, such as Dijkstra's.

### Hint 2

To reach a cell before its required time you may have to go back and forth between two cells of the matrix to waste time; note that such waiting changes the arrival time's parity.
