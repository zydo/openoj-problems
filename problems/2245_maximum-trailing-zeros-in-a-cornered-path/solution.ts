function maxTrailingZeros(grid: number[][]): number {
    ...
    const count2: number[][] = Array.from({ length: m }, () =>
        new Array(n).fill(0),
    );
    const count5: number[][] = Array.from({ length: m }, () =>
        new Array(n).fill(0),
    );
    for (let i = 0; i < m; i++) {
        for (let j = 0; j < n; j++) {
    ...
    const row2: number[][] = Array.from({ length: m }, () =>
        new Array(n + 1).fill(0),
    );
    const row5: number[][] = Array.from({ length: m }, () =>
        new Array(n + 1).fill(0),
    );
    for (let i = 0; i < m; i++) {
        for (let j = 0; j < n; j++) {
    ...
    const col2: number[][] = Array.from({ length: n }, () =>
        new Array(m + 1).fill(0),
    );
    const col5: number[][] = Array.from({ length: n }, () =>
        new Array(m + 1).fill(0),
    );
    for (let j = 0; j < n; j++) {
        for (let i = 0; i < m; i++) {
