 ...
 * @return {number}
 */
var gcdWeightTally = function (nums) {
    ...
    const spf = Array.from({ length: maxa + 1 }, (_, i) => i);
    ...
    const buckets = Array.from({ length: maxa + 1 }, () => []);
    const cache = new Map();
    ...
    for (let g = 1; g <= maxa; ++g) {
        ...
        const fen = new Array(size + 1).fill(0);
        let total = 0;
    ...
    let answer = 0;
    const exact = new Array(maxa + 1).fill(0);
