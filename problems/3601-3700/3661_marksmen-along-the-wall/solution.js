 ...
 * @return {number}
 */
var bestSalvo = function (robots, distance, walls) {
    ...
    const bots = robots.map((pos, i) => [pos, distance[i]]).sort((a, b) => a[0] - b[0]);
    walls.sort((a, b) => a - b);
    ...
    const leftLo = (i) => (i === 0 ? bots[i][0] - bots[i][1] : Math.max(bots[i][0] - bots[i][1], bots[i - 1][0] + 1));
