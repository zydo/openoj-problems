function maxRiseFallRiseSum(nums: number[]): number {
    ...
    for (let i = 1; i < nums.length; i++) {
        const prev = nums[i - 1],
            x = nums[i];
        if (x > prev) {
            // Rising step: the final climb continues or opens from a finished
            // descent; the first climb extends from itself or grows past the
            // lone previous element.
            s2 = Math.max(s2, s1) + x;
