...
Try a binary search approach over the minimum value that can be achieved for the maximum number of the array.
...
The answer is the maximum over every prefix of ceil(prefix_sum / prefix_length): value can only move left, so each prefix bounds the achievable maximum.
