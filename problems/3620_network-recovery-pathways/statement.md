...
The graph has two possible routes from node 0 to node 3:
  Path 0 -> 1 -> 3: total cost = 5 + 10 = 15, which exceeds k (15 > 10), so this path is invalid.
  Path 0 -> 2 -> 3: total cost = 3 + 4 = 7 <= k, so this path is valid; its score is min(3, 4) = 3.
...
Consider the remaining routes from 0 to 4:
  Path 0 -> 1 -> 4: total cost = 7 + 5 = 12 <= k, valid; score = min(7, 5) = 5.
  Path 0 -> 2 -> 3 -> 4: node 3 is offline, invalid regardless of cost.
...
- `0 <= costi <= 10^9`
- `0 <= k <= 5 * 10^13`
- `online[i]` is either `true` or `false`, and both `online[0]` and
  `online[n - 1]` are `true`.
...
Binary-search the answer S: a score S is achievable iff some valid path uses only edges with cost >= S.

### Hint 2

For a fixed S, keep edges with cost >= S and compute the minimum total cost path from 0 to n - 1 restricted to online nodes (a DAG, so a topological-order DP works).

### Hint 3

If even S = 0 yields no valid path, return -1.
