...
// / 3, failing on a 2. Since n <= 10^7 < 3^15, at most fifteen rounds run,
// all on values no larger than n itself, so nothing accumulates.
impl Solution {
    pub fn is_distinct_power_sum(n: i32) -> bool {
        let mut n = n;
        while n > 0 {
            if n % 3 == 2 {
                return false;
