# Solutions — Longest Two-Case Substring

A character whose case-partner is missing from the entire string can
never sit inside a two-case substring: every window containing it
fails the check. Such characters act as walls, and the answer must
...
pieces. A segment with no offenders is entirely two-case and is returned
...
On `"dDc"` the character 'c' (no 'C') is the lone offender, splitting
out `"dD"` — two-case, length 2. On `"eEfFgG"` every partner is
present, so the whole string is the answer. A single character has no
partner and yields the empty string.
