class Solution {
    findShortestRoute(maze: MazeController): number {
        const dirs = ["U", "D", "L", "R"];
        const dr = [-1, 1, 0, 0];
        const dc = [0, 0, -1, 1];
        const back: Record<string, string> = { U: "D", D: "U", L: "R", R: "L" };
        const key = (r: number, c: number): number => (r + 512) * 1024 + (c + 512);
        ...
        while (stack.length > 0) {
            ...
            if (!pushed) {
                stack.pop();
                const parentDir = parentDirs.pop() as number;
        ...
        for (let head = 0; head < queue.length; head++) {
            ...
            const d = dist.get(key(r, c)) as number;
            for (let i = 0; i < 4; i++) {
                const nr = r + dr[i];
                const nc = c + dc[i];
                const nk = key(nr, nc);
        ...
        return dist.get(targetKey) as number;
    }
}
