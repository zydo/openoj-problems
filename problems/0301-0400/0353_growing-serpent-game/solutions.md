# Solutions — Growing Serpent Game
...
the cells it covers. `advance` computes the new head from the current one,
...
per call, and eating is precisely the advance where the tail stays put, the
...
**Complexity:** `O(1)` time per advance amortized; `O(food.length)` space.
