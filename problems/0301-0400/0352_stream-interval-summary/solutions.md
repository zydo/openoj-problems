# Solutions — Stream Interval Summary
...
by `start`, with touching intervals merged the moment they touch. `addValue`
...
`currentIntervals` then has no thinking left to do: the list is already
...
**Complexity:** `O(log k + k)` per `addValue` worst case (search plus list
shift), `O(k)` per `currentIntervals`; `O(k)` space, where `k` is the number
