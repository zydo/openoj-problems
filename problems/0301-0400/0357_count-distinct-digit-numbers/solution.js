 ...
 * @return {number}
 */
var countDistinctDigitNumbers = function (n) {
    ...
    let total = 1; // f(0): x = 0 itself
    let run = 1; // f(k) so far, one factor per digit place
    for (let length = 1; length <= Math.min(n, 10); ++length) {
