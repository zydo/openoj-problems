...
Binary search on the value range [matrix[0][0], matrix[n-1][n-1]] rather than on matrix indices.

### Hint 2

Count how many elements are <= mid with a staircase walk from the bottom-left corner in O(n): move right when matrix[row][col] <= mid, otherwise move up.

### Hint 3

The search converges to a value that is guaranteed to be present in the matrix, so no extra pass is needed to snap it to an actual element.
