 ...
 * @return {number}
 */
var countUnfilledOrders = function (orders) {
    // Two binary heaps: sells as a min-heap on price, buys as a max-heap.
    // An incoming batch trades with the best-priced opposing batch while
    // the price condition holds; only its unmatched remainder joins the
    // backlog as one new batch.
    const swap = (h, i, j) => {
        [h[i], h[j]] = [h[j], h[i]];
    };
    const push = (h, less, x) => {
    ...
    const sells = []; // [price, amount], cheapest on top
    const buys = []; // [price, amount], priciest on top
    const sellLess = (a, b) => a[0] < b[0];
    const buyLess = (a, b) => a[0] > b[0];
    ...
    // Totals reach 1e5 * 1e9 = 1e14, far below 2^53, so plain numbers sum
    // exactly; the answer is reduced modulo 1e9 + 7 at the end.
    let total = 0;
