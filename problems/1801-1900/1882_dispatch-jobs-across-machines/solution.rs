...
impl Solution {
    ...
    pub fn dispatch_jobs(machines: Vec<i32>, jobs: Vec<i32>) -> Vec<i64> {
        let mut free: BinaryHeap<Reverse<(i64, i64)>> = BinaryHeap::new();
        for (i, &w) in machines.iter().enumerate() {
            free.push(Reverse((w as i64, i as i64)));
        }
        let mut busy: BinaryHeap<Reverse<(i64, i64, i64)>> = BinaryHeap::new();
        let mut ans = Vec::with_capacity(jobs.len());
        let mut cur: i64 = 0;
        for (j, &cost) in jobs.iter().enumerate() {
