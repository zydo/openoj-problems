...
provide power to all cities `j` such that `|i - j| <= r` and
`0 <= i, j <= n - 1`.
...
Pre-calculate the number of power stations serving each city using a line sweep (difference array).

### Hint 2

Use binary search to maximize the minimum power of a city.

### Hint 3

To check a candidate minimum X, sweep from left to right; whenever a city is below X, add stations at the rightmost city that can still help it.
