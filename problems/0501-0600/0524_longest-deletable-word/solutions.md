# Solutions — Longest Deletable Word

## Greedy subsequence test, best-so-far fold

Deleting characters from `s` can only leave a subsequence of `s`, so a
dictionary word is deletable exactly when it is a subsequence. Test each word
...
candidate: a deletable word wins when it is longer than the best so far, or of
...
deletable entry computes the same minimum, at the cost of ordering words the
