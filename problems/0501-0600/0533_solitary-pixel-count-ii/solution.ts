function countSolitaryPixels(picture: string[][], target: number): number {
    ...
    const classRowCount: number[] = [];
    // Black cells of one row.
    const countBlacks = (row: string[]): number => {
        let blacks = 0;
    ...
    const rowClass = new Array<number>(m);
    ...
    const blacks = Array.from({ length: n }, () => new Array<number>(classRowCount.length).fill(0));
    for (let i = 0; i < m; i++) {
        for (let j = 0; j < n; j++) {
