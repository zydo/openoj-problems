...
`A` and `B` such that every edge in the graph connects a node in set `A` and a
node in set `B`.
...
Try to 2-color the graph with a DFS or BFS: color a node, then color all its neighbors with the opposite color.

### Hint 2

The graph is not bipartite exactly when you find an edge whose two endpoints already share the same color.

### Hint 3

The graph may be disconnected, so start a fresh traversal from every still-uncolored node.
