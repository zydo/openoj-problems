...
`removable`, `p` is still a subsequence of `s`. More formally, you will mark
the character at `s[removable[i]]` for each `0 <= i < k`, then remove all
...
First solve an easier check: after removing a given set of indices from s, is p still a subsequence of what remains?

### Hint 2

If p survives removing the first k indices of removable, it survives any smaller k as well — binary search on k.
