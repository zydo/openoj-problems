...
Two anagrams are similar exactly when they differ in 0 or 2 positions, so checking a pair is a single linear scan.

### Hint 2

Build a graph whose nodes are the strings, with an edge between every similar pair; the answer is the number of connected components.

### Hint 3

Union-Find (or BFS/DFS) over all O(n^2) pairs is fast enough for n <= 300.
