...
Sort the indices in decreasing order of their value in nums2.

### Hint 2

Loop through nums2 in that order and compute the maximum score given that the current nums2 value is the minimum of the chosen set.

### Hint 3

Maintain a min-heap of the selected nums1 values of size k (sum included) so the best k elements seen so far are kept.

### Hint 4

Update the answer with sum * current nums2 value once k elements have been taken.
