...
Maintain a sliding window that is valid when it contains at most k characters that are the minority answer.

### Hint 2

Grow the window to the right one question at a time, and shrink from the left whenever both the 'T' count and the 'F' count exceed k.

### Hint 3

The answer is the largest window length seen; a window where min(countT, countF) <= k can always be converted into a uniform run.
