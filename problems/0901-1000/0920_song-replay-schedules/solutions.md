# Solutions — Song Replay Schedules

A prefix of a queue is pinned down for counting purposes by just two
...
## Dynamic program on queue length and distinct songs

Let `dp[i][j]` count the valid queues of length `i` that use exactly `j`
distinct songs. The `i`-th play is one of two moves. Introducing a song not
yet used leaves `n - (j - 1)` choices, so each `dp[i-1][j-1]` queue
...
to `n!` — every queue of length `n` using all `n` songs is a permutation,
