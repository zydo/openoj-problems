class Solution {
  public:
    int cheapestXPowerSum(int x, int target) {
        ...
        // long.
        long long cost0 = 0, cost1 = 1LL << 60;
        int i = 0;
        long long t = target;
        while (t > 0) {
            long long p = i == 0 ? 2 : i;
            long long r = t % x;
            t /= x;
            long long n0 = std::min(cost0 + r * p, cost1 + (r + 1) * p);
            long long n1 = std::min(cost0 + (x - r) * p, cost1 + (x - r - 1) * p);
            cost0 = n0;
            cost1 = n1;
            ++i;
        }
        return (int)(std::min(cost0, cost1 + (long long)i) - 1);
    }
};
