function evaluate(expression: string): number {
    ...
    const exprStart = (i: number): boolean => {
        const t = tokens[i];
        if (t === "(" || !isVar(t)) return true;
        return tokens[i + 1] === ")";
    };

    const parse = (i: number, env: Map<string, number>): [number, number] => {
        ...
        // let
        const local = new Map<string, number>(env);
        let value = 0;
        while (tokens[i] !== ")") {
            if (exprStart(i)) {
    ...
    const [result] = parse(0, new Map<string, number>());
    return result;
}
