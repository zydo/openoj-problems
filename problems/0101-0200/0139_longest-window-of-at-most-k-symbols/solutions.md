...
can never make it illegal. One solution uses that fact directly, growing
and shrinking a single window as its right edge walks the string, so the
answer is complete after one pass. The other uses it sideways: since every
...
length costs one sweep of the string.
...
the sliding window above with the shrink pinned to one shape — the window
