class Solution {
  public:
    int stockTradingTwoSales(vector<int> &prices) {
        ...
        vector<int> bestPrefix(n, 0);
        int minPrice = prices[0];
        for (int i = 1; i < n; i++) {
            minPrice = min(minPrice, prices[i]);
            bestPrefix[i] = max(bestPrefix[i - 1], prices[i] - minPrice);
        }
        // Backward: bestSuffix[i] is the best single-sale profit over days
        // i..n-1 -- day i's price buys and the running maximum sells.
        vector<int> bestSuffix(n, 0);
        int maxPrice = prices[n - 1];
        for (int i = n - 2; i >= 0; i--) {
            maxPrice = max(maxPrice, prices[i]);
            bestSuffix[i] = max(bestSuffix[i + 1], maxPrice - prices[i]);
        }
        ...
        int best = 0;
        for (int i = 0; i < n; i++) {
            best = max(best, bestPrefix[i] + bestSuffix[i]);
        }
        return best;
    }
};
