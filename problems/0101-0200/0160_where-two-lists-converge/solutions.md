...
meeting point unevenly — after the shorter prefix's pointer has already moved
...
when the lists never touch). The switchback form gets the same effect
...
join node — or at `null` after both have traversed everything.
