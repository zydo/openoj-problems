# Solutions — Counting Peak-Beauty K-Letter Picks

## Frequency-group greedy with binomial choice and modular powers

A k-pick uses each of its `k` letters exactly once, and letter `c` can be
placed in exactly `f(c)` ways (its index positions in `s`), so a fixed
...
letters no k-pick exists and the answer is 0. Otherwise sort those
...
multipliers. All arithmetic runs modulo 10⁹ + 7: products are reduced at
every step, `x^i` comes from fast
