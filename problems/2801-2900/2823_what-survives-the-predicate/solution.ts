// deepPrune works in one bottom-up sweep. A leaf (number, string,
...
function deepPrune(obj: any, fn: (value: any) => boolean): any {
    if (Array.isArray(obj)) {
        const survivors: any[] = [];
        for (let index = 0; index < obj.length; ++index) {
            const kept = deepPrune(obj[index], fn);
            if (kept !== undefined) {
                survivors.push(kept);
            }
        }
    ...
    if (obj !== null && typeof obj === "object") {
        ...
        for (let index = 0; index < keys.length; ++index) {
            ...
            const kept = deepPrune(obj[key], fn);
            if (kept !== undefined) {
                survivors[key] = kept;
            }
        }
