# Solutions — The Stretching Interval
...
`stretchInterval(fn, delay, period)` schedules its first tick after
...
`stretchCancel(id)` must stop the whole chain while knowing only the
