...
For a fixed number of candies c, a pile of size p yields floor(p / c) allocations, so you can check whether c candies per child is feasible in one pass.

### Hint 2

Feasibility is monotonic in c: if each child can get c candies, they can also get any smaller amount.

### Hint 3

Binary search on c between 0 and max(candies) to find the maximum feasible value.
