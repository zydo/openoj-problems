// One round of the score process turns an array into its adjacent XORs, so
// unrolling the rounds gives a Pascal-style recurrence over GF(2):
...
// cells. Every element is at most 2^31 - 1, so bit 31 is always 0, and the
// XOR of two bit-31-zero words has bit 31 zero too — by induction every
// score lies in [0, 2^31 - 1]. Such values are unchanged by ToInt32 and
// their XOR pattern reads back non-negative, so the 32-bit bitwise ops are
// exact rather than sign-truncating; they are also far below 2^53, so
// plain number arithmetic stays exact.
function maxFoldedXor(nums: number[], queries: number[][]): number[] {
    const n = nums.length;
    const bestRows: Int32Array[] = new Array(n);
