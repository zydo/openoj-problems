# Solutions — The Color Quota Game

## Per-player color tallies

Whether player i clears their quota depends only on their own picks: they
win exactly when some single color occurs at least `i + 1` times among
them. One pass over `pick` therefore settles everything — bump a counter keyed by the
...
the quota of `i + 1`. Player 0 wins with any single ball, matching the statement's
