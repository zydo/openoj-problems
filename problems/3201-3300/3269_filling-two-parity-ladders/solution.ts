...
// are <= 2*(n+m) <= 4000, far below Number's exact range.
function minParityPeak(nums1: number[], nums2: number[]): number {
    const BIG = 1 << 29;
    const n = nums1.length;
    const m = nums2.length;
    let prv0: number[] = new Array(m + 1).fill(BIG);
    let prv1: number[] = new Array(m + 1).fill(BIG);
    if (m >= 1) {
        prv1[1] = 2 - nums2[0];
        for (let j = 2; j <= m; ++j) {
    ...
    for (let i = 1; i <= n; ++i) {
        ...
        const cur0: number[] = new Array(m + 1).fill(BIG);
        const cur1: number[] = new Array(m + 1).fill(BIG);
