# Solutions — Ambiguous Abbreviation Check
...
it. `isAmbiguityFree` then restates the two-condition rule as one lookup: fetch the
...
repeats, so `isAmbiguityFree("deer")` stays true, where a collision-count check
...
from its ends); each `isAmbiguityFree` in `O(1)`; `O(n)` space.
