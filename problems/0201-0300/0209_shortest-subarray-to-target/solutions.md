...
only grows as its window extends. One precomputes prefix sums and
binary-searches each start position's shortest qualifying run; the other
walks both ends of a window across the array in a single linear pass.
