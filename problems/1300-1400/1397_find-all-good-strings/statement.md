...
"az". Then there are 25 good strings starting with 'c': "ca","cc","cd",...,
"cz" and finally there is one good string starting with 'd': "da".
```
...
Use digit DP with the four states (pos, posEvil, equalToS1, equalToS2): count valid strings of the remaining size pos where the longest suffix that is also a prefix of evil has size posEvil. equalToS1/equalToS2 track whether the prefix built so far equals s1/s2.

### Hint 2

Advance the maximum matching suffix against evil using KMP preprocessing so a transition can tell immediately when evil would be formed.
