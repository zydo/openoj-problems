...
var countFancy = function (l, r) {
    ...
    const good = new Array(145).fill(0);
    ...
    const overlap = new Array(goods.length + 1).fill(0);
    ...
    const countOverlap = (x) => overlap[countGood(x)];

    const countSumGood = (x) => {
    ...
    const countFancyUpTo = (x) => countSumGood(x) + countGood(x) - countOverlap(x);
