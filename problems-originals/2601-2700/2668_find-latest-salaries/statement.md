...
values) for this table. Each row contains employees details and their
yearly salaries, however, some of the records are old and contain outdated
salary information.

`salary` holds its numeral as text (the column is `varchar`); since
salaries increase each year, an employee's current record is the one with
the numerically greatest value — compare it as a number, not as text.
