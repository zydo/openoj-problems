...
exactly `k` moves. In case it is not possible to obtain a non-empty pile
after `k` moves, return -1.
...
For each index i, how can we check if nums[i] can be present at the top of
the pile or not after k moves?
