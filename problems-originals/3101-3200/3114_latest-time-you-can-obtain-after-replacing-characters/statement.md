...
12-hour times are formatted as `"HH:MM"`, where `HH` is between `00` and
`11`, and `MM` is between `00` and `59`. The earliest 12-hour time is
`00:00`, and the latest is `11:59`.
...
Iterate over all possible times that can be generated from the string and
find the latest one.
