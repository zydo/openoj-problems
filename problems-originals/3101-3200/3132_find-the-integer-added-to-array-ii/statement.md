...
represented by the variable `x`. As a result, `nums1` becomes equal to
`nums2`. Two arrays are considered equal when they contain the same
...
- The test cases are generated in a way that there is an integer `x` such
  that `nums1` can become equal to `nums2` by removing two elements and
...
Try all possibilities to remove 2 elements from `nums1`.

### Hint 2

`x` should be equal to `min(nums2) - min(nums1)`, check it naively.
