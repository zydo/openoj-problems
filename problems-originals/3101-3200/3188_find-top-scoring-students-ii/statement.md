...
(`student_id`, `course_id`, `semester`) is the primary key (combination
...
Each testcase supplies its own `dataset`: the DDL seeds the three tables
with that testcase's rows. In every dataset the `mandatory` flag is
stored in lowercase — `'yes'` or `'no'` — exactly as the example shows;
grades are single capital letters `'A'` through `'F'`; "at least B" means
the letter grades `'A'` or `'B'`; the mandatory requirement holds only
when every enrollment row the student has for each in-major mandatory
course is an `'A'`; the elective requirements look only at enrollments
for elective courses offered in the student's own major, of which there
must be at least two distinct ones, each of whose enrollment rows carries
at least a `'B'`; and the average GPA is the arithmetic mean of the
enrollment `GPA` values over all of the student's enrollment rows,
in-major or not. No test sits closer than `0.05` to the 2.5 cut except
...
Explanation:
    ...
    Bob (student_id 2) is a Computer Science major but did not receive
    an A in all required courses.
    Charlie (student_id 3) is a Mathematics major and has taken both
    Calculus and Linear Algebra, receiving an A in both. He has also
    taken Probability and Statistics as electives, receiving an A and B
    respectively.
...
`student_id` — containing exactly the qualifying students in ascending
