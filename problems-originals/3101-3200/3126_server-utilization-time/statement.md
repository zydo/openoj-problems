...
(server_id, status_time, session_status) is the primary key (combination of
columns with unique values) for this table. session_status is an ENUM
(category) type of ('start', 'stop'). Each row of this table contains
server_id, status_time, and session_status.
...
Each testcase supplies its own `dataset`: the DDL seeds the `Servers` table
...
column — `total_uptime_days`, where each server's consecutive
`start`/`stop` timestamps delimit one running span, every span's length is
summed across all servers, and that total is rounded down to whole days.
Return the result table in any order.
