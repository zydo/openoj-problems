...
a spell. Multiple spells can have the same damage value.
...
If we ever decide to use some spell with power x, then we will use all
spells with power x.
...
dp[i][j] represents the maximum damage considering up to the i-th unique
spell and j represents the number of spells skipped (up to 3 as per
constraints).
