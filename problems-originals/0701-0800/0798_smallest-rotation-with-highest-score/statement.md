...
Afterward, any entries that are less than or equal to their index are worth
one point.

For example, if `nums = [2,4,1,3,0]` and we rotate by `k = 2`, it becomes
`[1,3,0,2,4]`. This is worth 3 points because `1 > 0` (no points),
`3 > 1` (no points), `0 <= 2` (one point), `2 <= 3` (one point), and
`4 <= 4` (one point).
