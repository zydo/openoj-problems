// Row and column swaps preserve the XOR of any rectangle's four corners, and
// that XOR is 0 on every chessboard, so a solvable board must repeat one row
// (or its complement) everywhere.
function movesToChessboard(board: number[][]): number {
    ...
    const fits = (ones: number) => ones === half || ones === ceilHalf;
