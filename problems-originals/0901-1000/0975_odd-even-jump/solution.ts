function oddEvenJumps(arr: number[]): number {
    ...
    const oddOk: boolean[] = new Array(n).fill(false);
    const evenOk: boolean[] = new Array(n).fill(false);
    oddOk[n - 1] = true;
    evenOk[n - 1] = true;
    let count = 1;
    for (let i = n - 2; i >= 0; i--) {
...
function jumpTable(arr: number[], descending: boolean): number[] {
    const n = arr.length;
