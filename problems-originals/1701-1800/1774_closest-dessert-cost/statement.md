...
- `baseCosts`, an integer array of length `n`, where each `baseCosts[i]`
  represents the price of the `i`-th ice cream base flavor.
- `toppingCosts`, an integer array of length `m`, where each
  `toppingCosts[i]` is the price of one of the `i`-th topping.
...
Total: 3 + 4 + 10 + 0 = 17. You cannot make a dessert with a total cost
of 18.
```
...
As the constraints are not large, you can brute force and enumerate all
the possibilities.
