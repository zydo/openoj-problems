...
such that the `ith` subarray is equal to `groups[i]` (0-indexed), and if
`i > 0`, the `(i-1)th` subarray appears before the `ith` subarray in `nums`
...
the 1st one as [1,-1,0,1,-1,-1,3,-2,0].
...
When we use a subarray, the room for the next subarrays will be the suffix
after the used subarray.

### Hint 2

If we can match a group with multiple subarrays, we should choose the first
one, as this will just leave the largest room for the next subarrays.
