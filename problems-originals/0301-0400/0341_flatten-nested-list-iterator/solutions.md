# Solutions — Flatten Nested List Iterator
...
the order `next()` must produce, because a preorder traversal of a nested
list visits each integer precisely where it appears in the flattened
sequence. `next()` returns the array entry under a cursor and advances it;
`hasNext()` is a cursor-versus-length comparison. Empty list holds fall out
