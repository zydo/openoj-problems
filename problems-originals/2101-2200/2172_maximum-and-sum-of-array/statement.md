...
Use a bitmask over 2 * numSlots slot positions (two positions per slot) to represent which positions are already filled.

### Hint 3

The number of set bits in a mask tells you which nums element is placed next; try placing it into every free position.
