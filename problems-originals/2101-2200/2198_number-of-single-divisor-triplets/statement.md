...
Note that (0, 1, 2) is not a single divisor triplet because
nums[0] + nums[1] + nums[2] = 3 and 3 is divisible by nums[0], nums[1],
and nums[2].
```
...
The maximum value of `nums.length` is very large, but the maximum value of
`nums[i]` is not.

### Hint 2

Count the number of times each value appears in `nums`. Brute force through
every possible combination of values and count how many single divisor
