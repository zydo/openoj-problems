...
- The relative order of the elements less than `pivot` and the elements
  greater than `pivot` is maintained.
    ...
    of the `i`th element and `p_j` is the new position of the `j`th element. If
    `i < j` and both elements are smaller (or larger) than `pivot`, then
    `p_i < p_j`.
...
Could you put the elements smaller than the pivot and greater than the pivot
in a separate list as in the sequence that they occur?
