...
`(business_id, event_type)` is the primary key (combination of columns with
...
such that their `occurrences` is strictly greater than the average activity
for that event.
...
Write your solution as a single `SELECT` query returning `business_id`.
