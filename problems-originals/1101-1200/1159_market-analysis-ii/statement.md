...
items, report the answer for that user as `no`. It is guaranteed that no
seller sells more than one item in a day.
...
Write your solution as a single `SELECT` query returning `seller_id` and
`2nd_item_fav_brand`.
