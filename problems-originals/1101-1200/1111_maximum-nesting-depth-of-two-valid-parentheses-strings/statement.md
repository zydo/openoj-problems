...
that `A` and `B` are VPS's (and `A.length + B.length = seq.length`). The
...
This corresponds to the output `[0, 1, 0, 1, 0, 1, 0, 1, 0]` where `0`
indicates membership in `A` and `1` indicates membership in `B`.
...
of `A` and `B`: `answer[i] = 0` if `seq[i]` is part of `A`, else
`answer[i] = 1`. Note that even though multiple answers may exist, you may
return any of them.
