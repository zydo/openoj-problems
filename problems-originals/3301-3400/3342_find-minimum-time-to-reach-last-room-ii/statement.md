...
start moving to that room. You start from the room (0, 0) at time
`t = 0` and can move to an adjacent room. Moving between adjacent rooms
takes one second for one move and two seconds for the next, alternating
between the two.
...
Use shortest path algorithms with a state for the last move being odd or
even indexed.
