...
var minTimeToReach = function (moveTime) {
    ...
    // fixes each cell's outgoing cost, so plain Dijkstra applies: a cell
    // settled at time t offers a neighbour arrival
    // max(t, moveTime[next]) + cost_out(cell), and the first settle is
    // final. Totals stay below 1e9 + 2 * (n + m - 2) <= 1e9 + 2998, far
    // inside Number's exact range of 2^53.
    const n = moveTime.length;
    const m = moveTime[0].length;
    const infinity = Number.MAX_SAFE_INTEGER;
    const dist = Array.from({ length: n }, () => new Array(m).fill(infinity));
    const heap = []; // binary min-heap of [t, i, j]
    const push = (t, i, j) => {
        heap.push([t, i, j]);
