...
The resulting array becomes [1, 2, 0], which has the minimum possible sum
of 3 after applying the operations.
```
...
Think of dynamic programming with states to track progress and remaining
operations.

### Hint 2

Use dp[index][op1][op2] where each state tracks progress at index with
op1 and op2 operations left.

### Hint 3

At each state, try applying only operation 1, only operation 2, both in
sequence, or skip both to find optimal results.
