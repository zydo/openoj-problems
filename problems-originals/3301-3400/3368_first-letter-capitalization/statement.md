...
modified text where each word starts with a capital letter.

Each testcase supplies its own `dataset`: the DDL seeds the `user_content`
table with that testcase's rows. A word is a maximal run of English letters,
and every space is copied through exactly as written — repeated spaces too —
so the transformed text always has the same length as the original. Write your
solution as a single `SELECT` query returning three columns — `content_id`,
the unchanged text as `original_text`, and the transformed text as
`converted_text` — one row per input row, in any order.
