...
transactions have the same amount of 58, so we include both in the result
table.
...
Use MAX() inside a subquery or window function to find each day's maximum amount, then keep only the rows that reach it.
