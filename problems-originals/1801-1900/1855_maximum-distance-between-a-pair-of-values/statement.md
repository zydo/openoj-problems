...
Since both arrays are sorted in a non-increasing way, this means that for
each value in the first array, we can find the farthest value larger than
or equal to it using binary search.

### Hint 2

There is another solution using a two pointers approach. Since the first
array is non-increasing, the farthest j such that nums2[j] ≥ nums1[i] is
at least as far as the farthest j such that nums2[j] ≥ nums1[i-1]
