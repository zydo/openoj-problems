...
- The absolute difference between any 2 adjacent elements must be less
  ...
  for each index `i` where `1 <= i < arr.length` (0-indexed). `abs(x)` is
...
Input: arr = [2,2,1,2,1]
Output: 2
Explanation: We can satisfy the conditions by rearranging arr so it
becomes [1,2,2,2,1].
...
Decrement each element to the largest integer that satisfies the
conditions.
