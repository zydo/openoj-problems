...
through `n` roads. The road lengths are given as an integer array `dist`
of length `n`, where `dist[i]` describes the length of the `i`th road in
...
- For example, suppose traveling the first road takes 1.4 hours and
  ...
  first road will mean you finish traveling the second road right at the
  2 hour mark, letting you start traveling the third road immediately.
...
You can skip the first and third rest to arrive in ((7/2 + 0) + (3/2 +
0)) + ((5/2 + 0) + (5/2)) = 10 hours.
```
...
How would knowing the start time for each state help us solve the
problem?
