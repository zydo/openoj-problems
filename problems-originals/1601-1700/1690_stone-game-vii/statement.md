...
There are `n` stones arranged in a row. On each player's turn, they remove
either the leftmost stone or the rightmost stone from the row and receive
points equal to the sum of the remaining stones' values. The winner is the
...
the `i`th stone from the left, return the difference in Alice and Bob's
scores if they both play optimally.
...
The constraints are small enough for an `O(n²)` solution.
