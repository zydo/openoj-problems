...
The next closest time is the nearest time in the future. If no strictly
later time can be built, the search continues into the next day, so the
answer may be numerically smaller than the input. You may assume the given
input string is always valid. For example, "01:34", "12:09" are all valid.
...
22:22. It may be assumed that the returned time is next day's time since
