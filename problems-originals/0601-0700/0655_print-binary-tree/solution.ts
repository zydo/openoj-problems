function printTree(root: TreeNode | null): string[][] {
    ...
    let height = 0;
    const measure: [TreeNode, number][] = [[root!, 0]];
    ...
    const res: string[][] = Array.from({ length: rows }, () => new Array<string>(cols).fill(""));
    const place: [TreeNode, number, number][] = [[root!, 0, (cols - 1) >> 1]];
