...
have to process all `queries[i]` (from `i=0` to `i=queries.length-1`)
...
For each `i`, find the position of `queries[i]` with a simple scan over
`P` and then move this to the beginning.
