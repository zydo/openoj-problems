...
have done in a restaurant. More specifically
...
to each food item in alphabetical order. The first row should be a header
...
- `customerNamei` and `foodItemi` consist of lowercase and uppercase
...
Keep the frequency of all pairs `(tableNumber, foodItem)` using a
hashmap.

### Hint 2

Sort rows by `tableNumber` and columns by `foodItem`, then process the
resulted table.
