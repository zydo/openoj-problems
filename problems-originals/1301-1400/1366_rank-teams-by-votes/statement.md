...
the ranking system. Sort all teams according to the ranking system described
above.
...
Output: "XWYZ"
Explanation: X is the winner due to the tie-breaking rule. X has the same votes
as W for the first position, but X has one vote in the second position, while W
does not have any votes in the second position.
```
...
- All the characters of `votes[i]` are unique.
- All the characters that occur in `votes[0]` also occur in `votes[j]` where
  `1 <= j < votes.length`.
...
Build array rank where rank[i][j] is the number of votes for team i to be the
j-th rank.

### Hint 2

Sort the teams by rank array. If rank array is the same for two or more teams,
sort them by the ID in ascending order.
