...
`patient_id` is the unique identifier for this table. Each row contains
...
`test_id` is the unique identifier for this table. Each row represents a
COVID test result. The result can be Positive, Negative, or Inconclusive.
...
Write your solution as a single `SELECT` query returning every recovered
patient with their `recovery_time` in days.
