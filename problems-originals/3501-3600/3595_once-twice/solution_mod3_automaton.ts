function onceTwice(nums: number[]): number[] {
    ...
    const differ = ones | twos;
    const bit = differ & -differ;
    ...
    // recovers each value in full, shared bits included. Bitwise operators
    // work on 32-bit patterns directly, so negatives need no conversion.
    let onOnes = 0,
        onTwos = 0,
        offOnes = 0,
        offTwos = 0;
    ...
    // ones mask owns it; the masks are already signed 32-bit values.
    return (ones & bit) !== 0 ? [onOnes, offTwos] : [offOnes, onTwos];
}
