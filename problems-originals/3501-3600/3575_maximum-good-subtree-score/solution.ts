function goodSubtreeSum(vals: number[], par: number[]): number {
    ...
    const umask: number[] = new Array(n);
    const selectable: boolean[] = new Array(n);
    for (let i = 0; i < n; i++) {
    ...
    const subsetConvolve = (a: number[], b: number[]): number[] => {
        const res: number[] = new Array(1024).fill(NEG);
    ...
    for (let idx = n - 1; idx >= 0; idx--) {
        const u = order[idx];
        let comb: number[] = new Array(1024).fill(NEG);
        comb[0] = 0;
        for (const c of children[u]) {
            comb = subsetConvolve(comb, dp[c]);
        }

        const du: number[] = comb.slice();
        if (selectable[u]) {
            const mu = umask[u];
