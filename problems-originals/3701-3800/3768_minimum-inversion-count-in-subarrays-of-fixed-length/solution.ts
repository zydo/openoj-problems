function minInversionCount(nums: number[], k: number): number {
    ...
    let inversions = 0;
    for (let i = 0; i < k; ++i) {
        const rx = rank.get(nums[i]) as number;
        inversions += i - query(rx);
        update(rx, 1);
    }
    let best = inversions;
    for (let right = k; right < nums.length; ++right) {
        const ry = rank.get(nums[right - k]) as number;
        const rx = rank.get(nums[right]) as number;
