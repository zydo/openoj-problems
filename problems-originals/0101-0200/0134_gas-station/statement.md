...
If the total gas is less than the total cost, no starting point can complete the circuit, so return -1 immediately.

### Hint 2

If the tank goes negative somewhere between station i and station j, then no station between i and j can be the answer; restart from j + 1.

### Hint 3

One pass tracking the running tank and the total surplus finds the unique answer in O(n).
