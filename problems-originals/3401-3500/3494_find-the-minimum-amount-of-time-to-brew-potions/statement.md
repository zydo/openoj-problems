...
potion at time t = 50. At time t = 58, wizard 2 is done with the 1st potion,
...
Preparation of the 1st potion begins at time t = 1, and is completed by time
t = 4.
Preparation of the 2nd potion begins at time t = 2, and is completed by time
t = 5.
```
...
Let `x` be the current mana value. Starting from `now = f[0]`, update
`now = max(now + skill[i - 1] * x, f[i])` for `i` in `[1..n]`. Then, the final
`f[n - 1] = now + skill[n - 1] * x` for this potion.

### Hint 3

Update all other `f` values by `f[i] = f[i + 1] - skill[i + 1] * x` for `i` in
`[0..n - 2]` (in reverse order).
