...
and `email` of every user whose address satisfies all four criteria,
ordered by `user_id` in ascending order. Each testcase supplies its own
`dataset`: the script seeds the `Users` table before your query runs.
