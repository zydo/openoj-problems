...
0 to n - 1. You are given a 2D integer array edges of length n - 1, where
edges[i] = [ai, bi] indicates that there is an edge between nodes ai and
bi in the tree. You are also given a 0-indexed array coins of size n where
...
Coins at nodei can be collected in one of the following ways:
    ...
    this way is used, then for all the nodej present in the subtree of
...
Output: 11
Explanation:
Collect all the coins from node 0 using the first way. Total points =
10 - 5 = 5.
Collect all the coins from node 1 using the first way. Total points =
5 + (10 - 5) = 10.
...
Note that the value of each node ≤ 10⁴, so when t >= 14 dp[x][t] is always 0.
