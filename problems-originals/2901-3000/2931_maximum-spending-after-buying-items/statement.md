...
of value. That is, values[i][j] >= values[i][j + 1] for all
0 <= j < n - 1.
...
Return the maximum amount of money that can be spent on buying
all m * n products.
...
On each day, buy the product that minimizes values[i][values[i].length -
1], and pop it from values[i].
