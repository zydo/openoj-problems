...
For a fixed item with an index j, iterate over items with an index k > j
such that prices[k] > prices[j].

### Hint 3

Find the maximum profit[k] with the above condition. Let's call this
maximum value max_right.

### Hint 4

Do the same for items with an index i such that prices[i] < prices[j] and
find the maximum profit[i] among them. Let's call this maximum value
max_left.

### Hint 5

Now the profit when an item with the index j is the middle one would be
profit[j] + max_right + max_left.

### Hint 6

Finally, do the above procedure for all j's and find the maximum profit
among them. That would be the final answer.
