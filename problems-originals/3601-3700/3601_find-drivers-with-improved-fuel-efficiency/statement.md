...
Each testcase supplies its own `dataset`: the script seeds the `drivers` and
`trips` tables before your query runs. The result format is in the following
example.
...
+-----------+---------------+------------------+-------------------+------------------------+
Explanation: Alice Johnson (driver_id = 1):
...
`driver_id`, `driver_name`, `first_half_avg`, `second_half_avg`, and
`efficiency_improvement` — one row for each driver with trips in both halves
of the year, ordered by `efficiency_improvement` in descending order, then by
`driver_name` in ascending order.
