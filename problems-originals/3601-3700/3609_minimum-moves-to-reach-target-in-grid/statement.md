...
Output: -1
Explanation: It is impossible to reach (2, 2) from (1, 1) using the allowed moves. Thus, the answer is -1.
```
...
If the larger coordinate >= 2 × (the smaller), undo by halving the larger;
otherwise undo by subtracting the smaller from the larger.

### Hint 3

Count these undo-steps until you hit `(sx, sy)` (return the count), or
return `-1` if you drop below or get stuck.
