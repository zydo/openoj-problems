func longestValidParentheses(s string) int {
	...
	for i := 0; i < len(s); i++ {
		...
		// i-1-dp[i] sits immediately to its left. A '(' there is exactly
		// this closer's partner: a '(' and a ')' with only balanced
		// material between them is what being matched means.
		j := i - 1 - dp[i]
		if j >= 0 && s[j] == '(' {
