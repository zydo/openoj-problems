...
Output: [0,0,1,3]
Explanation:
    In the first query, all the edges in the path from 0 to 3 have a weight of
    1. Hence, the answer is 0.
    ...
    For each queries[i], it can be shown that answer[i] is the minimum number
    of operations needed to equalize all the edge weights in the path from ai
    to bi.
...
Output: [1,2,2,3]
Explanation:
    In the first query, we change the weight of edge [1,3] to 6. After this
    operation, all the edges in the path from 4 to 6 have a weight of 6.
...
Define a 2D array `freq[node][weight]` which saves the frequency of each edge
weight on the path from the root to each node.

### Hint 3

The frequency of edge weight `w` on the path from `a` to `b` is equal to
`freq[a][w] + freq[b][w] - freq[lca(a,b)][w] * 2`, where `lca(a,b)` is the
lowest common ancestor of `a` and `b` in the tree.

### Hint 4

`lca(a,b)` can be calculated using binary lifting algorithm or Tarjan
algorithm.
