...
Each bag contains a contiguous subarray, and only the endpoints of these subarrays matter.

### Hint 2

Splitting into k bags means choosing k - 1 adjacent pairs as boundaries; each boundary at positions (i, i+1) adds weights[i] + weights[i+1] to the score.

### Hint 3

The minimum score takes the k - 1 smallest adjacent-pair sums and the maximum takes the k - 1 largest.

### Hint 4

Sort the adjacent-pair sums (or use a priority queue) and return the difference of those two sums.
