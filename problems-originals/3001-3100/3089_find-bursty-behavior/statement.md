...
Each testcase supplies its own `dataset`: the DDL seeds the `Posts` table
...
`user_id`, `max_7day_posts`, the most posts the user made in any period of
7 consecutive days inside February 1–28, and `avg_weekly_posts`, the
user's February 1–28 post count divided by the 4 weeks — with one row per
user whose `max_7day_posts` is at least twice their `avg_weekly_posts`,
