...
case.
Note that 'A' and 'F' also appear in both lower and upper case, but 'R'
is greater than 'F' or 'A'.
```
...
Consider iterating through the string and storing each unique character that
occurs in a set.

### Hint 2

From `Z` to `A`, check whether both the uppercase and lowercase version
occur in the set.
