...
We had some two-dimensional coordinates, like "(1, 3)" or "(2, 0.5)". Then we
...
All coordinates in the answer have exactly one space between them (occurring
after the comma).

The possibilities form a set, but the answer is listed in one fixed order:
the digit run is split after its k-th digit with k ascending from 1; on each
side, every form carrying a decimal point comes first — point after the
side's first digit, then after its second, and so on — with the plain-integer
form last; and the left number varies slower than the right.
