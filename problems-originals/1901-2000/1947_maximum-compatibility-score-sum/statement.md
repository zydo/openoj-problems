...
array that contains the answers of the `i`th student (0-indexed). The answers
of the mentors are represented by a 2D integer array `mentors` where
`mentors[j]` is an integer array that contains the answers of the `j`th mentor
(0-indexed).
...
Calculate the compatibility score for each student-mentor pair first.

### Hint 2

Try every permutation of students against the original mentors array.

### Hint 3

Since m <= 8, you can track which mentors are already used with a bitmask and memoize on it.
