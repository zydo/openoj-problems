...
testcase's `Accounts` rows (whichever are present) before your query runs.
...
`category` / `accounts_count` — one row per salary category, with
categories that have no accounts reported as 0.
