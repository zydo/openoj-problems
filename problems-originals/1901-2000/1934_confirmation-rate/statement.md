...
Each testcase supplies its own `dataset`: the DDL seeds the `Signups` and
`Confirmations` tables with that testcase's rows. The result format is
...
A user with no confirmation requests gets rate `0.00`; every other user
gets their confirmed-to-total ratio rounded to two decimal places. Write
your solution as a single `SELECT` query returning two columns —
`user_id` and `confirmation_rate`, in that order — with one row for every
user in the `Signups` table.
