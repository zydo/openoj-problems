...
higher score will have the same or smaller `student_count` compared to a
row recording a lower score. More formally, for every two rows `i` and `j`
in the table, if `score_i` > `score_j` then `student_count_i` <=
`student_count_j`.
...
the smallest one. If the input data is not enough to determine the score,
report `-1`.

Return the result table in any order.

Each testcase supplies its own `dataset`: the DDL seeds the `Schools` and
`Exam` tables with that testcase's rows. The result format is shown in the
following example.
...
For a school, the chosen score is the `Exam` row whose `student_count` is
no larger than the school's `capacity` and whose `student_count` is as
large as possible; when several rows share that `student_count`, the
smallest such `score` is chosen. A school with no qualifying row — its
capacity is below every `student_count` in the `Exam` table — reports
`-1`. Write your solution as a single `SELECT` query returning two columns
— `school_id` and `score`, in that order — with one row for every school
