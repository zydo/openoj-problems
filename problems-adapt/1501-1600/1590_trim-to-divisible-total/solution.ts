function shortestTrim(nums: number[], p: number): number {
    const n = nums.length;
    // JS/TS numbers are exact doubles up to 2^53, comfortably covering
    // the 1e14 total, so no extra accumulation care is needed.
    let total = 0;
    ...
    const lastIndex = new Map<number, number>([[0, -1]]);
    let running = 0;
    let best = n;
