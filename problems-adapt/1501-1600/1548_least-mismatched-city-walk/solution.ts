function leastMismatchedWalk(n: number, roads: number[][], names: string[], targetPath: string[]): number[] {
    ...
    const dp: number[][] = Array.from({ length: pathLength }, () => new Array(n).fill(0));
    const parent: number[][] = Array.from({ length: pathLength }, () => new Array(n).fill(-1));
    ...
    const path = new Array<number>(pathLength).fill(0);
    let city = endCity;
