...
class Solution {
    ...
    public int callMajority(SealedBag arrayReader) {
        ...
        if (seen4 || seen0) {
            ...
            if (seen4) {
                ...
                anchorMatch = 2;
                anchorDiff = 1;
                matchIndex = -2; // sentinel: unresolved
            }
            int diffResult = seen4 ? 2 : 0;

            int countMatch = 0;
            for (int[] entry : results) {
                if (entry[1] == matchResult) {
                    countMatch++;
                }
            }
            int countDiff = results.size() - countMatch;
