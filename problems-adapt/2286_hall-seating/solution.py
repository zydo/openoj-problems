class HallSeating:
    ...
    def block(self, k: int, lastRow: int) -> list[int]:
        row = self._first_at_least(1, 0, self.n - 1, 0, lastRow, k)
        if row == -1:
            return []
    ...
    def spread(self, k: int, lastRow: int) -> bool:
        if self._range_sum(1, 0, self.n - 1, 0, lastRow) < k:
            return False
        row = 0
        while k > 0:
            row = self._first_at_least(1, 0, self.n - 1, row, lastRow, 1)
