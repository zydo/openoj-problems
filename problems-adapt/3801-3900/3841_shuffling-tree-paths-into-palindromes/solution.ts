function rearrangeablePath(n: number, edges: number[][], s: string, queries: string[]): boolean[] {
    ...
    // the LCA. The packed key passes 2^32, past 32-bit bitwise range, so it
    // is built arithmetically — every value stays far below 2^53, where
    // plain numbers are exact.
    let levels = 1;
    while (1 << levels <= walkLength) ++levels;
    const table: number[][] = [new Array<number>(walkLength)];
    ...
    for (let level = 1; level < levels; ++level) {
        ...
        const current = new Array<number>(length);
