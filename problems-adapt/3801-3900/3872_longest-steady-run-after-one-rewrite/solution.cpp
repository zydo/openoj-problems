class Solution {
  public:
    int longestSteadyRun(vector<int> &nums) {
        ...
        for (int p = 0; p < n; p++) {
            if (p >= 1) {
                best = max(best, left[p - 1] + 1);
            }
            if (p <= n - 2) {
                best = max(best, right[p + 1] + 1);
            }
            if (1 <= p && p <= n - 2) {
                int diff = nums[p + 1] - nums[p - 1];
                if (diff % 2 == 0) {
                    int d = diff / 2;
                    int left_len = (p >= 2 && nums[p - 1] - nums[p - 2] == d) ? left[p - 1] : 1;
                    int right_len = (p <= n - 3 && nums[p + 2] - nums[p + 1] == d) ? right[p + 1] : 1;
                    best = max(best, left_len + right_len + 1);
                }
            }
        }
        return best;
