# Solutions — The Longest Substring One Deletion From a Palindrome

## Rolling interval dynamic programming

For each interval, track two facts: whether it is a palindrome and whether it
is one deletion fixable. An interval `s[left..right]` is a palindrome when its
ends match and its interior is a palindrome. It is fixable when
deleting its left endpoint leaves a palindrome, deleting its right endpoint
leaves a palindrome, or its matching endpoints surround a fixable
...
palindrome results from lengths one and two smaller, plus fixable
...
fixable because deleting its character leaves the empty palindrome;
