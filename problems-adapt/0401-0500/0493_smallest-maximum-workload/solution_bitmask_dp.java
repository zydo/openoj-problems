class Solution {
    ...
    public int smallestMaxWorkload(int[] jobs, int k) {
        ...
        for (int mask = 1; mask < size; mask++) {
            int low = mask & -mask;
            sums[mask] = sums[mask ^ low] + jobs[Integer.numberOfTrailingZeros(low)];
        }
        long total = sums[full];
        ...
        for (int worker = 2; worker <= k; worker++) {
            ...
            for (int mask = 1; mask < size; mask++) {
                ...
                for (int sub = rest; ; sub = (sub - 1) & rest) {
                    ...
                    long carried = prev[rest ^ sub];
                    long load = sums[sub | low];
