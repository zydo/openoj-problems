class Solution {
    ...
    public int greatestPalindromicProduct(int n) {
        ...
        for (long half = hi; half >= lo; --half) {
            String text = Long.toString(half);
            String mirrored = new StringBuilder(text).reverse().toString();
            long palindrome = Long.parseLong(text + mirrored);
            // Double sqrt rounds above 2^53, so settle the floor exactly.
            long root = (long) Math.sqrt(palindrome);
            while (root * root > palindrome) --root;
            while ((root + 1) * (root + 1) <= palindrome) ++root;
            // The larger factor of any pair lies between hi and the integer
            // square root; the cofactor check rejects pairs whose cofactor
            // runs a digit long.
            for (long factor = hi; factor >= root; --factor) {
                if (palindrome % factor == 0) {
                    long other = palindrome / factor;
                    if (other >= lo && other <= hi) return (int) (palindrome % 1337);
                }
            }
        }
