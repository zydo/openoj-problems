function countShrinkingNumbers(s: string, k: number): number {
    const MOD = 1000000007;
    const L = s.length;
    // f[x] = number of operations to reduce x to 1.
    const f = new Array<number>(L + 1).fill(0);
    for (let x = 2; x <= L; x++) {
        let bits = 0,
            t = x;
        while (t) {
    ...
    const C: number[][] = [];
    for (let i = 0; i <= L; i++) {
        const row = new Array<number>(L + 1).fill(0);
        row[0] = 1;
        for (let j = 1; j <= i; j++) {
    ...
    const cnt = new Array<number>(L + 1).fill(0);
    let ones = 0;
    for (let i = 0; i < L; i++) {
        if (s[i] === "1") {
