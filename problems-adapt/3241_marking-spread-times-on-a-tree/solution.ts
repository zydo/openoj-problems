function spreadTimes(edges: number[][]): number[] {
    ...
    const last = new Array<number>(n).fill(0); // max time within u's subtree
    const lastNo = new Array<number>(n).fill(-1); // child attaining last[u]
    const second = new Array<number>(n).fill(0); // second-best child contribution
    ...
    const up = new Array<number>(n).fill(0); // best time outside u's subtree
