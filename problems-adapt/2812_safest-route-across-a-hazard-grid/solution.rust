impl Solution {
    pub fn maximum_clearance(grid: Vec<Vec<i32>>) -> i32 {
        ...
        let reachable = |threshold: i32| -> bool {
            ...
            seen[0][0] = true;
            let mut dq: Vec<(usize, usize)> = vec![(0, 0)];
            let mut h = 0usize;
            while h < dq.len() {
                let (r, c) = dq[h];
                h += 1;
                ...
                for &(dr, dc) in &dirs {
                    ...
                    if nr >= 0 && nr < n as i64 && nc >= 0 && nc < n as i64 {
                        let (nr, nc) = (nr as usize, nc as usize);
                        if !seen[nr][nc] && dist[nr][nc] >= threshold {
                            seen[nr][nc] = true;
                            dq.push((nr, nc));
                        }
                    }
        ...
        // largest feasible v over [0, 2n]. A hazard on a corner pins its
        // dist to 0, capping the answer at 0.
        let mut lo: i32 = 0;
        let mut hi: i32 = 2 * n as i32;
        let mut ans: i32 = 0;
        while lo <= hi {
            let mid = lo + (hi - lo) / 2;
            if reachable(mid) {
