// Problem-provided driver for 2795 one-settled-report-per-
// promise. Assembled into every submission by the judge ahead of
// the submitted code; never editable in the editor. This file is
...
class SettleDriver {
    ...
    constructor([functions]: any[], budget?: any) {
        ...
        this.functions = (functions as SettleOutcome[]).map((spec): FnPromise => () => {
            ...
            // handler only (mirroring settle_driver.js) — chaining it
    ...
    // Hand this case's functions to the submission's settleAll,
    ...
    async drive(settleAll: (functions: FnPromise[]) => Promise<unknown[]>): Promise<void> {
        const returned = settleAll(this.functions);
        if (!returned || typeof returned.then !== "function") {
            throw new Error("settleAll must return a promise");
        }
        let fired = 0;
