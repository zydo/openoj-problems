...
type FnSupplier = () => Promise<unknown>;

function gatherAll(functions: FnSupplier[]): Promise<unknown[]> {
    return new Promise((resolve, reject) => {
        const results = new Array<unknown>(functions.length);
...
class Solution {
    run(driver: BatchDriver) {
        return driver.drive(gatherAll);
    }
}
