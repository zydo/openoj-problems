class Solution {
  public:
    int nthDualMultiple(int n, int a, int b) {
        ...
        // reaches 4e13, so the search runs in long long; only the value
        ...
        long long lcm = a / g * b;
        long long lo = 1, hi = (long long)n * std::min(a, b);
        while (lo < hi) {
