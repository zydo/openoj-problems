...
class Solution:
    def dropZeroOnlySubtrees(self, root: Optional[TreeNode]) -> Optional[TreeNode]:
        ...
        if root is None:
            return None
        root.left = self.dropZeroOnlySubtrees(root.left)
        root.right = self.dropZeroOnlySubtrees(root.right)
