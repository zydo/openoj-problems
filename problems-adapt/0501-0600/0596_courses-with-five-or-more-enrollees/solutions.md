# Solutions — Courses With Five or More Enrollees

## Count the students in each class

A class's enrollment is exactly its rows: every `Enrollments` row is one
...
One aggregation pass reads the `N` rows of `Enrollments` once and materializes
...
and a correlated `(SELECT COUNT(*) FROM Enrollments c2 WHERE c2.class =
Enrollments.class) >= 5` states the same test per row — but the grouped
