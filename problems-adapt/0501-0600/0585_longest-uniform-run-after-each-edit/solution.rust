...
impl Solution {
    pub fn longest_uniform_run(s: String, rewriteChars: String, rewritePositions: Vec<i32>) -> Vec<i32> {
        let n = s.len();
        if n == 0 {
            return Vec::new();
        }
        let mut tree = SegTree::new(s.as_bytes());
        let mut result = Vec::with_capacity(rewritePositions.len());
        for (i, &idx) in rewritePositions.iter().enumerate() {
            tree.update(1, 0, n - 1, idx as usize, rewriteChars.as_bytes()[i]);
