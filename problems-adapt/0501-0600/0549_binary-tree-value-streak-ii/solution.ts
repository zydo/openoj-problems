// Run = [inc, dec, value]: the runs topping out at a node — the longest
// +1-downward and the longest -1-downward — plus that node's value.
type Run = [number, number, number];

// Frame = [node, state, left run, right run]; state counts the children
// still to visit: 0 = left pending, 1 = right pending, 2 = ready to judge
// the node itself.
type Frame = [TreeNode, number, Run, Run];

function longestValueStreak(root: TreeNode | null): number {
    ...
    while (stack.length > 0) {
        ...
        if (frame[1] === 0) {
            ...
            let inc = 1;
            let dec = 1;
            for (const child of [frame[2], frame[3]] as Run[]) {
            ...
            const report: Run = [inc, dec, value];
            if (stack.length > 0) {
