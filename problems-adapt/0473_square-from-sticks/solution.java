...
class Solution {
    ...
    public boolean canFormSquare(int[] lengths) {
        long total = 0;
        for (int v : lengths) total += v;
        ...
        Integer[] sticks = new Integer[lengths.length];
        for (int i = 0; i < lengths.length; i++) sticks[i] = lengths[i];
