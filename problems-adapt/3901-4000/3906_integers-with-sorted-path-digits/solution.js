 ...
 * @return {number}
 */
var countSortedPathIntegers = function (l, r, directions) {
    const selected = new Array(16).fill(false);
    let row = 0;
    let column = 0;
    selected[0] = true;
    ...
    function countUpTo(bound) {
        if (bound < 0) return 0;
        const digits = String(bound).padStart(16, "0").split("").map(Number);
        let dp = Array.from({ length: 2 }, () => new Array(11).fill(0));
        dp[1][10] = 1;
        for (let position = 0; position < 16; position++) {
            const next = Array.from({ length: 2 }, () => new Array(11).fill(0));
