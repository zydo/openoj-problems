impl Solution {
    pub fn cells_detached_by_removals(grid: Vec<Vec<i32>>, removals: Vec<Vec<i32>>) -> Vec<i32> {
        let m = grid.len();
        let n = grid[0].len();
        let top = m * n;
        ...
        let mut res: Vec<i32> = vec![0; removals.len()];
        for k in (0..removals.len()).rev() {
            let r = removals[k][0] as usize;
            let c = removals[k][1] as usize;
            if grid[r][c] != 1 {
                continue;
            }
