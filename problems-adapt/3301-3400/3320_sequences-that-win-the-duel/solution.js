...
var countDuelWins = function (s) {
    ...
    let dp = Array.from({ length: 3 }, () => new Array(width).fill(0));
    for (let m = 0; m < 3; ++m) dp[m][offset + DELTA[m][code(s[0])]] = 1;
    for (let i = 1; i < n; ++i) {
        const a = code(s[i]);
        const ndp = Array.from({ length: 3 }, () => new Array(width).fill(0));
        for (let t = 0; t < 3; ++t) {
            const u = (t + 1) % 3;
            const v = (t + 2) % 3;
            const d = DELTA[t][a];
