 ...
 * @return {number}
 */
var countBalancedRearrangements = function (num) {
    // A balanced permutation is decided by how many copies of each
    // digit land on even indices: a_d of the cnt[d] copies, with
    // sum(a_d) = ceil(n/2) and sum(d * a_d) = total / 2 (the odd-index
    // sum is then implied by the total), each choice contributing
    // C(cnt[d], a_d). A bottom-up DP over digits with states (even
    // slots used, even-index sum) accumulates those binomial products.
    // Arranging the two chosen multisets over the even and odd slots
    // multiplies by even_count! * odd_count! / cnt[d]!, folded in via
    // one modular inverse at the end. All arithmetic is modulo
    // 1e9 + 7, iterative — no recursion.
    ...
    let dp = Array.from({ length: evenCount + 1 }, () => new Array(half + 1).fill(0));
    dp[0][0] = 1;
    for (let d = 0; d < 10; d++) {
        const c = cnt[d];
        if (c === 0) continue;
        const ndp = Array.from({ length: evenCount + 1 }, () => new Array(half + 1).fill(0));
