class Solution {
    ...
    read(charSource: CharSource, queries: number[], buf: string[]): number {
        let total = 0;
        for (const n of queries) {
            total += this.transfer(charSource, n, buf, total);
        }
        return total;
    }

    private transfer(charSource: CharSource, n: number, buf: string[], offset: number): number {
        let transferred = 0;
        while (transferred < n) {
            if (this.buf4Index === this.buf4Count) {
                this.buf4Count = charSource.read4(this.buf4);
                this.buf4Index = 0;
