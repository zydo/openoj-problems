...
A path crosses at most `m + n - 1 = 399` rooms whose values are bounded by 1000 in magnitude, so no honest need exceeds `1 + 399 * 1000 = 399001`; the sentinel 10⁹ towers over that while staying three powers of ten below 32-bit overflow, and it never wins a `min` against a computed entry. The grid itself is never mutated.
