# Solutions — The In-Order Pair Sum

## Two pointers converging from both ends

The array is already sorted, so the smallest and the largest values can speak for every candidate pair at once: their sum is the largest sum any pair involving `nums[low]` can reach and the smallest sum any pair involving `nums[high]` can reach. Comparing that one sum against `target` therefore rules out an entire row of the pair table — if `nums[low] + nums[high]` is too small, no partner can rescue `nums[low]`; if too large, none can rescue `nums[high]`. And since the walk keeps only two indexes, it satisfies the statement's requirement of constant extra space, which rules out the hash map that solved the unsorted version.
