...
The traversal is deliberately iterative rather than the classic recursive `1 + max(height(left), height(right))`. The tree may hold up to `10⁴` nodes, and a skewed chain makes the recursion depth grow with the node count — around `10⁴` nested calls, deep enough that some language runtimes reject it as a stack overflow. The explicit frontier is the same order of memory without touching any call stack, and it behaves identically on chains, balanced trees, and the empty tree.
