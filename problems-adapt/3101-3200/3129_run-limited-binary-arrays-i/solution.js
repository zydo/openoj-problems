...
var countRunLimitedArrays = function (zero, one, limit) {
    ...
    const f0 = Array.from({ length: zero + 1 }, () => new Array(one + 1).fill(0));
    const f1 = Array.from({ length: zero + 1 }, () => new Array(one + 1).fill(0));
    const pref0 = Array.from({ length: zero + 1 }, () => new Array(one + 2).fill(0));
