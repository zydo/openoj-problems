function countRunLimitedArrays(zero: number, one: number, limit: number): number {
    ...
    const w = one + 1;
    const vert = new Array<number>(w).fill(0);
    let prevOnes = new Array<number>(w).fill(0);
