...
impl Solution {
    pub fn earliest_finalize_time(nums: Vec<i32>, resets: Vec<i32>) -> i32 {
        let n = nums.len();
        let m = resets.len();
        ...
        while low <= high {
            ...
            if Self::check(&nums, &resets, &first, total, mid as usize) {
                high = mid - 1;
            } else {
                low = mid + 1;
            }
    ...
    fn check(nums: &[i32], resets: &[i32], first: &[i64], total: i64, t: usize) -> bool {
