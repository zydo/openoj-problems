function sumOfExtremes(nums: number[], k: number): number {
    // Number of (a, b) with 0<=a<=A, 0<=b<=B, a+b<=K.
    function countPairs(A: number, B: number, K: number): number {
    ...
    const Lmax = new Array<number>(n);
    const Rmax = new Array<number>(n);
    let stack: number[] = [];
    for (let i = 0; i < n; i++) {
    ...
    const Lmin = new Array<number>(n);
    const Rmin = new Array<number>(n);
    stack = [];
    for (let i = 0; i < n; i++) {
        while (stack.length && nums[stack[stack.length - 1]] >= nums[i]) stack.pop();
