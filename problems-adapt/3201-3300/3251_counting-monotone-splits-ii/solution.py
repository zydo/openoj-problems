...
class Solution:
    def countMonotoneSplits(self, nums: List[int]) -> int:
        ...
        # and is re-summed into the next pref. That makes each position
        # cost O(max(nums)) rather than O(max(nums)^2), which is what fits
        # n = 2000 values up to 1000. The lookup index stays inside the
        # previous row (a rise subtracts back out of v), and every stored
        # value is reduced below 10**9 + 7, so intermediates stay under
        # 2 * (10**9 + 6).
        mod = 10**9 + 7
