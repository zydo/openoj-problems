function countBalancedCorners(grid: string[][]): number {
    ...
    let prevSum: number[] = new Array<number>(cols).fill(0);
    let prevX: number[] = new Array<number>(cols).fill(0);
    let total = 0;
    for (let r = 0; r < grid.length; ++r) {
        const curSum = new Array<number>(cols).fill(0);
        const curX = new Array<number>(cols).fill(0);
        let runSum = 0;
        let runX = 0;
        const above = r > 0;
