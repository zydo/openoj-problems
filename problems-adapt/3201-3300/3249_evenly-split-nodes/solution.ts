function countEvenlySplitNodes(edges: number[][]): number {
    ...
    const isGood = new Array<boolean>(n).fill(true);
    const seenChild = new Array<boolean>(n).fill(false);
    const firstSize = new Array<number>(n).fill(0);
