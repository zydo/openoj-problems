class Solution {
  public:
    int countAlternatingTrios(vector<int> &colors, int k) {
        ...
        // seam), tracking the alternating run ending there; each position
        // credits anchor p - (k - 1) when it is a real start (0..n-1) and
        // the run has reached k. Anchors are bounded to one lap, so nothing
        // double counts.
        int n = colors.size();
        int count = 0;
        int run = 0;
