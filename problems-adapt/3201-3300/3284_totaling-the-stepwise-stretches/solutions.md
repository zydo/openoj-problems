# Solutions — Totaling The Stepwise Stretches

## Run-Ending Sum Scan

Every stepped stretch lies inside a maximal run whose steps are all
...
current index, and `ending`, the sum of the values of **all** stepped
stretches that end at the current index. Adding every position's `ending`
into a running total counts each stepped stretch exactly once —
...
sum is discarded and rebuilt from what is genuinely stepped there:
the pair `[prev, x]` plus the singleton `[x]`, i.e. `ending =
nums[i - 1] + 2 * nums[i]`. That pair straddling the flip boundary is easy
to miss, yet it is stepped — a length-2 span checks only one step.
