...
Three ways to the same minimax answer: a bottleneck Dijkstra that carries the
running maximum, a binary search over candidate caps each checked with a plain
BFS, and a Kruskal-style union-find whose answer is the edge that first joins
the two corners.
