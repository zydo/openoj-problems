class Solution {
  public:
    ...
    int countGcdPairs(vector<int> &nums) {
        ...
        vector<vector<long long>> ndp(maxVal + 1, vector<long long>(maxVal + 1, 0));
        dp[0][0] = 1;
        for (int x : nums) {
            ...
            for (int g1 = 0; g1 <= maxVal; g1++) {
                const vector<long long> &row = dp[g1];
                for (int g2 = 0; g2 <= maxVal; g2++) {
                    long long cur = row[g2];
                    if (cur == 0)
                        continue;
                    int ng1 = gcdEuclid(g1, x);
                    ndp[ng1][g2] = (ndp[ng1][g2] + cur) % MOD;
                    int ng2 = gcdEuclid(g2, x);
