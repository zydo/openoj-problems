# Solutions — Counting the Tightest Letter Matches
...
the search from all index quadruples down to 26 letters.

## First and last occurrence per letter

A match's span is `j - a`, and any match longer than one character can
shrink: keeping the leading character of each substring leaves the
match `(i, i, a, a)` valid, `a` unchanged, and `j` only smaller. No
match with `j > i` can therefore hold the minimum, so the optimal set
...
no valid match exists. Indices stay below 2 * 10^5 and the count below
