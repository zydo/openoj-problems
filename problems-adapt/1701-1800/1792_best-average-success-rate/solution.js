 ...
 * @param {number} extraTrials
 * @return {number}
 */
var bestAverageSuccessRate = function (batches, extraTrials) {
    ...
    const heap = batches.map(([p, t]) => [-gain(p, t), p, t]);
    heapify(heap);
    for (let i = 0; i < extraTrials; i++) {
    ...
    let f = heap[0][1] / heap[0][2];
    let c = 0;
    for (let i = 1; i < heap.length; i++) {
        const x = heap[i][1] / heap[i][2];
        const t = f + x;
