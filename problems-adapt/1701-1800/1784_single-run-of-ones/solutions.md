# Solutions — A Single Run of Ones
...
On `"110100"` a second run starts right after the interior zero, so the
answer is false; on `"11100"` the lone run `"111"` is never followed by
another '1', so the answer is true.
