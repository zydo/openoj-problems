/**
 * @param {number} scoreLimit
 * @param {number} stopScore
 * @param {number} drawMaximum
 * @return {number}
 */
var boundedFinalScoreProbability = function (scoreLimit, stopScore, drawMaximum) {
    if (stopScore === 0 || scoreLimit >= stopScore - 1 + drawMaximum) {
        return 1.0;
    }
    // dp[i] = probability of ever holding exactly i points.
    const dp = new Array(scoreLimit + 1).fill(0.0);
    dp[0] = 1.0;
