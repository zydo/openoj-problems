...
One baseline that popcounts each value in isolation, and one linear
recurrence that reaches back into the output it is building — the
`O(n log n)` easy road set against the follow-up's `O(n)` target.
