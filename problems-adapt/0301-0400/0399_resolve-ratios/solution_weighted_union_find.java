...
class Solution {
    ...
    public double[] resolveRatios(String[][] pairs, double[] ratios, String[][] queries) {
        ...
        for (int i = 0; i < pairs.length; i++) {
            add(parent, weight, size, pairs[i][0]);
            add(parent, weight, size, pairs[i][1]);
            unite(parent, weight, size, pairs[i][0], pairs[i][1], ratios[i]);
        }
    ...
    private double ratioToRoot(Map<String, String> parent, Map<String, Double> weight, String x) {
        ...
        // cancel in pairs, leaving exactly x / root.
        double ratio = 1.0;
