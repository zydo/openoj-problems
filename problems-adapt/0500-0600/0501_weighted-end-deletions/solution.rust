impl Solution {
    pub fn max_weighted_end_score(nums: Vec<i32>, weights: Vec<i32>) -> i32 {
        let m = weights.len();
        let n = nums.len();
        let neg_inf = -(1i64 << 60);
        ...
        for i in (0..m).rev() {
            for l in 0..=i {
                ...
                let take_left = prev[l + 1] + weights[i] as i64 * nums[l] as i64;
                let take_right = prev[l] + weights[i] as i64 * nums[n - 1 - r] as i64;
