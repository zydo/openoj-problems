impl Solution {
    pub fn count_covering(intervals: Vec<Vec<i32>>, queries: Vec<i32>) -> Vec<i32> {
        ...
        let mut starts: Vec<i32> = intervals.iter().map(|f| f[0]).collect();
        let mut ends: Vec<i32> = intervals.iter().map(|f| f[1]).collect();
