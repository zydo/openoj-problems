...
class PrefixSuffixIndex {
    constructor(words) {
        this.prefixes = PrefixSuffixIndex.makeNode();
        this.suffixes = PrefixSuffixIndex.makeNode();
        words.forEach((word, index) => {
            let node = this.prefixes;
            for (const letter of word) {
                const slot = letter.charCodeAt(0) - 97;
                if (node.children[slot] === null) {
                    node.children[slot] = PrefixSuffixIndex.makeNode();
                }
    ...
    static makeNode() {
        return { children: new Array(26).fill(null), indices: [] };
    }
