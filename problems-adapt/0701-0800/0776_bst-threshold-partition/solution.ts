function splitTreeByThreshold(root: TreeNode | null, target: number): (TreeNode | null)[] {
    ...
    const small: TreeNode = new TreeNode();
    const large: TreeNode = new TreeNode();
    ...
    let smallTail: TreeNode = small;
    let largeTail: TreeNode = large;
    let node: TreeNode | null = root;
    while (node !== null) {
        if (node.val <= target) {
            const following: TreeNode | null = node.right;
            node.right = null;
            smallTail.right = node;
            smallTail = node;
            node = following;
        } else {
            const following: TreeNode | null = node.left;
            node.left = null;
            largeTail.left = node;
            largeTail = node;
            node = following;
