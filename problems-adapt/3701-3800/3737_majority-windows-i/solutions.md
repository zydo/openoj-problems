# Solutions — Majority Windows I

## Fixed-left sweep with a running count

A window has `target` as its majority exactly when `target` occurs strictly
...
window by its endpoints: fix the left endpoint, then extend the right end
...
window is visited by one left endpoint with one right endpoint. With
