 ...
 * @return {number[]}
 */
var rangeLevelingCosts = function (nums, k, queries) {
    const n = nums.length;
    ...
    const quot = nums.map((value) => Math.floor(value / k));
    ...
    const leftChild = new Array(nodeCap).fill(0);
    const rightChild = new Array(nodeCap).fill(0);
    const nodeCount = new Array(nodeCap).fill(0);
    const nodeSum = new Array(nodeCap).fill(0);
    let used = 1;
