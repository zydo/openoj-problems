# Solutions — Two Couriers On One Clock
...
a fixed `T` the fit is decided by three counting conditions — courier 1 needs
`d[0] <= T - floor(T / r[0])` usable hours, courier 2 needs its analogue, and
the combined load must fit into the `T - floor(T / lcm(r[0], r[1]))` hours
in which at least one courier is awake, since the multiples of the least
common multiple block both couriers at once. The conditions are also
sufficient: park each delivery in an hour only its own courier can use first,
