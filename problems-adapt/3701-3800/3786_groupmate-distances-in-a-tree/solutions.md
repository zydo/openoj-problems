# Solutions — Groupmate Distances In A Tree

## Edge contribution over subtree label counts

A label-mate pair's distance is the number of edges on its path, so the sum
over pairs can be re-read as a sum over edges: each edge collects one unit
from exactly the same-label pairs it separates. Deleting the edge between
node `c` and its parent splits the tree into `c`'s subtree and everything
else; if `a` nodes of label `g` lie inside, then `total[g] - a` lie outside,
and exactly `a * (total[g] - a)` label-mate pairs cross the split, each
paying
