...
var smallestSumGap = function (nums) {
    ...
    const gt = (a, b) => a > b; // max-heap
    const lt = (a, b) => a < b; // min-heap

    // left_min[i] = sum of the n smallest values among nums[0..i] (valid when i >= n-1)
    const leftMin = new Array(total).fill(null);
    let heap = [];
    let running = 0;
    ...
    const rightMax = new Array(total).fill(null);
    let heap2 = [];
    let running2 = 0;
