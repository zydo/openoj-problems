...
plain ascending order. The single pass exploits that locally, asking at
each step which half the seam spares and whether that half's bounds
admit the target. The two-stage search exploits it once and for all — a
first binary search pins the seam down by locating the array's smallest
value, after which the array is two ordinary sorted runs and a plain
binary search runs in whichever run can hold the target.
