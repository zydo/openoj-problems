...
impl Solution {
    pub fn find_grid_words(board: Vec<Vec<String>>, words: Vec<String>) -> Vec<String> {
        let m = board.len();
        let n = board[0].len();
        ...
        // current path); the BTreeSet dedups words and yields sorted output.
        ...
        for i in 0..m {
            for j in 0..n {
                Self::dfs(&grid, i as i64, j as i64, &root, &mut seen, &mut found, &dirs);
            }
        }
