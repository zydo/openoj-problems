...
and what is left to find is a pair summing to its negation. The sorted
variant pins the smallest value and hunts the pair with converging pointers
that need ordered ground to walk; the hash variant pins whichever value is in
hand and asks a set of already-passed values whether the partner it needs has
shown up. One buys its linear pair-hunt with a sort, the other with a table —
and either way the overall bill is quadratic, because there are `n` pins to
try.
