impl Solution {
    pub fn longest_palindrome_path(n: i32, edges: Vec<Vec<i32>>, label: String) -> i32 {
        let n = n as usize;
        ...
        let codes: Vec<u8> = label.bytes().collect();
        let mut memo: Vec<i32> = vec![-1; (1usize << n) * n * n];
        ...
        fn dp(
            ...
            for &u in &adj[left] {
                ...
                for &v in &adj[right] {
                    ...
                    let cand = dp(adj, codes, memo, n, mask | (1usize << u) | (1usize << v), u, v);
                    if cand > best {
                        best = cand;
                    }
                }
        ...
        for i in 0..n {
            let length = dp(&adj, &codes, &mut memo, n, 1usize << i, i, i);
            if length > answer {
                answer = length;
            }
        }
        for e in &edges {
            ...
            let v = e[1] as usize;
            if codes[u] == codes[v] {
                let length = dp(&adj, &codes, &mut memo, n, (1usize << u) | (1usize << v), u, v);
                if length > answer {
                    answer = length;
                }
            }
