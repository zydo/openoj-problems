...
Let `dp[lane]` be the fewest lane changes needed to be standing on that lane
...
lane change costs exactly one jump, so the new `dp[lane]` is the smaller
