...
class Solution {
    ...
    public int findCheapestRoute(MazeController maze) {
        Map<Long, Integer> cost = new HashMap<>();
        Map<Long, Integer> dist = new HashMap<>();
        long startKey = key(0, 0);
        cost.put(startKey, 0);
        int[] foundTarget = maze.isTarget() ? new int[] { 0, 0 } : null;
        ...
        Deque<int[]> stack = new ArrayDeque<>(); // r, c, nextDir index
        ...
        while (!stack.isEmpty()) {
            ...
            while (idx < DIRS.length) {
                ...
                int nr = r + DR[idx],
                    nc = c + DC[idx];
                idx += 1;
                if (maze.canMove(direction) && !cost.containsKey(key(nr, nc))) {
                    cost.put(key(nr, nc), maze.move(direction));
                    if (maze.isTarget()) {
