# Solutions — Most Divisors From a Factor Budget

Every full divisor carries at least one copy of each prime factor, so for
`n = p^a * q^b * ...` the number of full divisors is exactly
