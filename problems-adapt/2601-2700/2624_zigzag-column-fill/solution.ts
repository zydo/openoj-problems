// The `declare global` merge makes every number[] carry zigzagColumns() from this
...
declare global {
    interface Array<T> {
        zigzagColumns(rowsCount: number, colsCount: number): number[][];
    }
}

Array.prototype.zigzagColumns = function <T>(this: T[], rowsCount: number, colsCount: number): number[][] {
...
function zigzagColumns(nums: number[], rowsCount: number, colsCount: number): number[][] {
    return nums.zigzagColumns(rowsCount, colsCount);
}
