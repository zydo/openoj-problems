// bundle-provided driver for 2627 waiting-for-the-calls-to-settle.
// Assembled into every submission by the judge ahead of the submitted
// code; never editable in the editor. This file is the hidden implementation — solvers see only
...
class QuietWindowCase {
    ...
    drive(settleCalls: (fn: (...inputs: any[]) => void, t: number) => (...args: any[]) => void): void {
        ...
        try {
            ...
            const settled = settleCalls(fn, this.t);
            if (typeof settled !== "function") {
                throw new Error("settleCalls must return a function");
            }
            ...
            // settleCalls schedules at most one timer per outstanding window,
            // but the cap keeps any runaway rescheduling honest anyway.
            for (;;) {
