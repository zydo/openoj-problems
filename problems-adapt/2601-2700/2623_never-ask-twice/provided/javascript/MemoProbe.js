// Problem-provided driver for 2623 never-ask-twice. Assembled into every
...
// as the statement's Input). drive() receives the submission's cacheResults
// function, builds the underlying function named by fnName, wraps it in a
// call-counting shim, then replays the script against whatever cacheResults()
...
class MemoProbe {
    ...
    // Replay this case's script against the submission's cacheResults function.
    drive(memoizeFn) {
        if (typeof memoizeFn !== "function") {
            throw new Error("drive expects the submission's cacheResults function");
        }
        let calls = 0;
        ...
        if (typeof wrapped !== "function") {
            throw new Error("cacheResults must return a function");
        }
