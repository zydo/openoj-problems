impl Solution {
    pub fn count_rising_paths(grid: Vec<Vec<i32>>) -> i32 {
        ...
        for &(v, i, j) in &cells {
            for (di, dj) in dirs {
                ...
                if ni >= 0 && ni < m as isize && nj >= 0 && nj < n as isize && grid[ni as usize][nj as usize] > v {
                    dp[i][j] = (dp[i][j] + dp[ni as usize][nj as usize]) % MOD;
                }
            }
        }
