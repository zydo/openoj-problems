function countByDigitSum(num1: string, num2: string, min_sum: number, max_sum: number): number {
    const MOD = 1000000007;

    const countRange = (s: string): number => {
        const m = s.length;
        const ms = max_sum;
        let dp: number[][] = [new Array(ms + 1).fill(0), new Array(ms + 1).fill(0)];
        ...
        for (let pos = m - 1; pos >= 0; pos--) {
            ...
            const ndp: number[][] = [new Array(ms + 1).fill(0), new Array(ms + 1).fill(0)];
    ...
    const decrement = (s: string): string => {
        const arr = s.split("");
        let i = arr.length - 1;
        while (i >= 0 && arr[i] === "0") {
            arr[i] = "9";
