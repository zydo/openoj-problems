# Solutions — Pairs With Mismatched Gaps

## Count matched pairs, subtract from all pairs

The condition `j - i != nums[j] - nums[i]` rearranges to
`nums[i] - i != nums[j] - j`, which changes the picture completely: each
index carries a single shifted value `nums[i] - i`, and a pair is _matched_
exactly when the two shifted values are equal. Since every pair is either
matched or mismatched, counting the matched ones — far easier, because
they group by shifted value — and subtracting from the total `n(n-1)/2`
yields the mismatched count without ever enumerating a pair.
...
matched pair, so adding the current bucket size to the answer before
inserting accumulates exactly one count per matched pair.
