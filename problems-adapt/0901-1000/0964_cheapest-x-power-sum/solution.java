class Solution {
    ...
    public int cheapestXPowerSum(int x, int target) {
        ...
        while (t > 0) {
            ...
            long r = t % x;
            t /= x;
            long n0 = Math.min(cost0 + r * p, cost1 + (r + 1) * p);
            long n1 = Math.min(cost0 + (x - r) * p, cost1 + (x - r - 1) * p);
            cost0 = n0;
            cost1 = n1;
            i++;
        }
        return (int) (Math.min(cost0, cost1 + i) - 1);
    }
}
