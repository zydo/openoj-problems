# Solutions — Town Celebrity

## Net trust score per person

The celebrity's two properties translate directly into degree counts on the
trust relation: the celebrity trusts nobody, so their out-degree is 0, and
everybody else trusts the celebrity, so their in-degree is `n - 1`. Instead
...
score (b was trusted by someone). A candidate celebrity, trusting nobody and
trusted by everybody else, ends the pass with score exactly `n - 1`; no
non-celebrity can reach that value, since anyone who trusts even one person
...
equals `n - 1`, so they are correctly returned as the celebrity even with no
