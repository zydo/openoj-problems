function countInversionsBeyondDouble(nums: number[]): number {
    const mergeCount = (arr: number[]): [number[], number] => {
        ...
        let count = c1 + c2;
        // Count cross pairs beyond double left[i] > 2 * right[j]. j never
        // restarts: the next left[i] is at least as large, so every right
        // element already passed also qualifies — the sweep is linear per
        // merge level.
        let j = 0;
