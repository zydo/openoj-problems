/**
 * @param {number[]} startCosts
 * @param {number[]} usageCosts
 * @param {number} budget
 * @return {number}
 */
var longestAffordableStretch = function (startCosts, usageCosts, budget) {
    const n = startCosts.length;
    const dq = []; // indices with decreasing startCosts
    ...
    for (let right = 0; right < n; right++) {
        ...
        while (dq.length > head && startCosts[dq[dq.length - 1]] <= startCosts[right]) {
            dq.pop();
        }
        dq.push(right);
        ...
        while (dq.length > head && startCosts[dq[head]] + (right - left + 1) * run > budget) {
            if (dq[head] === left) {
                head++;
            }
            run -= usageCosts[left];
