# Solutions — The Traveling Pillow
...
`(n, time) = (6, 8)` one full traversal plus a three-step backward
leg from person 6 lands on person 3, and for `(5, 12)` an exact
third traversal of length 4 leaves the pillow at person 5. Boundary
times behave too: an exact multiple of `n - 1` lands exactly on
person `n` when the quotient is odd and back on person 1 when it is
even.
