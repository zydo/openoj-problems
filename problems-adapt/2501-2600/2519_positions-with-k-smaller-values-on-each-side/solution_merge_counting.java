...
class Solution {
    ...
    // Merge-sorts order[lo:hi) by value while filling both tallies: placing
    // a left-half element credits every right-half element already placed
    // below it, and placing a right-half element reads its strictly smaller
    // left-half predecessors off the sorted run.
    private static void mergeSort(int[] nums, int[] order, int[] leftCounts, int[] rightCounts, int lo, int hi) {
        if (hi - lo < 2) {
            return;
        }
        int mid = (lo + hi) / 2;
        mergeSort(nums, order, leftCounts, rightCounts, lo, mid);
        mergeSort(nums, order, leftCounts, rightCounts, mid, hi);
    ...
    public int countSheltered(int[] nums, int k) {
        ...
        mergeSort(nums, order, leftCounts, rightCounts, 0, n);
        int big = 0;
        for (int i = 0; i < n; i++) if (leftCounts[i] >= k && rightCounts[i] >= k) big++;
        return big;
    }
}
