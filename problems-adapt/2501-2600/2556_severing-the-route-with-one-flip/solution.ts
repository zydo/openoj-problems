function severableWithOneFlip(grid: number[][]): boolean {
    ...
    const inf: number = count + 2;
    const arcsTo: number[] = [];
    const arcsCap: number[] = [];
    const graph: number[][] = new Array(2 * count);
    for (let v = 0; v < graph.length; ++v) graph[v] = [];
    const connect = (u: number, v: number, cap: number): void => {
