# Solutions — Split the Array Into Two Heavy Groups

## Subset-sum complement under an inclusion–exclusion subtraction

Directly counting heavy deals would mean verifying both group sums
across all `2^n` assignments, far past any budget. The hint flips the
view: a deal is _bad_ exactly when its first group's sum falls
...
simultaneously and the true count of heavy deals is zero.
