# Solutions — Longest Squaring Chain

## Sorted distinct values with a hash-map chain length

A squaring chain read in sorted order is forced: from a value `v` the only
legal next element is `v * v`. So a chain is a path in the "squaring"
