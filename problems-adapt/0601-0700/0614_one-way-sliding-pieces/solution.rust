impl Solution {
    pub fn can_reach(start: String, target: String) -> bool {
        ...
        let s: Vec<(usize, char)> = start.chars().enumerate().filter(|&(_, c)| c != '_').collect();
        let t: Vec<(usize, char)> = target.chars().enumerate().filter(|&(_, c)| c != '_').collect();
