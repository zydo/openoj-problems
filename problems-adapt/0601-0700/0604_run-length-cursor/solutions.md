...
still unspent, and a parse position into the compressed string. `nextChar`
spends one copy of the current letter; when the count reaches zero the
nextChar call re-parses one segment — a letter, then its run of digits — to
...
`hasMore` answers without consuming anything: the iterator has more to
...
letter. Once both conditions fail, `nextChar` returns the white space `" "`
