...
stores each pair and does nothing else, so every `prefixSum` walks all the
stored keys and adds the values of those that start with the prefix. The
trie pays at insert time: each `put` folds the key's change in value into
running totals along its path, and `prefixSum` only reads the number
...
map and nothing more. `put` is one assignment — `values[key] = val` — and
...
`prefixSum` is where the bill arrives. With no totals kept, the query
...
nodes, no per-character structures, no deltas to keep honest — and `put`
is constant time. The price is that every query re-reads the whole key
set, so `prefixSum` grows with the number of stored keys rather than with
the prefix alone. Fine at this statement's scale of at most 50 calls; the
trie is the design that scales past it.

**Complexity:** `O(1)` expected time per `put` and `O(N * p)` per
`prefixSum`, with `N` stored keys and a prefix of length `p`; `O(C)` space,
