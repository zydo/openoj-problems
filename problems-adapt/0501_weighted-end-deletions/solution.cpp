class Solution {
  public:
    int maxWeightedEndScore(vector<int> &nums, vector<int> &weights) {
        int m = weights.size();
        int n = nums.size();
        ...
        for (int i = m - 1; i >= 0; i--) {
            for (int l = 0; l <= i; l++) {
                ...
                long long takeLeft = prev[l + 1] + (long long)weights[i] * nums[l];
                long long takeRight = prev[l] + (long long)weights[i] * nums[n - 1 - r];
