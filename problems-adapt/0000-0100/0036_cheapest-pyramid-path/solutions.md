...
One recurrence, read in two directions: climb from the base folding each pair
of children into their parent, or descend from the apex extending partial
paths into the next row. Both collapse the pyramid into a single rolling row
of totals, and they differ only in which end of the recursion does the work
and where the answer finally sits.
