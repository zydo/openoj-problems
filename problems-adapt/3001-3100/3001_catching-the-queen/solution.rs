impl Solution {
    pub fn moves_to_catch(a: i32, b: i32, c: i32, d: i32, e: i32, f: i32) -> i32 {
        ...
        if c - e == d - f || c - e == f - d {
            let on_diag = a - c == b - d || a - c == d - b;
            let in_span = a - e == b - f || a - e == f - b;
            let between = (c < a && a < e) || (e < a && a < c);
            if !(on_diag && in_span && between) {
                return 1; // bishop's diagonal is clear down to the queen
            }
        }
        2 // no immediate attack; a staging move always exists
    }
}
