 ...
 * @return {number}
 */
var countCappedNumbers = function (start, finish, limit, s) {
    function pow10(e) {
        let r = 1;
    ...
    function countExactLen(p, cap, lim) {
        if (cap < pow10(p - 1)) return 0;
        if (cap >= pow10(p) - 1) return lim * Math.pow(lim + 1, p - 1);
        const capDigits = String(cap).split("").map(Number);

        const memo = new Array(2 * (p + 1)).fill(-1);
