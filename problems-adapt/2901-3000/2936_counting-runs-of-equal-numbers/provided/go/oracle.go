...
// Problem-provided oracle (VastArray), Go side. Compiled beside every
...
// NewVastArray builds the oracle from the case's construction values (the
// maximal blocks as one generic slice) and the query budget.
func NewVastArray(construction []any, budget int64) *VastArray {
	raw, ok := construction[0].([]any)
	if !ok {
		panic("VastArray blocks must be an array")
	}
	array := &VastArray{budget: budget}
	var offset int64
	var previous int64
	for position, entry := range raw {
		pair, ok := entry.([]any)
		if !ok || len(pair) != 2 {
			panic("VastArray blocks must be [value, count] pairs")
		}
		value, ok1 := pair[0].(int64)
		count, ok2 := pair[1].(int64)
		if !ok1 || !ok2 {
			panic("VastArray block entries must be integers")
		}
		if position > 0 && value == previous {
			panic("VastArray blocks must alternate values")
		}
...
func (array *VastArray) At(index int64) int {
	if array.budget <= 0 {
		panic("VastArray query budget exhausted")
	}
	array.budget--
...
func (array *VastArray) Size() int64 {
	return array.total
}
