# Solutions — Closest Word Gap III
...
Concretely, for the second example `word1 = word2 = "moon"`: `"moon"` lands at indices 1 and 4. At index 1 `index1` becomes 1 with `index2` still unset; at index 4 the old `index1` moves into `index2` and the gap `abs(4 - 1) = 3` is the answer. The first example, with `"moon"` versus `"star"`, behaves like the simpler problem: `"star"` at index 2 meets `"moon"` at index 1 for a gap of 1, which the later `"moon"` at index 4 cannot improve on.
