function canTransformAdditivePair(startA: number, startB: number, goalA: number, goalB: number): boolean {
    ...
    while (goalA >= startA && goalB >= startB) {
        if (goalA === startA && goalB === startB) return true;
        ...
        if (goalA === startA) return (goalB - startB) % startA === 0;
        if (goalB === startB) return (goalA - startA) % startB === 0;
