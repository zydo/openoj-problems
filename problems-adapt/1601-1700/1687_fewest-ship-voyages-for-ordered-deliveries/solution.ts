...
function fewestVoyages(boxes: number[][], portsCount: number, maxBoxes: number, maxWeight: number): number {
    const n = boxes.length;
    // running loaded weight reaches 10^5 * 10^5 = 10^10, still exact in a double
    const weightPrefix: number[] = [0];
    const runs: number[] = [0];
    for (let i = 0; i < n; i++) {
    ...
    const key: number[] = new Array(n); // key[l] = dp[l] - runs[l+1], the part of the cost l alone decides
    const window: number[] = []; // candidate l values with strictly increasing keys
    let head = 0;
