 ...
 * never editable in the editor: linksFrom(url) returns the outgoing links
...
public class LinkIndex {
    ...
    public LinkIndex(List<Object> urls, List<Object> edges, long budget) {
    ...
    public List<String> linksFrom(String url) {
        if (budget <= 0) {
            throw new IllegalStateException("LinkIndex query budget exhausted");
        }
        budget -= 1;
        fetched.add(url);
