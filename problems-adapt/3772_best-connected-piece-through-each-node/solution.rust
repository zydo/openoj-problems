impl Solution {
    pub fn best_piece_scores(n: i32, edges: Vec<Vec<i32>>, marked: Vec<i32>) -> Vec<i32> {
        let n = n as usize;
        let neg: i64 = -1i64 << 60;
        ...
        // +1 for marked, -1 for unmarked: a connected subgraph's score is its weight sum,
        // so the task is the max-weight connected subgraph through each node.
        let weight: Vec<i64> = marked.iter().map(|&g| if g != 0 { 1 } else { -1 }).collect();
