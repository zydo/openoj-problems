...
dp[i][j][c] is the length of the longest palindromic subsequence in substring [i..j] with cost at most c.

### Hint 3

dp[i][j][c] = max(dp[i + 1][j][c], dp[i][j - 1][c], dp[i + 1][j - 1][c - dist(s[i], s[j])] + 2), where dist(x, y) is the minimum cyclic distance between x and y.
