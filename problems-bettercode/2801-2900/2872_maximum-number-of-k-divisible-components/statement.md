...
- `0 <= values[i] <= 10⁹`
- `1 <= k <= 10⁹`
- The sum of values is divisible by `k`.
...
Root the tree at node 0 and compute subtree sums bottom-up.

### Hint 2

Whenever a finished subtree sum is divisible by k, cut the edge to its parent — this can never hurt, so it is always safe to do greedily.

### Hint 3

If it is not divisible by k, merge the sum into the parent and keep going; count one component per cut plus the final root component.
