...
An easy approach is to sort the array in descending order and find the largest h such that the first h papers each have at least h citations.

### Hint 2

Note the h-index can never exceed the number of papers n.

### Hint 3

A faster O(n) approach buckets citation counts with extra space, clamping counts above n into the bucket for n.
