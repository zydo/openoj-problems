...
that the last element of `subi` appears before the first element of
`sub{i+1}` for all `1 <= i <= k - 1`. The goal is to maximize their combined
strength.
...
Let dp[i][j][x == 0/1] be the maximum strength to select j disjoint subarrays from the suffix nums[i..(n - 1)], where x denotes whether the element nums[i] is selected.

### Hint 2

Initialize dp[n][0][0] = 0.
