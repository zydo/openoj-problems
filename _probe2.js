...
(async () => {
    const beforeSize = openojLimitClock.size;
    console.error("size before:", beforeSize);
    globalThis.__origSetTimeout = null;
    const p = openojLimitClock && (() => {
        const pr = new LimitCase(["async (n) => {\n  await new Promise(res => setTimeout(res, 100));\n  return n * n;\n}", [5], 150], 1);
        console.error("size after construct:", openojLimitClock.size);
        // call fn directly, see whether a tick appears
        const fp = pr.fn(5);
        console.error("size after fn():", openojLimitClock.size,
                      "ticks:", JSON.stringify(openojLimitClock.ticks.map(t=>t.time)));
        return fp;
    })();
})();
