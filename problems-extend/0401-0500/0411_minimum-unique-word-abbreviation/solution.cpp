class Solution {
  public:
    string minAbbreviation(string target, vector<string>& dictionary) {
        ...
        for (const string& word : dictionary) {
            if (word.size() != m_) continue;
            int mask = 0;
            for (int i = 0; i < m_; ++i)
                if (word[i] != target[i]) mask |= 1 << i;
            if (mask != 0) diffs.insert(mask);
        }
        ...
        sort(byWeight.begin(), byWeight.end(), [](int a, int b) {
        ...
        for (int mask : byWeight) {
            ...
            if (!redundant) minimal.push_back(mask);
        }
  ...
  private:
    ...
    void walk(int pos, int mask, int kept, int runs, bool open_run, const vector<int>& pending) {
        ...
        vector<int> still;
        for (int diff : pending) {
            if ((diff >> pos & 1) == 0) still.push_back(diff);
        }
