...
- `void dec(String key)` Decrements the count of the string `key` by 1. If the
...
- `String getMinKey()` Returns one of the keys with the minimum count. If no
  element exists, return an empty string `""`.

When several keys share the extreme count, this judge pins both queries to the
lexicographically smallest key among them — and to `""` when the structure is
empty — so the returned key is deterministic where the original allowed any of
the tied keys.
