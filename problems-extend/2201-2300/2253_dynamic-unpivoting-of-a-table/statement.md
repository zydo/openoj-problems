...
Each testcase supplies its own `dataset`: the setup script itself creates the `Products` table with that testcase's store columns and seeds its rows, so the store column names must be discovered from the seeded schema. The submission is several SQLite statements: a discovery `SELECT` returning exactly one row and one column — the comma-separated store references, each expression doing its own quoting — followed by the answer `SELECT` in which every `__COLUMNS__` placeholder is replaced by that discovered list before execution. Column names are judged: the answer must expose exactly `product_id`, `store`, and `price`, one row per product-store pair where the product is sold. Rows compare in `product_id` then `store` order, the canonical form of the "any order" latitude above.
