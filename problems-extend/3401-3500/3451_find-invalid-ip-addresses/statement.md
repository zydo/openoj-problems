...
Write your solution as a single `SELECT` query returning each invalid `ip`
and its `invalid_count` — the number of log rows carrying that address —
ordered by `invalid_count` descending, then `ip` descending. Each testcase
supplies its own `dataset`: the script seeds the `logs` table before your
query runs.
