...
Each testcase supplies its own `dataset`: the script seeds the `samples`
...
`sample_id`, `dna_sequence`, `species`, `has_start`, `has_stop`, `has_atat`,
and `has_ggg` — one row for every sample, each flag `1` when its pattern is
present and `0` when it is not, ordered by `sample_id` in ascending order.
