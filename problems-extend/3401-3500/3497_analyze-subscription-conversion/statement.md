...
`'cancelled'`). `activity_duration` is the number of minutes the user
spent on the platform that day. Each row represents a user's activity on
a specific date.
...
`user_id`, `trial_avg_duration`, and `paid_avg_duration`, in that order —
with one row for every user who converted from free trial to paid
subscription.
