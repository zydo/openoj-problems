...
Output: -1
Explanation: It is impossible to make nums a Zero Array even after all
the queries.
```
...
The array will become [0, 0, 0, 0, 0]. Therefore, the minimum value of
k is 4.
```
...
For each `nums[i]`, use DP to check whether the `queries[.][2]` values
(i.e. the val values) of the queries that affect it can form a
combination with a sum equal to `nums[i]`.
