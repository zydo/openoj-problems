...
You are given a string `sentence` that consists of words separated by spaces.
...
- If a word begins with a vowel (`'a'`, `'e'`, `'i'`, `'o'`, or `'u'`),
  append `"ma"` to the end of the word. For example, the word `"apple"`
  becomes `"applema"`.
...
Return the final sentence representing the conversion from `sentence` to
Goat Latin.
