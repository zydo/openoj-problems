...
You are given an array `boxes`, where `boxes[i] = [port_i, weight_i]`, and
three integers `portsCount`, `maxBoxes`, and `maxWeight`.

- `port_i` is the port where you need to deliver the `i`-th box and
  `weight_i` is the weight of the `i`-th box.
...
delivered at port 2 before the third).
```
...
Think of the most basic DP first — the `O(n²)` one over the prefixes —
and only then optimize it.

### Hint 2

Think of any range query data structure to optimize the minimum lookup.
