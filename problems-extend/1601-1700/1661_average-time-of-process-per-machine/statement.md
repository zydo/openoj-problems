...
`(machine_id, process_id, activity_type)` is the primary key (a
combination of columns with unique values) of this table. `machine_id`
is the ID of a machine. `process_id` is the ID of a process running on
the machine with the ID `machine_id`. `activity_type` is an ENUM
(category) of type `('start', 'end')`. `timestamp` is a float
...
timestamp for every `(machine_id, process_id)` pair, and it is
guaranteed that each `(machine_id, process_id)` pair has a `'start'`
and an `'end'` timestamp.

There is a factory website that has several machines each running the
same number of processes. Write a solution to find the average time
...
and `processing_time` — the average of the `'end'` minus `'start'`
timestamp differences over each machine's processes, rounded to 3
decimal places — for every machine, in any order.
