...
there is no possible sequence of jumps that lands the bug on position `x`,
return `-1`.
...
To handle the no double back jumps condition you can handle it by holding the
state of your previous jump.
