...
However, you are not allowed to reorder the integers within each array
`pieces[i]`.
...
- The integers in `pieces` are distinct (i.e., if we flatten `pieces` into
  a 1-D array, all the integers in this array are distinct).
...
Note that you can find the piece every position belongs to naively.
