class Solution {
  public:
    bool isSolvable(vector<string> &words, string result) {
        ...
        for (const string &word : words) {
            for (char ch : word) seen[ch - 'A'] = true;
            leads[word[0] - 'A'] = true;
        }
        for (char ch : result) seen[ch - 'A'] = true;
        ...
        used_.fill(false);
        int widest = 0;
        for (const string &row : words_) widest = max(widest, (int)row.size());
        // No leading zeros, so the sum is at least 10^(widest-1): the result
        // needs at least `widest` digits and at most widest + 1.
        if ((int)target_.size() < widest || (int)target_.size() > widest + 1) return false;
        return dfs(0, 0, 0);
    }
  ...
  private:
    ...
    bool dfs(int pos, int row, int carry) {
        if (pos == (int)target_.size()) return carry == 0;
        if (row == (int)words_.size()) {
            ...
            if (used_[digit] || (digit == 0 && leads_[ch - 'A'])) return false;
            value_[ch - 'A'] = digit;
            used_[digit] = true;
        ...
        if (value_[ch - 'A'] != -1) return dfs(pos, row + 1, carry + value_[ch - 'A']);
        for (int digit = 0; digit < 10; ++digit) {
            if (used_[digit] || (digit == 0 && leads_[ch - 'A'])) continue;
            value_[ch - 'A'] = digit;
            used_[digit] = true;
