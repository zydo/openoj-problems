...
public:
    bool isSubPath(ListNode* head, TreeNode* root) {
        ...
        while (!stack.empty()) {
            ...
            if (treeNode->left != nullptr) stack.push_back(treeNode->left);
            if (treeNode->right != nullptr) stack.push_back(treeNode->right);
        }
        return false;
    }
...
private:
    bool matchFrom(TreeNode* start, const std::vector<int>& values) {
        if (values.empty() || start->val != values[0]) return false;
        std::vector<std::pair<TreeNode*, int>> stack{{start, 0}};
        while (!stack.empty()) {
            auto [node, index] = stack.back();
            stack.pop_back();
            if (index + 1 == static_cast<int>(values.size())) return true;
