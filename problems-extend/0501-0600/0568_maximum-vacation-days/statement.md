...
n cities to collect algorithm problems. But all work and no play makes Jack a
...
- The cities are connected by flights. The flights are represented as an
...
- For each city, you can only have restricted vacation days in different
  weeks, given an `n x k` matrix called `days` representing this
  relationship. For the value of `days[i][j]`, it represents the maximum
