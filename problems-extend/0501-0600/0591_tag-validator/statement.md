...
2. A closed tag (not necessarily valid) has exactly the following format :
   ...
   are valid.
3. A valid TAG_NAME only contain upper-case letters, and has length in range
...
4. A valid TAG_CONTENT may contain other valid closed tags, cdata and any
   ...
   is invalid.
5. A start tag is unmatched if no end tag exists with the same TAG_NAME, and
...
8. CDATA_CONTENT may contain any characters. The function of cdata is to
   forbid the validator to parse CDATA_CONTENT, so even it has some characters
   that can be parsed as tag (no matter valid or invalid), you should treat it
