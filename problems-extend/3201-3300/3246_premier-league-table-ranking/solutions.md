...
per-row expression — `3 * wins + draws`, with losses worth nothing — and no
join or aggregation is needed to produce them. The rank is competition
ranking: tied totals share a position and the next distinct total skips past
every tied team, which is exactly what `RANK() OVER (ORDER BY 3 * wins +
draws DESC)` computes. In the example, Manchester City's and Liverpool's 20
points both receive position 1, and Chelsea's 18 points receives position 3
— one past the two teams already placed.
