...
character in a non-empty string `s`. For example, if `s = "dcce"` then
`f(s) = 2` because the lexicographically smallest character is `'c'`, which
...
For each string from `words` calculate the leading count and store it in an
array, then sort the integer array.

### Hint 2

For each string from `queries` calculate the leading count "p" and in base
of the sorted array calculated on the step 1 do a binary search to count the
number of items greater than "p".
