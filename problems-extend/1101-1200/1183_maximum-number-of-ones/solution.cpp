class Solution {
public:
    int maximumNumberOfOnes(int width, int height, int sideLength,
                            int maxOnes) {
        ...
        for (int r = 0; r < sideLength; r++) {
            for (int c = 0; c < sideLength; c++) {
                long long rows = height / sideLength
                        + (height % sideLength > r ? 1 : 0);
                long long cols = width / sideLength
                        + (width % sideLength > c ? 1 : 0);
