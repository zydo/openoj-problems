...
number of users `x` such that `x` visited `"home"` then visited `"away"`
and visited `"love"` after that.
...
Let's find for every user separately the websites they visited.

### Hint 2

Consider all possible 3-sequences, find the number of distinct users who
visited each of them.
...
Store for every user all the 3-sequences they visited.
