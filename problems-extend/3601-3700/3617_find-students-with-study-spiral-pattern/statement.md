...
`session_id` is the unique identifier for this table. Each row represents a
study session by a student for a specific subject.
...
Each testcase's `dataset` seeds the `students` and `study_sessions` tables:
its script inserts the testcase's rows (whichever are present) before your
query runs. Write your solution as a single `SELECT` query returning
`student_id`, `student_name`, `major`, `cycle_length`, and
`total_study_hours` in that order.
