...
Explanation:
    The first and last elements are always valid.
    No other elements are strictly greater than all elements to their left or
    to their right.
...
Input: nums = [1]
Output: [1]
Explanation: Since there is only one element, it is always valid. Thus, the
answer is [1].
```
...
Precompute the maximum left and right for each index.

### Hint 2

The first and last are always valid.
