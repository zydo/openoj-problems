...
For example, `s1 = "Hello Jane"` and `s2 = "Hello my name is Jane"` can be
made equal by inserting `"my name is"` between `"Hello"` and `"Jane"` in
`s1`. `s1 = "Frog cool"` and `s2 = "Frogs are cool"` are not similar, since
...
One way to look at it is to find one sentence as a concatenation of a prefix
and suffix from the other sentence.
