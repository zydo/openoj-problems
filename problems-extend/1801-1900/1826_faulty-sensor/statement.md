...
sensors collecting data simultaneously. You are given two arrays `sensor1`
and `sensor2`, where `sensor1[i]` and `sensor2[i]` are the ith data points
collected by the two sensors.
...
data points to the right of the dropped data are shifted one place to the
left, and the last data point is replaced with some random value. It is
...
sensor or if it is impossible to determine the defective sensor, return -1.
...
After this common prefix, there should be one array similar to the other
but shifted by one.
