...
`employee_id` is the column with unique values for this table. Each row
of this table indicates the employee ID, employee name, and salary.
...
- A team's ID is assigned based on the rank of the team's salary relative
  ...
  `team_id = 1`. Note that the salaries for employees not on a team are
...
Juan's salary of 6100 is not included in the ranking because they are not
on a team.
```
