...
segment of 1's is strictly longer than the longest contiguous segment of
0's in `s`, or return `false` otherwise.
...
0's is considered to have a length 0. The same applies if there is no
1's.
...
Is there a way to iterate through the string to keep track of the current
