...
`encoded[i] = [vali, freqi]` describes the ith segment of repeated
numbers in `nums` where `vali` is the value that is repeated `freqi`
times.

- For example, `nums = [1,1,1,2,2,2,2,2]` is represented by the
  run-length encoded array `encoded = [[1,3],[2,5]]`. Another way to read
...
describes the ith segment of `nums1`, and each `encoded2[j] = [valj,
...
What is the maximum number of segments if we took the minimum number of
elements left on both the current segments every time?
