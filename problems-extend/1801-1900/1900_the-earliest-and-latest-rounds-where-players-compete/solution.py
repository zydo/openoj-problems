...
class Solution:
    def earliestAndLatest(self, n: int, firstPlayer: int,
        ...
        def dp(i: int, j: int, m: int):
            ...
            half = (m + 1) // 2
            free = [(k, m + 1 - k) for k in range(1, half + 1)
                    if k < m + 1 - k and i not in (k, m + 1 - k)
                    and j not in (k, m + 1 - k)]
            lo, hi = n, 0
            for mask in range(1 << len(free)):
                ...
                survivors.sort()
                sub_lo, sub_hi = dp(survivors.index(i) + 1,
                                    survivors.index(j) + 1, len(survivors))
                lo = min(lo, sub_lo)
                hi = max(hi, sub_hi)
            return (lo + 1, hi + 1)
