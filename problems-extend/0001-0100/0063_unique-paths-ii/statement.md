...
Assume `dp[i][j]` is the number of unique paths to reach `(i, j)`. Then `dp[i][j] = dp[i][j - 1] + dp[i - 1][j]`. Be careful when you encounter an obstacle: set its value in dp to 0.
