...
integer `k`. Your task is to convert the string into an integer by a
special process, and then transform it by summing its digits repeatedly `k`
times. More specifically, perform the following steps:

1. Convert `s` into an integer by replacing each letter with its position
   in the alphabet (i.e. replace `'a'` with 1, `'b'` with 2, ..., `'z'`
   with 26).
...
First, let's note that after the first transform the value will be at most
100 * 10 which is not much

### Hint 2

After The first transform, we can just do the rest of the transforms by
brute force
