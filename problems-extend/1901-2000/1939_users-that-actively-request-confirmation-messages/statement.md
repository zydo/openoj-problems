...
The qualifying check involves only the confirmation request times, never the
signup time or the action. Write your solution as a single `SELECT` query
returning one column — `user_id` — with one row per qualifying user, in any
order.
