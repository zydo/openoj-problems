class Solution {
    ...
    public boolean[] canMakePalindromeQueries(String s, int[][] queries) {
        ...
        for (int x = 0; x < half; ++x)
            mismatch[x + 1] = mismatch[x] + (s.charAt(x) != s.charAt(n - 1 - x) ? 1 : 0);

        boolean[] answer = new boolean[queries.length];
        int[] fixedLo = new int[4], fixedHi = new int[4]; // fully fixed left-half ranges
        int[] flLo = new int[2], flHi = new int[2];       // fixed characters facing [c, d]
        int[] frLo = new int[2], frHi = new int[2];       // fixed characters facing [a, b]
        for (int index = 0; index < queries.length; ++index) {
            int a = queries[index][0], b = queries[index][1];
            int c = queries[index][2], d = queries[index][3];
