...
[1,2], [2,3], [3,4], [1,2,3], [2,3,4], and [1,2,3,4], because on removing
any one of these subarrays nums becomes strictly increasing. Note that
...
Calculate the smallest y such that nums[y..nums.length-1] is strictly
increasing.

### Hint 3

For each i in [0, x], select the smallest j in [y, nums.length - 1]. Then
we can keep the prefix with any suffix of [j, nums.length - 1] (including
the empty one).
...
Note that we cannot delete an empty array, but we can delete the whole
array.
