...
For each index `i >= indexDifference`, keep the indices `j₁` and `j₂` in
the range `[0, i - indexDifference]` such that `nums[j₁]` and `nums[j₂]`
are the minimum and maximum values in the index range.

### Hint 2

Check if `abs(nums[i] - nums[j₁]) >= valueDifference` or
`abs(nums[i] - nums[j₂]) >= valueDifference`.

### Hint 3

`j₁` and `j₂` can be updated dynamically, or they can be pre-computed
since they are just prefix minimum and maximum.
