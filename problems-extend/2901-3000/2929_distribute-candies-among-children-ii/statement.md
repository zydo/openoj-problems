...
The 3rd child will hence get n - i - j candies and we should have
0 ≤ n - i - j ≤ limit.

### Hint 4

After some transformations, for each i, we have
max(0, n - i - limit) ≤ j ≤ min(limit, n - i), each j corresponding to a
solution. So the number of solutions for some i is
max(min(limit, n - i) - max(0, n - i - limit) + 1, 0). Sum the expression
for every i in [0, min(n, limit)].
