...
table contains user_id, spend, and transaction_date.
...
+---------+-------------------------+------------------------+
Explanation
- For user_id 1, their third transaction occurred on 2023-11-18 at 13:49:42
  with an amount of $65.56, surpassing the expenditures of the previous two
  transactions which were $7.44 on 2023-11-02 at 12:15:23 and $49.78 on
  2023-11-12 at 00:13:46. Thus, this third transaction will be included in
  the output table.
...
A user's transactions are ordered by `transaction_date` alone; because
`(user_id, transaction_date)` is unique, that order is total and the third
transaction is well defined. It qualifies only when its spend is strictly
greater than the spend of both preceding transactions — a tie with either
one disqualifies it — and users with fewer than three transactions never
appear, while later transactions play no role at all. Write your solution
as a single `SELECT` query returning three columns — `user_id`,
`third_transaction_spend`, and `third_transaction_date`, in that order —
with the rows ordered by `user_id` ascending.
