...
- `leaveTime_i` is the time at which the employee finished the ith task. All
  the values `leaveTime_i` are unique.
...
For employees that have the same longest task time, we only need the
