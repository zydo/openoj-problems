...
Each testcase supplies its own `dataset`, whose statements insert all rows into `Terms` before the query runs. The result format is in the following examples.
...
Write your solution as a single `SELECT` query returning the column `equation`.
