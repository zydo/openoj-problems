...
Output: [0,1,2,3,4]
Explanation: For all indices i in nums, there exists some index j such
that |i - j| <= k and nums[j] == key, so every index is a k-distant
index.
...
For every occurrence of `key` in `nums`, find all indices within distance
`k` from it.
