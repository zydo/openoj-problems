...
- `1 <= size <= 10⁵`
- `0 <= idx <= size - 1`
- At most `10⁵` calls will be made in total to `fix`, `unfix`, `flip`,
...
In order to determine the value of a bit, consider how you can efficiently
count the number of flips made on the bit since its latest update.
