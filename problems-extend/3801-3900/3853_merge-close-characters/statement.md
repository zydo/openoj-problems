...
happen one at a time, and after each merge, the string updates until no
...
Now the remaining 'a' characters at indices i = 0 and i = 3 are not close
as k < 3, so no further merges occur.
```
