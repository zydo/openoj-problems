...
represented by a 2D array `edges` of length `n - 1`, where
...
- `"update ui c"`: Change the character at node `ui` to `c`. Formally, update
  `s[ui] = c`.
...
of type `"query ui vi"` can be rearranged into a palindrome, and `false`
otherwise.
...
Explanation:
    ...
    "query 0 2": Path characters are "abc", which cannot be rearranged to form
    a palindrome. Thus, answer[1] = false.
...
Explanation:
    "query 1 2": Path 1 → 0 → 2 gives "bac", which cannot be rearranged to form
    a palindrome. Thus, answer[0] = false.
    "update 0 b": Update node 0 to 'b', now s = "bbca".
    "query 2 3": Path 2 → 0 → 3 gives "cba", which cannot be rearranged to form
    a palindrome. Thus, answer[1] = false.
...
Represent characters as a 26-bit mask and maintain segment data with XOR. A
path can form a palindrome if the resulting mask has at most one bit set.
