...
You are given two integer arrays costs and capacity, both of length n, where
costs[i] represents the purchase cost of the ith machine and capacity[i]
represents its performance capacity.
...
Output: 8
Explanation: Choose two machines with costs[0] = 4 and costs[3] = 3.
...
Output: 6
Explanation: Choose one machine with costs[3] = 4.
...
Output: 9
Explanation: Choose two machines with costs[1] = 2 and costs[2] = 2.
...
Build a prefix array where prefMax[i] stores the maximum capacity among
machines with index <= i.

### Hint 3

For selecting one machine, take the maximum capacity among all machines
with cost < budget.

### Hint 4

For selecting two machines, fix machine i and binary search the largest
index j with costs[j] < budget - costs[i].
