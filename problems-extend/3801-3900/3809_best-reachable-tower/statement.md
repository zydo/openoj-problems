...
You are also given an integer array `center = [cx, cy]` representing your
location, and an integer `radius`.
...
Explanation:
    ...
    [1, 3] and [2, 2] have the same quality, so the lexicographically
    smaller coordinate is [1, 3].
```
...
Iterate through all towers and compute the Manhattan distance
`abs(xi - cx) + abs(yi - cy)`.

### Hint 2

Consider only towers with distance less than or equal to `radius`.

### Hint 3

Track the reachable tower with the highest `qi`; break ties using
lexicographical order on `[xi, yi]`.
