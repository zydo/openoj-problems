...
Each row in this table contains the degree of the weather of a city on
a certain day. All the degrees are recorded in the year 2022.
...
Each testcase supplies its own `dataset`: the DDL seeds the `Weather`
table with that testcase's rows. Write your solution as a single
`SELECT` query returning three columns — `city_id`, `day`, and `degree`
— one row per city, and no rows at all when the table is empty. The
