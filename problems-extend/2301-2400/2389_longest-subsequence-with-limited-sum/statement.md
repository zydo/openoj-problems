...
size of a subsequence that you can take from `nums` such that the sum of
its elements is less than or equal to `queries[i]`.
...
When solving a query, which elements of `nums` should you choose to make
the subsequence as long as possible?

### Hint 3

Choose the smallest elements in `nums` that add up to a sum less than the
query.
