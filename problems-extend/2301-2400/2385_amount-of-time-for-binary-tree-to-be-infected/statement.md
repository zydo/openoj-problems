...
integer `start`. At minute 0, an infection starts from the node with value
`start`.
...
Use BFS starting at the start node to find the distance between each node
and the start node. The answer is the maximum distance.
