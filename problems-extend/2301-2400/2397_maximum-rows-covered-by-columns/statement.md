...
You are given an `m x n` binary matrix `matrix` and an integer
`numSelect`.
...
More formally, let us consider `selected = {c1, c2, ...., cnumSelect}` as
the set of columns selected by you. A row i is covered by `selected` if:
...
Output: 2
Explanation:
Selecting the only column will result in both rows being covered since
the entire matrix is selected.
...
For each valid set, check how many rows are covered, and return the
maximum.
