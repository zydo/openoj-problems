...
var beautifulPair = function (nums1, nums2) {
  ...
  const solve = (left, right) => {
    if (right - left <= 3) {
      ...
      for (let a = left; a < right; a++) {
        for (let b = a + 1; b < right; b++) {
          delta = Math.min(
            delta,
            Math.abs(nums1[idx[a]] - nums1[idx[b]]) +
              Math.abs(nums2[idx[a]] - nums2[idx[b]]),
          );
        }
      }
    ...
    for (let pos = left; pos < length; pos++) {
      for (
        let follow = pos + 1;
        follow < length && nums2[tmp[follow]] - nums2[tmp[pos]] < delta;
        follow++
      ) {
        delta = Math.min(
          delta,
          Math.abs(nums1[tmp[pos]] - nums1[tmp[follow]]) +
            Math.abs(nums2[tmp[pos]] - nums2[tmp[follow]]),
        );
      }
    }
    return delta;
    ...
    for (let gx = cx - 1; gx <= cx + 1; gx++) {
      for (let gy = cy - 1; gy <= cy + 1; gy++) {
        ...
        for (const j of bucket) {
          const gap =
            Math.abs(nums1[i] - nums1[j]) + Math.abs(nums2[i] - nums2[j]);
          if (gap === dist && j < bestJ) {
            bestJ = j;
            bestK = i;
          }
