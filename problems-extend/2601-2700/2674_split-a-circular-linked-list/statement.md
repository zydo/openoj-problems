...
split it into 2 circular linked lists so that the first one contains the first
half of the nodes in `list` (exactly `ceil(list.length / 2)` nodes) in the same
order they appeared in `list`, and the second one contains the rest of the
nodes in `list` in the same order they appeared in `list`.
