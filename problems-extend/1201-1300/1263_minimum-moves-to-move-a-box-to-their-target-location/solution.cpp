    ...
    int minPushBox(vector<vector<string>>& grid) {
        ...
        for (int r = 0; r < m; ++r) {
            for (int c = 0; c < n; ++c) {
                ...
                else if (grid[r][c] == "S") { playR = r; playC = c; }
                else if (grid[r][c] == "T") { targR = r; targC = c; }
            }
        }
        ...
        auto reachable = [&](int br, int bc, int sr, int sc,
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (!freeCell(standR, standC) || !freeCell(destR, destC)) continue;
