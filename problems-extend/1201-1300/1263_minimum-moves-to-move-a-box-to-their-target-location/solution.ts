...
function minPushBox(grid: string[][]): number {
    ...
    const reachable = (br: number, bc: number, sr: number, sc: number,
                       seen: boolean[][]): void => {
    ...
    const key = (br: number, bc: number, side: number): number =>
        (br * n + bc) * 4 + side;
    const queue: [number, number, number, number][] = [];
    const around: boolean[][] =
        Array.from({length: m}, () => new Array(n).fill(false));
    reachable(box[0], box[1], player[0], player[1], around);
    for (let i = 0; i < 4; ++i) {
        const standR = box[0] + DELTAS[i][0], standC = box[1] + DELTAS[i][1];
        const destR = box[0] - DELTAS[i][0], destC = box[1] - DELTAS[i][1];
    ...
    while (queue.length > 0) {
        ...
        for (let i = 0; i < 4; ++i) {
            const standR = br + DELTAS[i][0], standC = bc + DELTAS[i][1];
            const destR = br - DELTAS[i][0], destC = bc - DELTAS[i][1];
            if (!free(standR, standC) || !free(destR, destC)) continue;
