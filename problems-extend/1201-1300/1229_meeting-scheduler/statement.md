...
Assume that in the solution, the selected slot from slotsA is bigger than the
respectively selected slot from slotsB.

### Hint 2

Use two pointers in order to try all the possible intersections, and check the
length.

### Hint 3

Do the same as in step 1 but now assume that the selected slot from slotsB is
bigger, and return the minimum of the two options.
