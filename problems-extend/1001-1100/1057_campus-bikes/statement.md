...
You are given an array `workers` of length n where `workers[i] = [xi,
yi]` is the position of the ith worker. You are also given an array
`bikes` of length m where `bikes[j] = [xj, yj]` is the position of the
...
(0-indexed) of the bike that the ith worker is assigned to.
...
Sort the elements by distance. In case of a tie, sort them by the index
of the worker. After that, if there are still ties, sort them by the
index of the bike. Can you do this in less than O(nlogn) time, where n
is the total number of pairs between workers and bikes?

### Hint 2

Loop the sorted elements and match each pair of worker and bike if the
given worker and bike where not used.
