...
Each testcase supplies its own `dataset`, whose statements insert all rows into `Transactions` before the query runs. The result format is in the following example.
...
Write your solution as a single `SELECT` query returning `account_id`, `day`, and `balance` in the required order.
