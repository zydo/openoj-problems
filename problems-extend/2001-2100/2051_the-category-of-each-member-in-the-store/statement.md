...
Each testcase supplies its own `dataset`, which seeds all three tables. The result format is in the following example.
...
Write your solution as a single `SELECT` query returning `member_id`, `name`, and `category`, one row for every member.
