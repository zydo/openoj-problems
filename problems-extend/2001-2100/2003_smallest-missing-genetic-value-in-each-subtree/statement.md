...
`n - 1`. You are given a 0-indexed integer array `parents`, where `parents[i]`
is the parent of node `i`. Since node `0` is the root, `parents[0] == -1`.

There are `10⁵` genetic values, each represented by an integer in the inclusive
range `[1, 10⁵]`. You are given a 0-indexed integer array `nums`, where
`nums[i]` is a distinct genetic value for node `i`.
