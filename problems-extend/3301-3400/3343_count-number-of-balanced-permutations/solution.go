func countBalancedPermutations(num string) int {
	// A balanced permutation is decided by how many copies of each
	// digit land on even indices: a_d of the cnt[d] copies, with
	// sum(a_d) = ceil(n/2) and sum(d * a_d) = total / 2 (the
	// odd-index sum is then implied by the total), each choice
	// contributing C(cnt[d], a_d). A bottom-up DP over digits with
	// states (even slots used, even-index sum) accumulates those
	// binomial products. Arranging the two chosen multisets over the
	// even and odd slots multiplies by even_count! * odd_count! /
	// cnt[d]!, folded in via one modular inverse at the end. All
	// arithmetic is modulo 1e9 + 7 in int64s (products < 2^63),
	// iterative — no recursion.
	const MOD int64 = 1000000007
	n := len(num)
	cnt := make([]int, 10)
	for _, ch := range num {
		cnt[ch-'0']++
	}
	total := 0
	for d := 0; d < 10; d++ {
		total += d * cnt[d]
	}
	if total%2 != 0 {
		return 0
	}
	evenCount := (n + 1) / 2
	half := total / 2
	binom := make([][]int64, n+1)
	for i := range binom {
		binom[i] = make([]int64, n+1)
		binom[i][0] = 1
		for j := 1; j <= i; j++ {
			binom[i][j] = (binom[i-1][j-1] + binom[i-1][j]) % MOD
		}
	}
	dp := make([][]int64, evenCount+1)
	for i := range dp {
		dp[i] = make([]int64, half+1)
	}
	dp[0][0] = 1
	for d := 0; d < 10; d++ {
		c := cnt[d]
		if c == 0 {
			continue
		}
		ndp := make([][]int64, evenCount+1)
		for i := range ndp {
			ndp[i] = make([]int64, half+1)
		}
		for k := 0; k <= evenCount; k++ {
			for s := 0; s <= half; s++ {
				v := dp[k][s]
				if v == 0 {
					continue
				}
				for j := 0; j <= c && k+j <= evenCount && s+d*j <= half; j++ {
					ndp[k+j][s+d*j] = (ndp[k+j][s+d*j] + v*binom[c][j]) % MOD
				}
			}
		}
		dp = ndp
	}
	fact := make([]int64, n+1)
	fact[0] = 1
	for i := 1; i <= n; i++ {
		fact[i] = fact[i-1] * int64(i) % MOD
	}
	slotWays := fact[evenCount] * fact[n-evenCount] % MOD
	denom := int64(1)
	for _, c := range cnt {
		denom = denom * fact[c] % MOD
	}
	inv, base, exp := int64(1), denom, MOD-2
	for exp > 0 {
		if exp&1 == 1 {
			inv = inv * base % MOD
		}
		base = base * base % MOD
		exp >>= 1
	}
	return int(dp[evenCount][half] * slotWays % MOD * inv % MOD)
}
