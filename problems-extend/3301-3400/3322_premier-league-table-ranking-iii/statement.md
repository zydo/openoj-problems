...
(`season_id`, `team_id`) is the unique key for this table. This table
contains season id, team id, team name, matches played, wins, draws,
losses, goals scored (`goals_for`), and goals conceded
...
by position in ascending order, and finally by `team_name` in ascending
order.

Each testcase supplies its own `dataset`: the DDL seeds the `SeasonStats`
table with that testcase's rows. The query result format is in the
following example.
...
`season_id`, `team_id`, `team_name`, `points`, `goal_difference`, and
`position`, in that order — with one row per team per season. Position
is the team's 1-based rank inside its own season under the total order:
points descending, then goal difference descending, then team name
ascending. Rows come back ordered by `season_id` ascending, then
`position` ascending, then `team_name` ascending.
