...
in `source`. For example, if you remove 'c' from "acb", the character at
index 2 would still be 'b'.
...
At each index in `targetIndices`, make the choice to remove or not remove
the character.
