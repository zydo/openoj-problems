...
this table. Each row of this table contains the state name and the city
...
`state`, `cities`, and `matching_letter_count`, in that order. The
`cities` column holds every city of the state joined into one string,
alphabetically ordered — comparing city names case-insensitively with
spaces ignored, so `Newark` precedes `New York City` — and separated by
a comma and a space (`', '`). The `matching_letter_count` column counts
that state's cities whose first letter equals the first letter of the
state name. States qualify only with at least 3 cities and at least one
such match; rows come back ordered by `matching_letter_count`
descending, then `state` ascending.
