...
- `Excel(int height, char width)` Initializes the object with the height and the
  width of the sheet. The sheet is an integer matrix `mat` of size
  `height x width` with the row index in the range `[1, height]` and the column
  index in the range `['A', width]`. All the values should be zero initially.
...
- `int sum(int row, char column, List<String> numbers)` Sets the value at
  `mat[row][column]` to be the sum of cells represented by `numbers` and returns
...
// set mat[3]["C"] to be the sum of value at mat[1]["A"] and the values sum of
// the rectangle range whose top-left cell is mat[1]["A"] and bottom-right cell
// is mat[2]["B"].
//   A B C
// 1 2 0 0
// 2 0 0 0
