...
Where 0 <= board[r][c] <= 9 and each element of pattern is either a digit or
...
letters in pattern with some digits (each distinct letter with a unique
digit) in such a way that the resulting matrix becomes identical to the
...
is more than one such submatrix, return the coordinates of the submatrix with
the lowest row index, and in case there is still a tie, return the
...
Use brute force and check all the possible submatrices.
