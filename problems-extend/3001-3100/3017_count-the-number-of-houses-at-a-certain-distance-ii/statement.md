...
There is a street connecting the house numbered `i` with the house numbered
`i + 1` for all `1 <= i <= n - 1`. An additional street connects the house
...
the total number of pairs of houses such that the minimum streets required to
...
The shortest distance between house i and house j (j ) is along one of these paths:
