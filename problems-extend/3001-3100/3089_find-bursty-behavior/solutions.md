...
the seven-day span. Any window can be slid left until its first day is a
post of that user without losing posts, so anchoring at posts visits a
window at least as full as the true maximum, and `best` takes `MAX` over
each user's anchored counts. Posts sharing a date all anchor the same
...
...) is rarely integral. With `n` February posts, `u` distinct users and
`w` posts for the busiest user, the self-join compares `O(w² / u)`-ish
pairs per user and the aggregation is linear beyond it.
