...
(`product_id`, `store`, `price`). If a product is not available in a
store, do not include a row with that `product_id` and `store`
combination in the result table.

Each testcase's `dataset` seeds the table: its script inserts the
testcase's `Products` rows before your query runs. Return the result
