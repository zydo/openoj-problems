...
Let's ignore the non-empty subsequence constraint. We can concatenate the two
strings and find the largest palindromic subsequence with dynamic programming.

### Hint 2

Iterate through every pair of characters `word1[i]` and `word2[j]`, and see if
some palindrome begins with `word1[i]` and ends with `word2[j]`. This ensures
that the subsequences are non-empty.
