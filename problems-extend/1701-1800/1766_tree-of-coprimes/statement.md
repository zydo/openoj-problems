...
`edges[j] = [uj, vj]` represents an edge between nodes `uj` and `vj` in
the tree.
...
Note that for a node, it's not optimal to consider two nodes with the same
value.

### Hint 2

Note that the values are small enough for you to iterate over them instead
of iterating over the parent nodes.
