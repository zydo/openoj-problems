...
`[2,3,5]` is a subsequence of `[1,2,3,4,5]` while `[1,5,3]` is not).
...
from nums2. Their dot product is (2*3 + (-2)*(-6)) = 18.
```
...
Use dynamic programming, define `DP[i][j]` as the maximum dot product of
two subsequences starting in the position `i` of `nums1` and position `j`
of `nums2`.
