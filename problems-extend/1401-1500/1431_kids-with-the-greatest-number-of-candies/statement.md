...
ith kid has, and an integer `extraCandies`, denoting the number of extra
candies that you have.

Return a boolean array `result` of length `n`, where `result[i]` is
`true` if, after giving the ith kid all the `extraCandies`, they will
...
greatest number of candies, even if a different kid is given the extra
candy.
```
...
For each kid check if `candies[i] + extraCandies >= maximum in
Candies[i]`.
