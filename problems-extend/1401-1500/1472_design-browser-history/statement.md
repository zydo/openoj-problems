...
- `string back(int steps)` Move `steps` back in history. If you can only
  ...
  `x` steps. Return the current url after moving back in history at most
  `steps`.
...
Use two stacks: one for back history, and one for forward history. You can
simulate the functions by popping an element from one stack and pushing it
into the other.
