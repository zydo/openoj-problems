...
and is not lonely. Nodes 2 and 3 have the same parent and are not lonely.
```
...
Output: [6,2]
Explanation: Light blue nodes are lonely nodes. Please remember that
order doesn't matter, [2,6] is also an acceptable answer.
```
...
Do a simple tree traversal, try to check if the current node is lonely or
not.
