...
whole string "52233" has two adjacent same digit pairs 22 and 33, but at
most one is allowed.
```
...
substring "111" has two adjacent same digit pairs, but at most one is
allowed.
```
...
Since n is small, we can just check every substring, and if the substring is
semi-repetitive, maximize the answer with its length.
