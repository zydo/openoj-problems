  ...
  public:
    int maxNonDecreasingLength(vector<int> &nums1, vector<int> &nums2) {
        ...
        for (int i = 1; i < (int)nums1.size(); ++i) {
            int next1 = 1;
            if (nums1[i] >= nums1[i - 1]) next1 = max(next1, run1 + 1);
            if (nums1[i] >= nums2[i - 1]) next1 = max(next1, run2 + 1);
            int next2 = 1;
            if (nums2[i] >= nums1[i - 1]) next2 = max(next2, run1 + 1);
            if (nums2[i] >= nums2[i - 1]) next2 = max(next2, run2 + 1);
            run1 = next1;
            run2 = next2;
