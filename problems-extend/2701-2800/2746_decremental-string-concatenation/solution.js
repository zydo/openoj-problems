var minimizeConcatenatedLength = function (words) {
  ...
  const INF = Infinity;
  const dp = Array.from({ length: 26 }, () => Array(26).fill(INF));
  dp[words[0].charCodeAt(0) - 97][words[0].charCodeAt(words[0].length - 1) - 97] =
    words[0].length;
  for (let i = 1; i < words.length; i++) {
    ...
    const ndp = Array.from({ length: 26 }, () => Array(26).fill(INF));
    for (let f = 0; f < 26; f++) {
      for (let l = 0; l < 26; l++) {
