...
For x = 2, the possible operations are:
    ...
    Remove the empty prefix and the suffix [4, 8, 16, 32]. nums becomes
    [1, 2].
    Remove the prefix [1] and the suffix [4, 8, 16, 32]. nums becomes [2].
...
Define dp[i][r] as the count of subarrays ending at index i whose product
modulo k equals r.

### Hint 3

Compute dp[i][r] for each index i in nums and sum over all indices to get
the final counts for each remainder.
