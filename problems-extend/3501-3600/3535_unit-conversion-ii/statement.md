...
as `pq⁻¹` modulo `10⁹ + 7`, where `q⁻¹` represents the multiplicative
inverse of `q` modulo `10⁹ + 7`.
...
- In the third query, we can convert unit 6 into 1 unit of type 5 using the
  inverse of conversions[5], the inverse of conversions[2], conversions[1],
  then conversions[4].
...
- In the fifth query, we can convert unit 6 into 1/12 units of type 1 using
  the inverse of conversions[5], the inverse of conversions[2], then
...
Using a dfs, compute an array fromRoot such that fromRoot[a] is the number
of units of type a equivalent to a single unit of type 0.

### Hint 2

answer[i] = fromRoot[unitAi]⁻¹ * fromRoot[unitBi]
