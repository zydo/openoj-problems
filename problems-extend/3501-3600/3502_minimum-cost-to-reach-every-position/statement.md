...
a specific amount to swap places. The cost to swap with person `i` is given
by `cost[i]`.
...
Output: [1,1,1,1,1]
Explanation: We can swap with person 0 for a cost of 1, then we will be able
to reach any position i for free.
```
...
Note that once you swap to a position with a lower cost, you can reach any
later position for free.
