...
any two adjacent signs at indices `i` and `i + 1` (with `i > 0` and
`i + 1 < n`) and:

- Update the sign at index `i + 1` so that its time becomes
  `time[i] + time[i + 1]`.
...
Output: 62
Explanation: Merge the signs at indices 1 and 2. Remove the sign at index
1, and change the time at index 2 to 8 + 3 = 11.
After the merge:
...
Output: 34
Explanation: Merge the signs at indices 1 and 2. Remove the sign at index
1, and change the time at index 2 to 3 + 9 = 12.
After the merge:
...
Define DP[i][j][s] as the minimum travel time for positions 0..i when i is
kept, j deletions are done overall, and s consecutive deletions occurred
immediately before i.

### Hint 4

Update the DP by either merging (increment s and j) or not merging (reset
s) and adding the appropriate travel time.
