...
contribute to the final count. The queries are independent of each other.
...
The answer should be the maximum of ans[i] = len[i - 1] + len[i + 1], where
i is a one-segment.
...
Note that all three segments (i - 1, i, and i + 1) should be fully covered
by the substring.

### Hint 5

Use a segment tree to perform range maximum queries on the answer. The
query to the segment tree is not straightforward since we need to ensure the
zero-segments are fully covered. Handle the first and last segments
separately.
