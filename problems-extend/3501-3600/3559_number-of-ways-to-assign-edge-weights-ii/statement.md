...
`n - 1`, where `edges[i] = [ui, vi]` indicates that there is an edge
...
You are given a 2D integer array `queries`. For each `queries[i] = [ui,
vi]`, determine the number of ways to assign weights to edges in the path
...
Use Lowest Common Ancestor to find the distance between any two nodes
quickly in O(logn).
