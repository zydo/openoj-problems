...
`ProductPurchases` and `ProductInfo` tables with that testcase's rows.
Every `product_id` in `ProductPurchases` appears in `ProductInfo`, while
products that were never bought still occupy their `ProductInfo` row and
belong to no category pair. A customer who bought several products of the
same category counts for that category exactly once. Write your solution
as a single `SELECT` query returning three columns — `category1`,
`category2`, and `customer_count`. The result format is in the following
example.
