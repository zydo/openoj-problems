...
class TodoList {
  public:
    ...
    int addTask(int userId, string taskDescription, int dueDate,
                vector<string> tags) {
        tasks_.push_back(Task{userId, std::move(taskDescription), dueDate,
                              {tags.begin(), tags.end()}, false});
        return (int)tasks_.size();
    }

    vector<string> getAllTasks(int userId) {
        return collect(userId, nullptr);
    }
    ...
    void completeTask(int userId, int taskId) {
        if (taskId <= 0 || taskId > (int)tasks_.size()) return;
  ...
  private:
    ...
    vector<string> collect(int userId, const string *tag) {
        ...
        for (const Task &task : tasks_) {
            ...
            if (tag != nullptr &&
                std::find(task.tags.begin(), task.tags.end(), *tag) ==
                    task.tags.end()) {
                continue;
            }
        ...
        for (const Task *task : pending) result.push_back(task->description);
        return result;
    }
};
