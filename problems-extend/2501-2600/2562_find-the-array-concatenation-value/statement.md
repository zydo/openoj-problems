...
- If `nums` has a size greater than one, add the value of the concatenation
  of the first and the last element to the concatenation value of `nums`,
  and remove those two elements from `nums`. For example, if the nums was
  `[1, 2, 4, 5, 6]`, add `16` to the concatenation value.
- If only one element exists in `nums`, add its value to the concatenation
  value of `nums`, then remove it.
...
Then we delete them from nums, so nums becomes equal to [13].
