...
Output: 5
Explanation: The subarrays where target = 2 is the majority element are
nums[1..1] = [2], nums[2..2] = [2], nums[1..2] = [2,2], nums[0..2] =
[1,2,2], and nums[1..3] = [2,2,3], so there are 5 such subarrays.
```
...
Output: 0
Explanation: target = 4 does not appear in nums at all, so no subarray can
have it as the majority element. Hence the answer is 0.
```
