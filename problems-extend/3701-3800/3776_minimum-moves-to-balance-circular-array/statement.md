...
Input: balance = [-3,2]
Output: -1
Explanation: It is impossible to make all balances non-negative for
...
If there is no negative value, then the answer is 0. If the total sum is
less than 0, then the answer is -1.

### Hint 2

Sort the positive values in nums by their distance from the index with the
negative value.

### Hint 3

Greedily use as many values as needed from the sorted nums to offset the
