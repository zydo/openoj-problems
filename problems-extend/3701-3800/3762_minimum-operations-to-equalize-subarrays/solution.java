...
class Solution {
    ...
    public long[] minOperations(int[] nums, int k, int[][] queries) {
        ...
        for (int qi = 0; qi < queries.length; qi++) {
            ...
            while (!stack.isEmpty()) {
                int[] top = stack.pop();
                int node = top[0], lo = top[1], hi = top[2];
                if (r < lo || hi < l) {
                    continue;
                }
                if (l <= lo && hi <= r) {
            ...
            result[qi] = median * below[0] - below[1]
                    + ((grandTotal - at[1]) - median * (size - at[0]));
        }
        return result;
    }
    ...
    private void build(int node, int lo, int hi) {
        ...
        int[] left = sortedNodes[2 * node], right = sortedNodes[2 * node + 1];
