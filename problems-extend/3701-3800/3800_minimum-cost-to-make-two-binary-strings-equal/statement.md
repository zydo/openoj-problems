...
t = "10111".
Cross swap s[2] and t[2] (crossCost = 2). After this operation, s = "10100"
and t = "10011".
...
Output: 6
Explanation: Flipping all the bits of s makes the strings equal, and the
total cost is 3 * flipCost = 3 * 2 = 6.
```
...
Count mismatches: a = #(s = '0' , t = '1' ) , b = #(s = '1' , t = '0').

### Hint 2

Pair opposite mismatches: p = min( a , b). Fix each pair with cost
min(swapCost , 2 * flipCost).

### Hint 3

Update counts: a = a - p, b = b - p. Let r = abs( a - b).

### Hint 4

Fix remaining mismatches in pairs: each pair costs
min(crossCost + swapCost , 2 * flipCost).

### Hint 5

If one mismatch remains (r % 2 == 1), pay flipCost.
