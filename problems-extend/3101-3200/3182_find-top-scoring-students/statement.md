...
student_id is the primary key (combination of columns with unique values) for
this table.
...
course_id is the primary key (combination of columns with unique values) for
this table.
...
(student_id, course_id, semester) is the primary key (combination of columns
with unique values) for this table.
Each row of this table contains the student ID, course ID, semester, and grade
received.

Write a solution to find the students who have taken all courses offered in
their major and have achieved a grade of A in all these courses.

Return the result table ordered by student_id in ascending order.

Each testcase supplies its own `dataset`: the DDL seeds the `students`,
`courses`, and `enrollments` tables with that testcase's rows. The result
...
and "Linear Algebra", receiving an 'A' in both.
...
`student_id`, listing every student who (a) has an enrollment for every
course whose `major` equals theirs, and (b) holds a grade of exactly `'A'`
in each of their enrollments that belong to such courses — enrollments in
other majors are ignored; retaking a course never rescues an earlier non-A;
students whose major offers no courses qualify vacuously. Return the rows
