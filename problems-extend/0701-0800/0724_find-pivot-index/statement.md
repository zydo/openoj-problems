...
Output: 3
Explanation: The pivot index is 3. Left sum = nums[0] + nums[1] + nums[2]
...
Create an array `sumLeft` where `sumLeft[i]` is the sum of all the numbers to
the left of index `i`.
...
For each index `i`, check if `sumLeft[i]` equals `sumRight[i]`. If so, return
`i`. If no such `i` is found, return `-1`.
