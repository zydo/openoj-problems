...
Given the `head` of a singly linked list and an integer `k`, split the list
...
Return an array of the `k` parts, where each part is represented as the
sequence of its values and an empty part as `[]`.
