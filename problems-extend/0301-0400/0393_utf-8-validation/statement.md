...
that's correct. But the second continuation byte does not start with 10, so it
is invalid.
```
...
Read the data integer by integer. When you read it, process the least
significant 8 bits of it.

### Hint 2

Assume the next encoding is 1-byte data. If it is not 1-byte data, read the
next integer and assume it is 2-bytes data.

### Hint 3

Similarly, if it is not 2-bytes data, try 3-bytes then 4-bytes. If you read
four integers and it still does not match any pattern, return false.
