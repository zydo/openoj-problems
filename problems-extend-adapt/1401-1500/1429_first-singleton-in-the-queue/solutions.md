# Solutions — First Singleton In The Queue
...
`showSingleton` then exploits a monotonicity the counts give for free:
...
**Complexity:** amortized `O(1)` per `add` and `showSingleton`,
