# Solutions — The Grown Word's K-th Letter II
...
`k = 7` sits inside the last step's appended half, so it maps down to `k = 3`
and banks one shift; `3` then sits inside the middle step's appended half and
maps down to `1`, banking a second shift; the first step leaves position `1`
alone, and `'a'` advanced twice is `'c'`. JavaScript's plain
