class Solution {
    ...
    public long bestFactorScore(int[] nums) {
        ...
        // below 6987268688400, comfortably inside long.
        int n = nums.length;
        long[] preG = new long[n + 1];
        long[] preL = new long[n + 1];
        long[] sufG = new long[n + 1];
        long[] sufL = new long[n + 1];
        preL[0] = 1;
        sufL[n] = 1;
        for (int i = 0; i < n; ++i) {
            preG[i + 1] = gcd(preG[i], nums[i]);
            preL[i + 1] = (preL[i] / gcd(preL[i], nums[i])) * nums[i];
        }
        for (int i = n - 1; i >= 0; --i) {
            sufG[i] = gcd(sufG[i + 1], nums[i]);
            sufL[i] = (sufL[i + 1] / gcd(sufL[i + 1], nums[i])) * nums[i];
        }
        long best = preG[n] * preL[n];
        for (int i = 0; i < n; ++i) {
            long g = gcd(preG[i], sufG[i + 1]);
            long l = (preL[i] / gcd(preL[i], sufL[i + 1])) * sufL[i + 1];
