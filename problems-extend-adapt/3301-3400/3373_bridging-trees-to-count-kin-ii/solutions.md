# Solutions — Bridging Trees To Count Kin II

Distance parity in a tree is the XOR of the two endpoints' depth parities
(every edge on a path flips the parity once), so "kin" is exactly "same
...
routing through the connecting edge makes `w` kin to `i` exactly when
