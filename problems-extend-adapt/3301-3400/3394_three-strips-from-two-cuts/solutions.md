# Solutions — Three Strips From Two Cuts

## Sort projections and count gaps

A cut across the board is legal for one axis exactly when no rectangle
...
groups, each group landing in one strip. Since the rectangles never
...
then the y-axis answers the question. The board size `n` never enters
