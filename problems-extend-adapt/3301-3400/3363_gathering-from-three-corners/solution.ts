function mostFruitsGathered(fruits: number[][]): number {
    ...
    const triangle = (swapped: boolean): number => {
        ...
        const cell = (i: number, j: number): number => (swapped ? fruits[j][i] : fruits[i][j]);
