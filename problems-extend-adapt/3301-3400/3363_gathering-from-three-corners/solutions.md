# Solutions — Gathering From Three Corners

## Fixed Diagonal and Two Boundary Dynamic Programs

The first picker's moves always advance both coordinates, so after `n − 1`
...
pickers advance one coordinate per move (rows for the top-right picker,
columns for the bottom-left picker), which pins each of them to one side
...
picker 1 has already emptied every plot — so each picker's optimum is its
...
bottom-left picker is the identical DP with rows and columns swapped, so
