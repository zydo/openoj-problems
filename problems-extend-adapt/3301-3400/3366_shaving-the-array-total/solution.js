 ...
 * @return {number}
 */
var smallestTotal = function (nums, k, op1, op2) {
    ...
    let nxt = Array.from({ length: op1 + 1 }, () => new Array(op2 + 1).fill(0));
    for (let i = nums.length - 1; i >= 0; --i) {
        const value = nums[i];
        const halved = (value + 1) >> 1;
        const cur = Array.from({ length: op1 + 1 }, () => new Array(op2 + 1).fill(0));
        for (let a = 0; a <= op1; ++a) {
            for (let b = 0; b <= op2; ++b) {
