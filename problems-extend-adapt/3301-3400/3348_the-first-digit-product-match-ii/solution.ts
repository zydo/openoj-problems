function firstDigitProductMatch(num: string, t: number): string {
    ...
    const vec: number[][] = Array.from({ length: 10 }, () => [0, 0, 0, 0]);
    for (let d = 2; d <= 9; d++) {
        primes.forEach((prime, idx) => {
    ...
    const minDigits = (r: number[]): number => {
    ...
    const build = (length: number, r: number[]): string => {
        const out: string[] = [];
