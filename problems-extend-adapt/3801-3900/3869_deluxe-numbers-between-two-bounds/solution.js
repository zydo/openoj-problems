...
var countDeluxe = function (l, r) {
    ...
    const sleek = new Array(145).fill(0);
    ...
    const overlap = new Array(sleeks.length + 1).fill(0);
    ...
    const countOverlap = (x) => overlap[countSleek(x)];

    const countSleekSum = (x) => {
        ...
        const ways = Array.from({ length: n + 1 }, () => new Array(145).fill(0));
        ways[0][0] = 1;
        for (let k = 1; k <= n; k++) {
            for (let t = 0; t <= 144; t++) {
                let total = 0;
    ...
    const countDeluxeUpTo = (x) => countSleekSum(x) + countSleek(x) - countOverlap(x);
