 ...
 * @return {number}
 */
var maxAbsorbedWindows = function (nums) {
    ...
    // and every intermediate stays below 10^10 — both far below 2^53 ≈
    // 9.0*10^15, where doubles represent integers exactly.
    const n = nums.length;
    ...
    const last = new Array(bits).fill(-1);
    const nxt = new Array(bits).fill(n);
