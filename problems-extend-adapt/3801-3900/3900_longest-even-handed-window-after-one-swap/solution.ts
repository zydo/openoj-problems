function longestEvenHandedWindow(s: string): number {
    const longestWithDelta = (target: number, cap: number): number => {
        const prefix = new Array<number>(s.length + 1).fill(0);
        const positions = new Map<number, { values: number[]; head: number }>([[0, { values: [0], head: 0 }]]);
        let best = 0;
