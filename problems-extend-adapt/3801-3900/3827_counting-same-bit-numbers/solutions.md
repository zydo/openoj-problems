# Solutions — Count Same-bit Integers

A Same-bit integer has every bit of its binary representation identical. A
positive integer's representation begins with a 1, so its bits can all agree
only when every one of them is 1: the positive Same-bit integers are exactly
