function rightTriangleCount(n: number, edges: number[][], x: number, y: number, z: number): number {
    ...
    const distances = (source: number): number[] => {
    ...
    for (let node = 0; node < n; node++) {
        ...
        // Squares reach 2 * 10^10, far below 2^53: exact as numbers.
        if (a * a + b * b === c * c) {
            answer += 1;
        }
    }
