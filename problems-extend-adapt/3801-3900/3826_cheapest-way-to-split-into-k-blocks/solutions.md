# Solutions — Cheapest Way To Split Into K Blocks
...
as the minimum cost for covering exactly the first i elements with exactly
...
f(a) - f(b) = a·b > 0, so the k = 1 cost is the maximum any partition can
