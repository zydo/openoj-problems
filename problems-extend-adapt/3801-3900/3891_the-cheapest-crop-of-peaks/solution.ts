// Peaks can only be created by raising their own cell, so two of them can
...
// values. Each cost is at most 10^9 and there are at most n/2 picks, so the
// total stays far below 2^53 and every intermediate is an exact TS number.
function cheapestPeaks(nums: number[]): number {
    const n = nums.length;
    ...
    for (let i = 1; i < n - 1; i++) {
        ...
        let curNotCount: number, curNotCost: number;
