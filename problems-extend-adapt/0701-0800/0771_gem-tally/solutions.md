...
count individually, which is the whole semantics: against `jewels = "bR"` the
string `"RRbBrb"` contributes its two `R`s, its two `b`s, and none of its
`B` or `r`, for a count of 4. Nothing about the scan depends on the order of
either string, and a stone absent from the table falls through with the same
single lookup a match costs.
