# Solutions — Unique Palindromic Subsequences
...
exactly `"x"` and `"xx"` survive and the cell is 2. In `"abba"` this prices
the `a`-outered family at `2 + dp[b]` over the interior `"bb"` = 4 (`a`,
`aa`, `aba`, `abba`), the `b`-family shrinks down to that same inner `"bb"`
window for a price of 2 (`b`, `bb`), and the total is 6.
...
limit. Raw counts pass `10^9 + 7` well before the bound (the 64-character
example already holds 2,695,180,477) and grow past every fixed width, so
