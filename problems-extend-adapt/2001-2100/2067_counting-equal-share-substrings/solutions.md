# Solutions — Counting Equal-Share Substrings

## Fixed windows for each distinct-letter count

If an equal-share substring contains `d` distinct letters, its length must be `d * count`. Try every `d` from 1 through 26 and slide a window of that fixed length, stopping once the required length exceeds the string.
