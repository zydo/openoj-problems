function chessMoves(piece, position) {
    const orthogonal = [
        [1, 0],
        [-1, 0],
        [0, 1],
 ...
 * @return {number}
 */
var countClashFreeMoves = function (pieces, positions) {
    const options = pieces.map((piece, index) => chessMoves(piece, positions[index]));
    const chosen = [];

    const compatible = (index, move, other) => {
