function cheapestRoute(n: number, highways: number[][], discounts: number): number {
    ...
    const distances = Array.from({ length: n }, () => new Array<number>(discounts + 1).fill(Infinity));
    distances[0][0] = 0;
    const heap: Array<[number, number, number]> = [[0, 0, 0]];
    const push = (state: [number, number, number]): void => {
        heap.push(state);
