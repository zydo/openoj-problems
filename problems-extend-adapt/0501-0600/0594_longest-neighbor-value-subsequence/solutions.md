# Solutions — Longest Neighbor-Value Subsequence

## Count map over adjacent value pairs

A subsequence only deletes elements, and deletion never constrains the
survivors: any subset of positions may be kept, so which neighbor-value
runs exist depends on the multiset of values alone, never on the order
they appear in. A neighbor-value run spans a max-min gap of exactly 1,
...
stays 0, while Example 1's counts of five 14s and three 15s make the pair
`(14, 15)` the winner at length 8.
