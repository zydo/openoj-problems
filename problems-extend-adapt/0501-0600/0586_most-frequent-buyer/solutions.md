...
`Purchases` row is exactly one placed order, the group's `COUNT(*)` is
...
COUNT(*) AS c FROM Purchases GROUP BY customer_number))` reports several
winners by keeping every group at the maximum instead of just the top
one.

One aggregation pass reads the `N` rows of `Purchases` once and materializes
