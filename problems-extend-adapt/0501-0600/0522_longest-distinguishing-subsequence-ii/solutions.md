# Solutions — Longest Distinguishing Subsequence II
...
distinguishing. Conversely, when no other string contains `s`, the string
`s` itself is a subsequence of exactly one input string and is therefore
distinguishing. Equal strings contain each other, so a repeated string
disqualifies every copy of itself — that is what the second example's
three copies of `"moo"` run into, and `"m"` is disqualified too since it
sits inside every `"moo"`.
