# Solutions — URL Shortener Codec

## Counter-indexed base-36 suffixes

`shorten` appends the URL to a list the object keeps for its lifetime and
returns `http://tinyurl.com/` followed by the URL's 1-based position in that
list, written in lowercase base-36 — `1` for the first shorten, `a` for the
...
`expand` is the table lookup that scheme implies. It strips the fixed
...
what makes the list a complete expand table: every position the suffix
alphabet can name up to the object's count was handed out by that object's own
`shorten`, so the lookup never misses and never needs to validate.
