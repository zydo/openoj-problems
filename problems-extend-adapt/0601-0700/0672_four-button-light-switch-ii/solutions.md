# Solutions — Four-Button Light Switch II
...
and 4 coincide there, each flipping only bulb 1, so a single press of
either one lands on the same status as the other, leaving three reachable
statuses rather than four — and all four pairs from `p = 2`. For `n >= 3`
the counts are 4, 7, 8 at `p = 1, 2, 3+`; the one string `p = 2` misses is
