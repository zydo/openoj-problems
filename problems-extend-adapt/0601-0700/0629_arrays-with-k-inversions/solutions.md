# Solutions — Arrays With K Inversions
...
forms no other inversions and disturbs nothing already placed. Each
arrangement of `1..m` with `j` inversions therefore splits uniquely into
an arrangement of `1..m-1` with `j-i` inversions plus a slot choice `i` in
`0..m-1`, giving `dp[m][j]` as the sum of `dp[m-1][j-i]` over that range,
seeded by `dp[1][0] = 1`. For `n = 3` the rows grow `[1]`, `[1,1]`,
`[1,2,2,1]` — `dp[3][0]` is the lone identity arrangement `[1,2,3]`, and
`dp[3][1]` counts the two single-inversion arrangements `[1,3,2]` and
`[2,1,3]`.
