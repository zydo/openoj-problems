# Solutions — Fairway Tree Felling

## Breadth-first search, shortest tree first

The rule fixes the itinerary completely: trees must fall shortest to tallest, so
...
no tour to optimize — a "greedy" ascending order is the only legal one. Felling a
tree rewrites its cell to `1`, which is still walkable, so the grid never changes
underneath the crew and every leg can be searched on the original forest.
