# Solutions — Knight Survival Probability
...
On the first example (`n = 4`, `k = 1`, corner start) a single sweep
leaves `2/8` of mass on the board — only the moves to `(1, 2)` and
`(2, 1)` land inside a `4 x 4` board — so the answer is `0.25` directly.
The edges need no special cases: `k = 0` never enters the sweep and
returns the untouched `1.0`, and boards small enough that every move
leaves them (a `1 x 1` board, or the center of a `3 x 3`) simply drain
to `0.0` after the first sweep and stay there for any further `k`.
