...
numbering a level's span is exactly its last index minus its first index plus
...
definition speaks of. A one-node level has span 1, a chain keeps every level
...
stored index never exceeds twice its level's span. A span is a difference
between two indices of one level, and a shift by the level's own constant
leaves every such difference unchanged — the measured spans, and only those,
survive intact. Even re-based, a child index can reach twice the level's
span — one bit past the 32-bit answer itself — so the indices stay 64-bit.
...
dense tree costs its widest level, and a sparse one — where the span lives in
the gaps, not the nodes — costs almost nothing.

**Complexity:** `O(n)` time, `O(span)` space.
