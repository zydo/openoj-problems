function minimumLetterSheets(stickers: string[], target: string): number {
    ...
    const full = (1 << m) - 1;
    const available: boolean[] = new Array(26).fill(false);
    ...
    const distance: number[] = new Array(full + 1).fill(-1);
    distance[0] = 0;
