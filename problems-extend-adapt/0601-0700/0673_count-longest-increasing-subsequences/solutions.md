# Solutions — Count Longest Increasing Subsequences
...
`[3, 1, 4, 1, 5, 9, 2, 6]` the length-4 arrivals at the trailing 9 and 6
each decompose through two distinct predecessors apiece, summing to four;
in `[7, 7, 7, 7]` strictness blocks every extension, leaving four
length-1 subsequences. The double loop is `n²` pair checks — four million
at the `n = 2000` ceiling.
