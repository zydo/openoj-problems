class Solution {
  public:
    int bestTripleProduct(vector<int> &nums) {
        ...
        // as long long before the comparison, per the house rule.
        long long top = (long long)max1 * max2 * max3;
        long long spread = (long long)min1 * min2 * max1;
        return int(max(top, spread));
    }
};
