# Solutions — Widest Non-Decreasing Span

A span's width is pure index distance, so for any fixed right end the best
...
holds a smaller value, because that earlier position would widen every span
...
greater width — `i` can never be the left end of a widest span. Plateaus are
...
remaining candidates `j'` all lie further left, so any span this top could
still form is narrower than the one just recorded. In Example 1 the record
lows of `[7,2,9,4,3,6]` sit at indices 0 and 1; scanning back, `j = 5` (the 6) pops the 2 at index 1 for width 4, and the 7 at index 0 is only popped at
