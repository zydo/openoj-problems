class Solution {
    ...
    public String stringWithoutTriples(int a, int b) {
        ...
        while (big > small && small > 0) {
            answer.append(bigLetter).append(bigLetter).append(smallLetter);
            big -= 2;
            small -= 1;
        }
        while (big > 0 || small > 0) {
            if (big > 0) {
                answer.append(bigLetter);
                big--;
            }
            if (small > 0) {
                answer.append(smallLetter);
                small--;
            }
        }
