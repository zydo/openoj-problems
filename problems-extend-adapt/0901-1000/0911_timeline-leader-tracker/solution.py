...
class TimelineLeaderTracker:
    ...
    the tied candidates. ``leaderAt(t)`` then only has to locate the last vote at
    or before ``t``, which is a binary search because ``times`` is
    strictly increasing, and read the leader recorded there.
    """

    def __init__(self, labels: List[int], times: List[int]) -> None:
