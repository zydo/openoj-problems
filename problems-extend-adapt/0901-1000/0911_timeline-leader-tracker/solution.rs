...
// recorded there.
pub struct TimelineLeaderTracker {
    times: Vec<i32>,
    leaders: Vec<i32>,
}

impl TimelineLeaderTracker {
    pub fn new(labels: Vec<i32>, times: Vec<i32>) -> Self {
        // person ids are dense in [0, n), so a plain count array indexes them
        let mut counts = vec![0; labels.len()];
        let mut leaders = Vec::with_capacity(labels.len());
        let mut best = 0;
        let mut leader = 0;
        ...
        TimelineLeaderTracker { times, leaders }
    }

    pub fn leaderAt(&mut self, t: i32) -> i32 {
