...
var tallestTwinSupport = function (rods) {
    ...
    // support reachable with left - right == d; unreachable differences hold
    // -1. Each rod is welded left, welded right, or discarded.
    const total = rods.reduce((acc, rod) => acc + rod, 0);
    const span = 2 * total + 1;
    let best = new Array(span).fill(-1);
    // index d + total keeps every difference non-negative
    best[total] = 0;
    for (const rod of rods) {
        const nxt = new Array(span).fill(-1);
