class Solution {
  public:
    vector<int> parent;
    vector<int> sz;

    int find(int x) {
        while (parent[x] != x) {
            parent[x] = parent[parent[x]];
            x = parent[x];
        }
        return x;
    }

    void unite(int a, int b) {
        int ra = find(a), rb = find(b);
        if (ra == rb)
            return;
        if (sz[ra] < sz[rb])
            swap(ra, rb);
        parent[rb] = ra;
        sz[ra] += sz[rb];
    }

    int chooseIsolationII(vector<vector<int>> &graph, vector<int> &initial) {
        // Deleting a node erases its edges, so only the removed initial node
        // can stop the spread: a component of non-initial nodes is infected
        // exactly when some initial node stands adjacent to it, and it is
        // spared iff its sole adjacent initial node is the one removed.
        // Union-find builds those components (merging only pairs of
        // non-initial nodes); the answer maximizes the total size spared,
        // ties to the smallest index, else the smallest initial node.
        int n = graph.size();
        vector<bool> infected(n, false);
        for (int node : initial)
            infected[node] = true;
        parent.resize(n);
        for (int i = 0; i < n; i++)
            parent[i] = i;
        sz.assign(n, 1);

        for (int i = 0; i < n; i++) {
            if (infected[i])
                continue;
            for (int j = i + 1; j < n; j++) {
                if (!infected[j] && graph[i][j] == 1)
                    unite(i, j);
            }
        }

        vector<int> touches(n, 0); // per root: how many distinct initial nodes adjoin it
        vector<int> owner(n, -1);  // per root: the sole adjoining initial node
        for (int node : initial) {
            for (int j = 0; j < n; j++) {
                if (graph[node][j] == 1 && !infected[j]) {
                    int root = find(j);
                    if (owner[root] == -1) {
                        owner[root] = node;
                        touches[root] = 1;
                    } else if (owner[root] != node) {
                        touches[root] = 2;
                    }
                }
            }
        }

        vector<int> saved(n, 0);
        for (int root = 0; root < n; root++)
            if (touches[root] == 1)
                saved[owner[root]] += sz[root];

        int bestNode = -1;
        int bestSaved = -1;
        for (int node : initial) {
            if (saved[node] > bestSaved || (saved[node] == bestSaved && node < bestNode)) {
                bestNode = node;
                bestSaved = saved[node];
            }
        }
        return bestNode;
    }
};
