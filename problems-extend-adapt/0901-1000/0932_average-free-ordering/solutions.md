# Solutions — Average-Free Ordering
...
Why the doubling produces an average-free ordering: within the front block every value is
...
`[1]`, which is average-free, rules out in-block violations at every size. When the
...
recursion — average-free(n) as the odd images of average-free(`ceil(n / 2)`) followed by
the even images of average-free(`floor(n / 2)`) — because `2 * x - 1` exceeds `n`
