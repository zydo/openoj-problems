function hasDigitSolution(words: string[], result: string): boolean {
    ...
    const code = (ch: string): number => ch.charCodeAt(0) - 65;
    const seen = new Array<boolean>(26).fill(false);
    const leading = new Array<boolean>(26).fill(false);
    ...
    const value = new Array<number>(26).fill(-1);
    const used = new Array<boolean>(10).fill(false);

    const dfs = (pos: number, row: number, carry: number): boolean => {
