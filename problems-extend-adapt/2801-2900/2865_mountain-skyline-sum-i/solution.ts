...
function maxSkylineSum(heights: number[]): number {
...
function rampSums(nums: number[]): number[] {
    const best = new Array<number>(nums.length).fill(0);
    const runHeight: number[] = []; // parallel run stacks, strictly rising
