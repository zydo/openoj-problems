# Solutions — Checkout After Rounding The Bill

## Add five, then floor to tens

Everything reduces to computing `roundedAmount` without searching for it.
Adding 5 to `price` and then flooring to a multiple of 10 does the
whole job in one expression: `(price + 5) / 10`, floored, counts
...
The nudge is precisely the stated tie rule. Split `price` into its
...
non-negative — `price + 5` never exceeds 105, and the rounded
