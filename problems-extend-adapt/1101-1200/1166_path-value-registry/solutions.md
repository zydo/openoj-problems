...
complete absolute path, and the two rules that govern `addPath` — no
...
`addPath` rejects a repeat or an orphan outright; otherwise it records
...
`addPath`) against keys of length at most 100 — `O(L)` time for path
