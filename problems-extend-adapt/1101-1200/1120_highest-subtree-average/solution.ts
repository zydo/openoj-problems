function highestSubtreeAverage(root: TreeNode | null): number {
    ...
    const aggregate = new Map<TreeNode, [number, number]>(); // node -> [sum, size]
    let best = 0.0;
    for (let i = order.length - 1; i >= 0; --i) {
        ...
        for (const child of [node.left, node.right]) {
            if (child !== null) {
                const pair = aggregate.get(child) as [number, number];
                total += pair[0];
                size += pair[1];
            }
        }
