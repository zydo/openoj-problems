# Solutions — The Skipping Serpentine Sweep

## One walk with a take/skip toggle

The serpentine order is fixed by the grid alone: even rows are read
...
the order the sweep visits them. No coordinates need to be tracked;
...
sweep is `O(m * n)` time with `O(1)` state beyond the output list
itself.
