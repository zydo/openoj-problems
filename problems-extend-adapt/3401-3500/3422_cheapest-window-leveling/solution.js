 ...
 * raw keys -- with running half-sums making each window's cost O(1). Keys
 * pack (v + 2^20) * 2^17 + index, so they never tie, max(low) < min(high)
 * strictly, and a leaving element routes to its true half by one
 * comparison; deletion is lazy, dropping stale copies at heap tops. Keys
 * stay under 2^38 and costs under 2 * 10^11, exact as JS numbers.
 * @param {number[]} nums
 * @param {number} k
 * @return {number}
 */
var minLevelingCost = function (nums, k) {
    const push = (h, x) => {
        h.push(x);
        let c = h.length - 1;
        while (c > 0) {
    ...
    // below 2^53.
    const key = (v, i) => (v + 1048576) * 131072 + i;
    const val = (key_) => Math.floor(key_ / 131072) - 1048576;
    const idx = (key_) => key_ % 131072;
