# Solutions — Leads With Well-Formed Addresses

An address is well-formed when it has exactly one `@`, a local part built only
...
The subquery keeps rows where `length(address) -
length(replace(address, '@', '')) = 1` — exactly one `@` — and, for those, `instr` finds its position
...
checks as shape checks: `length(local_part) > 0` plus
...
demands the domain be letters only. `ORDER BY lead_id ASC` finishes the
contract.
