function minClearCost(nums: number[]): number {
    ...
    const rowN: number[] = [0].concat(nums);
    const rowNm1: number[] = [nums[n - 1]];
    for (let c = 0; c < n - 1; c++) {
        rowNm1.push(Math.max(nums[c], nums[n - 1]));
    }
    const rowNm2: number[] = [Math.max(nums[n - 2], nums[n - 1])];
    for (let c = 0; c < n - 2; c++) {
        const a = nums[c];
        const b = nums[n - 2];
        const d = nums[n - 1];
    ...
    let ring: number[][] = [rowNm2, rowNm1, rowN];
    for (let j = n - 3; j >= 0; j--) {
        const r2 = ring[1];
        const r3 = ring[2];
        const a = nums[j];
