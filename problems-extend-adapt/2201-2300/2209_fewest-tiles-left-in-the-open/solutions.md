# Solutions — Fewest Tiles Left in the Open

## Suffix DP over tile positions

Each tile faces a binary choice once the state is fixed, which is what makes
a two-index dynamic programming exact: `dp[i][j]` is the fewest bright tiles
still exposed among `floor[i:]` when at most `j` rugs remain. Either tile
`i` is left in the open and pays `floor[i]` on top of `dp[i+1][j]`, or a rug
is laid with its left end exactly at `i` — hiding `i .. i+carpetLen-1` and
jumping the state to `dp[min(i+carpetLen, n)][j-1]`. Enumerating the rug's
left end at the current tile loses no placement: any optimal cover can be
scanned left to right, and its leftmost rug that touches the suffix
...
is already computed; the `j = 0` column is just the suffix bright counts.
Because the recurrence takes a minimum over placements rather than a sum,
overlapping or wasted rugs never hurt — extra rugs simply keep the
...
with `2` rugs of length `2`, the four bright runs cannot all be hidden and
the optimum leaves `2` bright tiles.
