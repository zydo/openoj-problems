# Solutions — Running a Sale on Dollar Words
...
every legal input because there is never anything to round — and the first
example pins exactly this reading (`$8` at 25% is precisely 600 cents, hence
`$6.00`). The code therefore computes cents in pure integer arithmetic — a
