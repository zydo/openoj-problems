# Solutions — The Cheapest Prime Sum
...
handled naturally — example 1's `26 = 5 + 5 + 5 + 5 + 3 + 3` is the chain of six
such transitions.
...
`dp[n]` never leaves its sentinel value no multiset exists and `cheapestPrimeCount`
