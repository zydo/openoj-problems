# Solutions — The Warp-Gate Crossing

## Layered dynamic programming over warp counts

Group routes by how many warps they spend and solve one layer per
count. `d[t][i][j]` is the cheapest way to be standing on `(i, j)` having
used at most `t` warps, so layer 0 is the ordinary right/down minimum
path sum with the start cell free. A later layer opens with its warp:
...
warp entry cost; a single row-major pass then layers normal moves on
