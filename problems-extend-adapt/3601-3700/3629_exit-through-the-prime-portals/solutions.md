# Solutions — Exit Through The Prime Portals
...
could portal to are exactly the entries of `bucket[p]` — every index
...
total portal work is `Σ max(nums)/p` over the distinct prime values
