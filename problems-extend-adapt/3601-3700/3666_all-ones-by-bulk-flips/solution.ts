function minBulkFlips(s: string, k: number): number {
    ...
    const find = (nxt: number[], i: number): number => {
        let root = i;
        while (nxt[root] !== root) {
            root = nxt[root];
        }
    ...
    const nextEven: number[] = Array.from({ length: Math.floor(n / 2) + 2 }, (_, i) => i);
    const nextOdd: number[] = Array.from({ length: Math.floor((n + 1) / 2) + 1 }, (_, i) => i);
