...
Concretely, for the second example `word1 = "beta"` and `word2 = "delta"`: `"beta"` lands at indices 1 and 4, `"delta"` at index 3. At index 3 the gap is `abs(3 - 1) = 2`; at index 4 `"beta"` refreshes `index1` and the gap tightens to `abs(4 - 3) = 1`, the answer. The earlier `"beta"` at index 1 never needs revisiting — the newer occurrence at 4 is closer to every later `"delta"`.
