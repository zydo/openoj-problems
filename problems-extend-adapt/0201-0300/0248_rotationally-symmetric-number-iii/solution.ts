function rotationalSymmetricsInRange(low: string, high: string): number {
    // 0, 1 and 8 rotate to themselves, 6 and 9 swap; anything else is not
    // a strobogrammatic digit and fails any equality test.
    const rotate = (digit: string): string =>
        digit === "6" ? "9" : digit === "9" ? "6" : "018".includes(digit) ? digit : "?";
    ...
    const choicesAt = (position: number, length: number, half: number): string => {
    ...
    const totalOfLength = (length: number): number => {
    ...
    const countAtLeast = (boundary: string): number => {
        const length = boundary.length;
        const half = Math.ceil(length / 2);
        const ways: number[] = new Array(half + 1).fill(1);
        for (let position = half - 1; position >= 0; --position) {
    ...
    const isStrobogrammatic = (value: string): boolean => {
