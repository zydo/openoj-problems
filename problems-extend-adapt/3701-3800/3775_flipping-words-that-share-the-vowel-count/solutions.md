# Solutions — Flipping Words That Share the Vowel Count
...
flipped exactly when its own count equals that target. Flipping is the
...
more only when its word is flipped). Nothing beyond the word list and
...
destroy word boundaries, and flipping the first word would break
inputs whose first word matches its own count — the statement flips
