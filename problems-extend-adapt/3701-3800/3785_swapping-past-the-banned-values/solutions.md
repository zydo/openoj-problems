# Solutions — Swapping Past The Banned Values

## Bad-cluster counting

Call an index bad when nums[i] equals banned[i]. Feasibility is a pure
counting question: a value v that occurs c_n times in nums and c_f times
in banned can only occupy slots that do not forbid it, and there are
...
v with one swap against a donor slot whose value and banned value
...
Two hash-map passes realize the formula: one folds nums and banned
