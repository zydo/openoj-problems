# Solutions — The Longest Fair-Share Substring I
...
A window of length `len` over `distinct` live letters is fair-share precisely
...
Each qualifying window is a candidate answer, so the sweep records the best
length seen.
