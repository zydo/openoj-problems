# Solutions — Summing Digit Bends In A Range I
...
every number in `[num1, num2]` once and add up its bends. The only real
content is computing one number's bends correctly. Rather than converting
...
previous digit that is strictly greater than both neighbors bends upward; one
strictly smaller than both bends downward; equality on either side
disqualifies it. The first and last digits never enter this comparison as the
middle element, which is exactly the rule that only interior digits can bend,
and numbers with fewer than three digits simply never trigger it.
