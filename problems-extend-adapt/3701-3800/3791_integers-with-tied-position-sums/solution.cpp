...
class Solution {
  public:
    long long countTiedSums(long long low, long long high) { return countUpTo(high) - countUpTo(low - 1); }
  ...
  private:
    long long countUpTo(long long x) {
        ...
        // reach 10^15 + 1, past 32 bits, so the table holds long longs.
        std::vector<std::vector<long long>> ways(n + 1, std::vector<long long>(size, 0));
        ways[n][span] = 1;
        for (int i = n - 1; i >= 0; i--) {
            int sign = (i % 2 == 0) ? 1 : -1;
