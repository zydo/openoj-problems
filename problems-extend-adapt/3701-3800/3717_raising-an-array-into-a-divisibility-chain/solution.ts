function minChainCost(nums: number[]): number {
    ...
    const divisors: number[][] = Array.from({ length: limit + 1 }, () => []);
    ...
    let dp: number[] = new Array(limit + 1).fill(inf);
    dp[nums[0]] = 0;
    for (let i = 1; i < nums.length; i++) {
        const need = nums[i];
        const ndp: number[] = new Array(limit + 1).fill(inf);
