function zeroFreeTimesDigitSum(s: string, queries: number[][]): number[] {
    ...
    const prefSum: number[] = new Array(n + 1).fill(0);
    const prefCnt: number[] = new Array(n + 1).fill(0);
    const pow10: number[] = new Array(n + 1).fill(1);
    for (let i = 0; i < n; i++) {
