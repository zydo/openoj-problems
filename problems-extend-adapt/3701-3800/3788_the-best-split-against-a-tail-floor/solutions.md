# Solutions — The Best Split Against A Tail Floor

## Reverse sweep with running accumulators

Both quantities a split needs are trailing views of one left-to-right scan.
The head's running total is what a running sum has accumulated at stop `i`, and
the tail minimum is what a running minimum over the tail holds when the split
