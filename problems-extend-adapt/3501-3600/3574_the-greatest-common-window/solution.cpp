class Solution {
  public:
    long long bestGcdWindow(vector<int> &nums, int k) {
        ...
        int n = nums.size();
        vector<int> odd(n), tier(n);
        for (int i = 0; i < n; ++i) {
            int low = nums[i] & (-nums[i]);
            odd[i] = nums[i] / low;
            tier[i] = __builtin_ctz(low);
        }
        long long p2[34];
        p2[0] = 1;
        for (int i = 1; i < 34; ++i)
