 ...
 * @return {number}
 */
var smallestDropBudget = function (nums, k) {
    ...
    const count = (x) => {
        ...
        for (const v of nums) {
            ...
            const hi = upperBound(vals, v + x);
            const c = bisectLeft(vals, v);
            for (let i = hi; i > 0; i -= i & -i) total += tree[i];
