function quickestCrossing(n: number, k: number, m: number, time: number[], mul: number[]): number {
    ...
    const groups: number[][] = Array.from({ length: full + 1 }, () => []);
    ...
    const mx = new Array<number>(full + 1).fill(0);
    ...
    const keyOf = (mask: number, j: number, side: number): number => mask * 10 + j * 2 + side;
    const heap: number[][] = [[0, keyOf(full, 0, 0), full, 0, 0]];
