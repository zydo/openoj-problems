function stockTradingWithShorts(prices: number[], k: number): number {
    ...
    const NEG = -1e15;
    let done: number[] = new Array(k + 1).fill(NEG);
    let openLong: number[] = new Array(k + 1).fill(NEG);
    let openShort: number[] = new Array(k + 1).fill(NEG);
    done[0] = 0;
