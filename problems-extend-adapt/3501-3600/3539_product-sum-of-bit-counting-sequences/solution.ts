...
function productSum(m: number, k: number, nums: number[]): number {
    ...
    let dp: number[][][] = Array.from({ length: m + 1 }, () =>
        Array.from({ length: m + 1 }, () => new Array(32).fill(0)),
    );
    dp[0][0][0] = 1;
    for (let i = 0; i < n; i++) {
        const ndp: number[][][] = Array.from({ length: m + 1 }, () =>
            Array.from({ length: m + 1 }, () => new Array(32).fill(0)),
        );
        for (let j = 0; j <= m; j++) {
            for (let b = 0; b <= m; b++) {
