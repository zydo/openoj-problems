...
function maxPairings(grid: number[][]): number {
    ...
    const invitations: number[] = new Array(n).fill(-1); // girl j is invited by boy invitations[j]

    const invite = (boy: number, seen: Set<number>): boolean => {
