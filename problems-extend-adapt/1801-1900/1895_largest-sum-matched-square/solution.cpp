class Solution {
  public:
    ...
    int largestBalancedSquare(vector<vector<int>> &grid) {
        ...
        auto rsum = [&](int i, int j, int k) { return rs[i + 1][j + k] - rs[i + 1][j]; };
        auto csum = [&](int i, int j, int k) { return cs[i + k][j + 1] - cs[i][j + 1]; };
