...
// top bit first. Children of node live at 2 * node and 2 * node + 1 in next;
// each element is counted against the trie before it is inserted, so every
// unordered pair is counted exactly once. Counts stay under 2 * 10^8, far
// below 2^53, so plain numbers are exact.
function pairsInXorWindow(nums: number[], low: number, high: number): number {
    const next: number[] = [-1, -1];
    const cnt: number[] = [0];
    ...
    const countAtMost = (x: number, k: number): number => {
        let node = 0;
        let total = 0;
        for (let b = 14; b >= 0; b--) {
            const xb = (x >> b) & 1;
