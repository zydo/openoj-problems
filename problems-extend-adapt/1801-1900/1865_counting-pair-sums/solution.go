// PairSums keeps a frequency map of nums2 and scans the short nums1 on
...
func NewPairSumsTyped(nums1 []int, nums2 []int) *PairSums {
	freq2 := make(map[int]int64, len(nums2)*2)
	for _, v := range nums2 {
		freq2[v]++
	}
	return &PairSums{nums1: nums1, nums2: nums2, freq2: freq2}
}

func (design *PairSums) add(index int, val int) {
...
func (design *PairSums) count(tot int) int64 {
	var total int64
