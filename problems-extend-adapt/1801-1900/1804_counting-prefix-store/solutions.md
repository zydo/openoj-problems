# Solutions — Counting Prefix Store
...
`countPrefixed` walks the prefix as far as its nodes exist and
returns the `prefix_count` found there; when the walk falls off the trie
the answer is 0. `countExact` walks the whole word the same way and
