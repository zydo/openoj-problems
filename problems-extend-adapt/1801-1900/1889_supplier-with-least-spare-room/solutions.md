# Solutions — Supplier with the Least Spare Room

For a fixed supplier the assignment is forced — each package takes its
smallest fitting box, because spare room is `sum(box sizes) - sum(package
...
the spare room, and advance. A supplier whose largest box cannot hold the
biggest package is skipped outright. Take the minimum spare room over
suppliers; `-1` if none qualifies.

Spare-room magnitudes reach `10^5` packages times `10^5` box size
