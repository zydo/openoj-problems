# Solutions — Reversal Twin Pairs

The twin-pair condition mixes four values, but it is linear in each index, so it
...
indices form a twin pair exactly when their keys are equal, which turns the
