# Solutions — Adjacent Swaps to the Kth Next Arrangement

Two independent facts compose the solution. First, the kth next
arrangement is exactly k applications of next permutation to the
digit array. Second, the cheapest way to turn `num` into a digit multiset
into the target with adjacent swaps is to keep equal digits in their
