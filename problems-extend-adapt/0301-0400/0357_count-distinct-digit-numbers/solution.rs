impl Solution {
    pub fn count_distinct_digit_numbers(n: i32) -> i32 {
        ...
        let mut total = 1; // f(0): x = 0 itself
        let mut run = 1; // f(k) so far, one factor per digit place
        for length in 1..=n.min(10) {
            // place 1 picks 1-9; place k >= 2 finds 11 - k digits still free
            run *= if length == 1 { 9 } else { 11 - length };
            total += run;
        }
        total
    }
