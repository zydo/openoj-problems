function incrementDigits(head: ListNode | null): ListNode | null {
    ...
    let last: ListNode = sentinel;
    for (let current = sentinel.next; current !== null; current = current.next) {
        if (current.val !== 9) {
            last = current;
        }
    }
    last.val += 1;
    for (let current = last.next; current !== null; current = current.next) {
        current.val = 0;
    }
