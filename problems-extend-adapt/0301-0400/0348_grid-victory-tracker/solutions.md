# Solutions — Grid Victory Tracker

## One counter per line, four touched per placeMark
...
marks on every row, on every column, and on each diagonal. A `placeMark` bumps
...
touched reaches `n`. Since every placeMark lands on an empty cell, a counter at
`n` can only mean `n` distinct cells of that one line, all owned by the same
player — nothing else needs checking.

Each placeMark therefore does a bounded amount of work no matter how large the
...
and the closing `placeMark(2, 1, 1)` takes it to 3 = n, which is why the win is
reported on that placeMark and on none earlier. The construction allocates the
2 x n row and column counters plus the two diagonal scalars, which is also
the whole space bill.

**Complexity:** `O(1)` per placeMark, `O(n)` space.
