...
makes the large-`k` cases disappear into ordinary lookups — when `k`
is a multiple of `n` every character maps to itself (a uniform string like
`"aaa"`, or any string with `k = n`), and when `k = n - 1` the answer is
just the input rotated by one place. On Example 1, `"cipher"` with `k = 2`
reads sources `2`, `3`, `4`, `5`, `0`, `1` — `'p'`, `'h'`, `'e'`, `'r'`,
`'c'`, `'i'` — assembling `"pherci"`.
