# Solutions — Peaks With A Tall Predecessor

## Single pass over adjacent pairs

Firmness is a purely local verdict: peak `i` qualifies exactly when its
immediate predecessor exists and satisfies `height[i - 1] > threshold`, so no
index ever depends on anything farther than one step away. Peak 0 has no
predecessor and can never be firm, which fixes the scan's starting point at
index 1.
...
output order without any post-sorting. Every peak is judged exactly once,
