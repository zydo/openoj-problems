# Solutions — Season Standings

## Window ranking over pre-aggregated records

The table hands over each club's finished season, so the points are a pure
per-row expression — `3 * won + drawn`, with lost worth nothing — and
no join or aggregation is needed to produce them. The rank is competition
ranking: tied totals share a place and the next distinct total skips
past every tied team, which is exactly what `RANK() OVER (ORDER BY
3 * won + drawn DESC)` computes. In the example, Manchester City's and
Liverpool's 20 points both receive place 1, and Chelsea's 18 points
receives place 3 — one past the two teams already placed.

`DENSE_RANK()` is the natural misstep: it also gives ties equal numbers but
counts them as one, so Chelsea would land in place 2 instead of 3 and
...
The final `ORDER BY points DESC, club_name` presents the required table:
