...
class NeighborTotals {
  public:
    NeighborTotals(vector<vector<int>> grid) : grid(grid), n(grid.size()) {
    ...
    int sideSum(int value) { return sumAround(rowOf[value], colOf[value], {{-1, 0}, {1, 0}, {0, -1}, {0, 1}}); }

    int cornerSum(int value) { return sumAround(rowOf[value], colOf[value], {{-1, -1}, {-1, 1}, {1, -1}, {1, 1}}); }
