function countAlternatingTrios(colors: number[], queries: number[][]): number[] {
    ...
    colors = [...colors];
    const posCnt: number[] = new Array(n + 1).fill(0);
    const runCnt: number[] = new Array(n + 1).fill(0);
    const runSum: number[] = new Array(n + 1).fill(0);
    let total = 0;
    const totals: number[] = [0, 0];

    const addPos = (i: number, delta: number): void => {
        i += 1;
        while (i <= n) {
            posCnt[i] += delta;
            i += i & -i;
    ...
    const prefixPos = (i: number): number => {
        i += 1;
        let t = 0;
        while (i > 0) {
            t += posCnt[i];
    ...
    const addRun = (fen: number[], length: number, delta: number): void => {
        length += 1;
        while (length <= n) {
            fen[length] += delta;
    ...
    const prefixRun = (fen: number[], length: number): number => {
        length += 1;
        let t = 0;
        while (length > 0) {
            t += fen[length];
    ...
    const cyc = (d: number): number => ((d % n) + n) % n;
    const runsUpdate = (length: number, delta: number): void => {
        if (length > 0) {
    ...
    const prevBad = (e: number): number => {
        const c = prefixPos(e - 1);
        return c > 0 ? kth(c) : kth(total);
    };
    const nextBad = (e: number): number => {
        const c = prefixPos(e);
        return c < total ? kth(c + 1) : kth(1);
    };
    const insertEdge = (e: number): void => {
        if (total > 0) {
            const p = prevBad(e);
            const nx = nextBad(e);
    ...
    const removeEdge = (e: number): void => {
        addPos(e, -1);
        total -= 1;
        if (total > 0) {
            const p = prevBad(e);
