...
`MIN((mask >> b) & 1)` collapses a whole column of staff members into
...
dense the masks are. The result comes back as one row — `all_bits`
then `any_bits` — which trivially satisfies the statement's any-order
...
**Complexity:** `O(U · B)` time and space for `U` staff members and `B = 31` bit
