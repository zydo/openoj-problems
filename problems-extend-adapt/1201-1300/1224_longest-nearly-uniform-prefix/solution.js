 ...
 * @return {number}
 */
var longestFixablePrefix = function (nums) {
    const count = new Map(); // value -> occurrences
    const freq = new Map(); // occurrence count -> how many values have it
    let best = 0;
    for (let n = 1; n <= nums.length; ++n) {
        const value = nums[n - 1];
        const before = count.get(value) ?? 0;
        if (before > 0) {
            const slot = freq.get(before) - 1;
        ...
        if (keys.length === 1) {
            ...
            else if (a === 1 && freq.get(a) === 1 && 1 + b * freq.get(b) === n) best = n;
        }
    }
    return best;
};
