// Problem-provided oracle (LinkIndex), C++ side. Compiled into the
...
class LinkIndex {
public:
    LinkIndex(const OjValue& urls, const OjValue& edges, long long budget) : budget_(budget) {
        if (urls.kind != OjValue::Array) throw std::runtime_error("LinkIndex urls must be an array");
        for (const OjValue& url : urls.items) {
            if (url.kind != OjValue::String) throw std::runtime_error("LinkIndex urls must be strings");
        ...
        if (edges.kind != OjValue::Array) throw std::runtime_error("LinkIndex edges must be an array");
        for (const OjValue& edge : edges.items) {
            if (edge.kind != OjValue::Array || edge.items.size() != 2 ||
                ...
                throw std::runtime_error("LinkIndex edges must be pairs of integers");
            }
            links_[edge.items[0].integer].push_back(names_[edge.items[1].integer]);
        }
    }

    std::vector<std::string> linksFrom(const std::string& url) {
        if (budget_ <= 0) throw std::runtime_error("LinkIndex query budget exhausted");
        budget_ -= 1;
        fetched_.insert(url);
