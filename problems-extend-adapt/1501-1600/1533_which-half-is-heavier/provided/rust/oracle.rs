// Problem-provided oracle (BalanceReader), Rust side. Assembled into
...
impl BalanceReader {
    pub fn new(construction: &[OjValue], budget: i64) -> Self {
        let raw = match construction.first() {
            ...
            _ => panic!("BalanceReader arr must be an array"),
        };
        let mut values = Vec::with_capacity(raw.len());
        for entry in raw {
            match entry {
                OjValue::Int(v) => values.push(v),
                _ => panic!("BalanceReader entries must be integers"),
            }
        }
        BalanceReader { values, budget }
    }
