function firstPlayerWins(n: number): boolean {
    ...
    const wins = new Array<boolean>(n + 1).fill(false);
    for (let total = 1; total <= n; total++) {
        for (let k = 1; k * k <= total; k++) {
            // Removing k*k stones hands the opponent a pile of size
            // total - k*k. If that leaves the opponent in a losing state,
            // the mover wins by making this exact move.
            if (!wins[total - k * k]) {
                wins[total] = true;
                break;
            }
