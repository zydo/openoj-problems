# Solutions — Fitting Boxes Into Rooms II
...
`effective[i] = max(prefixMin[i], suffixMin[i])`. Unlike in the one-sided
version I, this array is no longer monotonic — a short pair of rooms in the middle
...
version I no longer applies; what still holds is that placement reduces
