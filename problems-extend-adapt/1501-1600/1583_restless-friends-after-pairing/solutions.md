# Solutions — Restless Friends After Pairing

## Rank Lookup

Checking "does `i` prefer `j` over `k`" by scanning `preferences[i]` for both
positions costs `O(n)` per question, and the restless test asks it twice for
...
satisfies both marks `x` restless, so the inner scan stops immediately.

Building `rank` costs `O(n^2)` (each of the `n` rows has `n - 1` entries),
and the restless check runs the same `O(n)` scan for each of the `n` friends,
