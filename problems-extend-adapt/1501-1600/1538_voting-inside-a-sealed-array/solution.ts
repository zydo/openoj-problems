class Solution {
    callMajority(arrayReader: SealedBag): number {
        const n = arrayReader.length();
        ...
        let seen0 = false;
        for (let i = 3; i < n; i++) {
            const r = arrayReader.query(0, 1, 2, i);
            results.push([i, r]);
            if (r === 4) {
                seen4 = true;
            } else if (r === 0) {
        ...
        if (seen4 || seen0) {
            ...
            // (contributing 1).
            const matchResult = seen4 ? 4 : 2;
            const anchorMatch = seen4 ? 3 : 2;
            const anchorDiff = seen4 ? 0 : 1;
            const diffResult = seen4 ? 2 : 0;
            const haveMatchIndex = seen4;

            const countMatch = results.filter(([, r]) => r === matchResult).length;
            const countDiff = results.length - countMatch;
            const totalMatch = countMatch + anchorMatch;
            const totalDiff = countDiff + anchorDiff;
        ...
        // equal v.
        const vIndex = 3;
        const r2 = arrayReader.query(0, 1, 3, 4);
        const tail = n - 3;
        let trioMatchesV: number;
        ...
        const countV = tail + trioMatchesV;
        const countOther = 3 - trioMatchesV;
        if (countV === countOther) {
            return -1;
        }
        return countV > countOther ? vIndex : otherIndex;
    }
}
