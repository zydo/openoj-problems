// Problem-provided oracle (SealedBag), Rust side. Assembled into
...
impl SealedBag {
    pub fn new(construction: &[OjValue], _budget: i64) -> Self {
        let items = match construction.first() {
            ...
            _ => panic!("SealedBag values must be an array"),
        };
        let mut nums = Vec::with_capacity(items.len());
        for item in items {
            match item {
                OjValue::Int(v) => nums.push(v as i32),
                _ => panic!("SealedBag values must be integers"),
            }
        }
        let budget = 2 * nums.len() as i64;
        SealedBag { nums, budget }
    }
