# Solutions — Best Permuted Request Total
...
case. A difference array avoids that: bump a counter by `+1` at `left`
and by `-1` at `right + 1` for every request, then take a prefix sum over
