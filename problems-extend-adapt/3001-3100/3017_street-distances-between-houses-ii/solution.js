 ...
 * @return {number[]}
 */
var distanceTally = function (n, x, y) {
    ...
    const departures = new Array(n + 2).fill(0);
    const arrivals = new Array(n + 2).fill(0);
    const addRange = (diff, low, high) => {
    ...
    const departed = new Array(n + 2).fill(0);
    const arrived = new Array(n + 2).fill(0);
    for (let k = 1; k <= n + 1; k++) {
