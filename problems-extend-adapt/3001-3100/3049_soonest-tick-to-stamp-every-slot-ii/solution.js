 ...
 * @return {number}
 */
var soonestStampSecond = function (nums, changeIndices) {
    const n = nums.length;

    // Min-heap of nums values of currently chosen clearances, sifted by
    // hand: values stay at or below 1e9 and sums at n * 1e9, both exact in
    // IEEE doubles.
    const siftUp = (bank, i) => {
        while (i > 0) {
            const p = (i - 1) >> 1;
