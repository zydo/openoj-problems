# Solutions — Soonest Tick To Stamp Every Slot II

## Binary search on the answer plus a deadline-cap greedy

Feasibility is monotone in the horizon: finishing within `t` ticks also
finishes within `t + 1`, so the answer is binary-searchable over `[1, m]`.
For a fixed horizon `t`, a reset only pays when it zeroes an index whose
value is at least 2, and resetting a slot at its **first** occurrence
...
release. So a schedule is fully described by the set `S` of slots reset
...
A set of resets `S` is schedulable exactly when two families of
conditions hold. Counting: `sum(nums) + n - sum(nums[i]-1 for i in S) <= t`
(the resets replace whole decrement chains with single ticks). Deadlines:
ranking the chosen resets by first occurrence `f`, the stamps of every
suffix of them must fit into the ticks after `f` that are not themselves
...
violated, give back the banked reset with the smallest saving (a
