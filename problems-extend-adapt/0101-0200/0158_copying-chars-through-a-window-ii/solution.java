class Solution {
    ...
    public int read(CharSource charSource, int[] queries, char[] buf) {
        int total = 0;
        for (int n : queries) {
    ...
    private int transfer(CharSource charSource, int n, char[] buf, int offset) {
        int transferred = 0;
        while (transferred < n) {
            if (buf4Index == buf4Count) {
                buf4Count = charSource.read4(buf4);
                buf4Index = 0;
                if (buf4Count == 0) {
                    break;
                }
