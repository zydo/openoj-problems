# Solutions — Xth Smallest of Each Window

## Sliding Frequency Table

Only negative values can ever be the answer: when a window holds fewer
...
bucket crosses the threshold is that window's rating. If the sweep falls
off the end of the buckets, fewer than x negatives are present and the
answer is 0. Each slide touches one entering and one leaving element and
