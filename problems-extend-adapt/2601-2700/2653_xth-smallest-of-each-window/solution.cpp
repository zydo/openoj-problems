class Solution {
  public:
    ...
    // ever be an answer: cnt[v + 50] counts copies of the negative value
    // v inside the current window. Each answer is found by walking those
    // buckets smallest value first until x negatives accumulate (0 when
    // fewer than x).
    vector<int> xthSmallestOfWindow(vector<int> &nums, int k, int x) {
        vector<int> cnt(50, 0);
