# Solutions — Answers To The Class
...
The walk works unchanged for primitives because `Object.getPrototypeOf`
auto-boxes the value first, so the chain for `true` starts from
`Boolean.prototype`, exactly the reach-the-methods framing of Example 1 —
which the `instanceof` operator would have answered false. Built-in
constructors behave like any user class here: a constructor
