...
class CopyOnWriteEditor {
    ...
    produce(mutator: (draft: any) => void): any {
        const shadows = new Map<any, Record<string, any>>(); // node -> overrides
        const proxies = new Map<any, any>(); // node -> proxy standing in for it

        const proxyFor = (node: any): any => {
        ...
        const shared = new Set<any>(); // subtrees proven unchanged (shareable)
        const materialize = (node: any): any => {
            if (node === null || typeof node !== "object") {
                return node;
            }
            if (shared.has(node)) {
...
class Solution {
    run(mutationReplay: MutationReplay): void {
