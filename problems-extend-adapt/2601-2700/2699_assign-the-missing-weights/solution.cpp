...
class Solution {
  public:
    vector<vector<int>> assignMissingWeights(int n, vector<vector<int>> &edges, int source, int destination,
                                             int target) {
        ...
        for (int index = 0; index < count; ++index) {
            answer[index] = {edges[index][0], edges[index][1], static_cast<int>(weights[index])};
        }
        return answer;
    }
};
