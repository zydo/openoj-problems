# Solutions — Write-Proof JSON Views
...
`writeProof(obj)` returns `new Proxy(obj, handlers)` and never copies
...
applies `writeProof` to any object or array it sees (the hint's
...
constant work, and wrapping is lazy — `writeProof` itself is a single
