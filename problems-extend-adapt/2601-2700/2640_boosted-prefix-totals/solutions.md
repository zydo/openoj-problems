# Solutions — Boosted Prefix Totals

## Fused running maximum and running total

The answer is the prefix-sum array of the boosted array, and each
boosted value needs only the running maximum seen so far — so the whole
...
emitted as `ans[i]`. The boosted array itself is never materialized: one
...
The arithmetic outgrows 32 bits by design: a single boosted value already
