...
// statement's Input). drive() receives the submission's atMostOnce function,
...
// whatever atMostOnce() handed back: row 0 must return the shim's recorded
...
class SingleUseCase {
    ...
    constructor([fnSource, calls], budget) {
        void budget;
    ...
    _underlying() {
        return new Function("return (" + this.fnSource + ");")();
    }

    // Replay this case's rows against the submission's atMostOnce function.
    drive(atMostOnceFn) {
        if (typeof atMostOnceFn !== "function") {
            throw new Error("drive expects the submission's atMostOnce function");
        }
        let realCalls = 0;
        let value;
        ...
        const wrapped = atMostOnceFn(counting);
        if (typeof wrapped !== "function") {
            throw new Error("atMostOnce must return a function");
        }
