...
// CurfewCache class on a deterministic virtual clock: while drive()
...
class CacheProbe {
    ...
    constructor([actions, values, timeDelays], budget) {
        void budget;
    ...
    _advanceTo(state, timers, target) {
    ...
    // Replay this case's script against the submission's CurfewCache
    ...
    drive(cacheClass) {
        ...
        try {
            ...
            for (let at = 0; at < this.actions.length; at++) {
                ...
                if (action === "CurfewCache") {
                    if (cache !== null) {
                        throw new Error("the script constructs CurfewCache more than once");
                    }
                    cache = new cacheClass();
                    this.outputs.push(null);
                } else if (action === "set") {
                    if (cache === null) {
                        throw new Error("the script must construct the CurfewCache first");
                    }
