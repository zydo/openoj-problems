 ...
 * @return {boolean[]}
 */
var blockFits = function (queries) {
    ...
    const fen = new Array(k + 1).fill(0);

    const fenAdd = function (i) {
        for (; i <= k; i += i & -i) {
            fen[i]++;
        }
    };

    const fenSum = function (i) {
        let total = 0;
        for (; i > 0; i -= i & -i) {
            total += fen[i];
        }
    ...
    const segMax = new Array(4 * span).fill(0);
    const segTag = new Array(4 * span).fill(0); // 0 = untagged; obstacles are >= 1

    const build = function (node, lo, hi) {
        if (lo === hi) {
    ...
    const applyTo = function (node, lo, t) {
        segTag[node] = t;
        // The run t - i shrinks as i grows, so the gap's best sits left.
        segMax[node] = t - lo;
    };

    const pushDown = function (node, lo, mid) {
        if (segTag[node] !== 0) {
    ...
    const update = function (node, lo, hi, l, r, t) {
        if (r < lo || hi < l) {
            return;
        }
        if (l <= lo && hi <= r) {
    ...
    const query = function (node, lo, hi, l, r) {
        if (r < lo || hi < l) {
            return 0;
        }
        if (l <= lo && hi <= r) {
