# Solutions — Ways Up A Doubling Staircase

## Gap combinatorics

Order is the only freedom here: every valid route with `x` leaps and
`y` down-steps ends on stair `2^x - y` regardless of interleaving, because
the climbs contribute exactly `2^0 + ... + 2^(x-1)`. So a route ending
...
(`y >= 0`), and the down-steps cannot be consecutive, so each must occupy a
...
reached on the stairs too — before any down-step the position is
