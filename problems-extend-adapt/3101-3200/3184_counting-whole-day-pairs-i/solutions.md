# Solutions — Counting Whole-Day Pairs I
...
mod 24, then two nested loops test every pair once: a pair spans whole
days precisely when its residue sum vanishes mod 24.
...
matching the statement's own hint (`(hours[i] + hours[j]) % 24 == 0`)
while sidestepping its overflow hazard. The loop order keeps `i < j`,
which is exactly the statement's index convention, so nothing is counted
twice or skipped. The scale makes any counting-bucket alternative
unnecessary here — that machinery earns its keep in the larger twin
problem.
