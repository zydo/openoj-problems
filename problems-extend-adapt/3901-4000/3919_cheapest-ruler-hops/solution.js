 ...
 * @return {number[]}
 */
var cheapestHops = function (nums, queries) {
    const n = nums.length;
    const forward = new Array(Math.max(n - 1, 0)).fill(0);
    const backward = new Array(Math.max(n - 1, 0)).fill(0);
    ...
    const prefixForward = new Array(n).fill(0);
    const prefixBackward = new Array(n).fill(0);
    for (let i = 1; i < n; i++) {
