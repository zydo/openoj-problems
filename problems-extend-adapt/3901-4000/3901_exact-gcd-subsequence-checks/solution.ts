function countGcdChecks(nums: number[], p: number, queries: number[][]): number {
    const limit = 50000;
    const n = nums.length;
    ...
    const factors = (original: number): number[] => {
        const result: number[] = [];
        let value = original;
        while (value > 1) {
    ...
    const adjust = (prime: number, index: number, delta: number): void => {
