...
class Solution {
  public:
    int countGcdChecks(std::vector<int> &nums, int p, std::vector<std::vector<int>> &queries) {
        ...
        for (int i = 0; i < n; ++i) {
            if (nums[i] % p == 0) {
                ++active;
                for (int prime : factors(nums[i] / p))
                    adjust(prime, i, 1);
            }
        }
        int answer = 0;
        for (const auto &query : queries) {
            ...
            if (nums[index] % p == 0) {
                for (int prime : factors(nums[index] / p))
