impl Solution {
    pub fn max_pool_ii(grid: Vec<Vec<i32>>) -> i32 {
        let rows = grid.len();
        let columns = grid[0].len();
        let mut positions = vec![Vec::<(usize, usize)>::new(); 201];
        for row in 0..rows {
            for column in 0..columns {
                let value = grid[row][column] as usize;
                if value != 0 {
        ...
        for value in 1..=200_usize {
            ...
            for row in 0..rows {
                let mut running = 0;
                for column in 0..columns {
                    running += (grid[row][column] as usize > value) as i32;
                ...
                for corner_row in [row as isize - value as isize, row as isize + value as isize] {
                    for corner_column in [column as isize - value as isize, column as isize + value as isize] {
                        if corner_row >= 0
                            ...
                            && grid[corner_row as usize][corner_column as usize] as usize > value
                        {
                            greater -= 1;
                        }
                    }
