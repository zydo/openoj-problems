# Solutions — Even-Gap Range Queries

## Extract, Sort, and Check

A set of numbers can be rearranged into an even-gapped sequence if and only
if its sorted order is already even-gapped: sorting always produces the one
...
sorted version even-gapped" — which a single linear scan can answer.
