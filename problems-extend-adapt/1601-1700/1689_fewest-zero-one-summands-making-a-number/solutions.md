# Solutions — Fewest Zero-One Summands Making a Number

How many zero-one numbers does it take to sum to `n`? The knee-jerk
instinct is to reason about the value of `n` and search for a partition, but
the zero-one shape makes the position of each digit, not the magnitude of
...
zero-one numbers that fit below it are 0 and 1, so exactly `d` of them
must be added up. With several digits, addition is position-wise — a
zero-one number contributes either 0 or 1 to each column and carries are
...
zero-one, and after `m` subtractions every digit is 0, exactly as the
second hint describes.

That construction also proves optimality. No zero-one summand can
