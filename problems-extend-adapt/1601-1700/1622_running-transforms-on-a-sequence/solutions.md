# Solutions — AffineSequence Sequence

## Lazy global transform with a modular-inverse append

`shiftAll` and `scaleAll` never touch the stored elements directly. Instead
...
current value is `stored * mult + add (mod 10⁹ + 7)`. `shiftAll(inc)` folds
in by setting `add = add + inc`; `scaleAll(m)` scales both fields at once —
...
every prior `shiftAll`/`scaleAll` call and will keep evolving afterward. The
...
**Complexity:** `append`, `shiftAll`, `scaleAll`, and `getIndex` each run in
