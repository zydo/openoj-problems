 ...
 * @return {number}
 */
var zigzagQuadruplets = function (nums) {
    // For every pair j < k with nums[k] < nums[j], a quadruplet is any
    // i < j with nums[i] < nums[k] plus any l > k with nums[l] > nums[j].
    // Each j rebuilds the less-than row and sweeps its window right-to-left
    // carrying the suffix-greater count. Answers top out at C(4000,4) =
    // 10650673999000 < 2^53, so Number addition stays integer-exact.
    const n = nums.length;
    const less = new Array(n + 2).fill(0);
    let ans = 0;
    for (let j = 1; j <= n - 3; j++) {
