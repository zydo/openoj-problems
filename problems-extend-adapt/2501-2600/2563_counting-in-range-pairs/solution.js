 ...
 * @return {number}
 */
var countRangePairs = function (nums, lower, upper) {
    // Sorting discards index identity, but fairness only depends on values:
    // counting ordered positions i < j in the sorted array counts each
    // original pair exactly once. Sums touch +-2*10^9 and answers reach
    // C(n,2) ~= 5*10^9 — both well inside the exact-Number window < 2^53.
