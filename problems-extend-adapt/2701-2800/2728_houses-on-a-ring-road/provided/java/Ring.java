...
/** Problem-provided oracle (Ring). Compiled with every submission
...
public class Ring {
    ...
    public Ring(List<Object> values, long budget) {
    ...
    public void moveLeft() {
        spend();
        position = (position + doors.length - 1) % doors.length;
    }

    private void spend() {
        if (budget <= 0) {
            throw new IllegalStateException("Ring query budget exhausted");
        }
        budget -= 1;
    }
}
