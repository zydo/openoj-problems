...
class Solution {
    ...
    public int fewestPowerOfFiveCuts(String s) {
        int n = s.length();
        // dp[i] holds the minimum number of tidy pieces covering the
        // suffix s.substring(i). A longer first piece can strand a remainder
        // that cannot be split at all, so every cut point j is tried, not just
        // the longest or shortest tidy prefix. More pieces than cutting
