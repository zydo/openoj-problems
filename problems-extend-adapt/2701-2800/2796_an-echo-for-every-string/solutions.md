# Solutions — An Echo For Every String

## Binary Squaring on the String Prototype

The enhancement installs `echo` on `String.prototype` through
...
`echo` must materialize `len × times` result characters, so
