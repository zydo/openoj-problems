function countCohesiveSubarrays(nums: number[]): number {
    const minDq: number[] = []; // indices, values increasing (front = min)
    const maxDq: number[] = []; // indices, values decreasing (front = max)
    let left = 0;
    let count = 0;
