// Problem-provided driver for 2721 one-promise-over-a-parallel-batch.
// Assembled into every submission by the judge ahead of the
...
class BatchDriver {
    ...
    private outcome: { resolved: unknown[] } | { rejected: unknown } | null = null;
    private clock = new BatchClock();
    ...
    constructor([functions]: any[], budget?: any) {
        ...
        // no-op catch is attached as a SIDE handler only (mirroring
        // parallel_driver.js), so even a submission that forgets rejection
    ...
    // Hand this case's functions to the submission's gatherAll, pump every
    ...
    async drive(gatherAll: (functions: FnPromise[]) => Promise<unknown[]>): Promise<void> {
        const returned = gatherAll(this.functions);
        if (!returned || typeof returned.then !== "function") {
            throw new Error("gatherAll must return a promise");
        }
        let fired = 0;
        ...
        await null;
        try {
            this.outcome = { resolved: (await returned) as unknown[] };
        } catch (reason) {
