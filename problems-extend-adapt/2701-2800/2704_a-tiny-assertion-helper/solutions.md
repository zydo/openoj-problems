# Solutions — A Tiny Assertion Helper
...
`checkThat(val)` returns an object literal whose two methods close over the
one `val` the call captured. `sameAs(other)` checks `val === other` and
returns `true` on a match; otherwise it throws `new Error("Not Equal")`.
`notSameAs(other)` is the exact complement: `val !== other` returns `true`,
...
byte-exact — `Not Equal` from `sameAs`, `Equal` from `notSameAs` — and the
success path must return the boolean `true` itself, not a truthy stand-in.
Each call to `checkThat` builds a fresh pair of matchers over its own
