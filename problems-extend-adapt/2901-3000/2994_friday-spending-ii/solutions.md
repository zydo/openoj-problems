...
`SpendLog`. A recursive CTE supplies them instead: it seeds
...
Each surviving calendar day is then `LEFT JOIN`-ed to `SpendLog` on the
date. A Friday with purchases matches one joined row per purchase, and
`SUM(spend_amount)` totals them; a Friday without purchases matches
nothing, but the outer join keeps the day alive as a single row whose
`spend_amount` is null — `SUM` over that lone null is null, and
...
join probe, so with `n` rows in `SpendLog` the whole query is linear in
