// Problem-provided oracle (VastArray), Rust side. Assembled into
...
impl VastArray {
    pub fn new(construction: &[OjValue], budget: i64) -> Self {
        let raw = match construction.first() {
            ...
            _ => panic!("VastArray blocks must be an array"),
        };
            ...
            match block {
                OjValue::Array(pair) if pair.len() == 2 => {
                    let value = match &pair[0] {
                        OjValue::Int(v) => *v,
                        _ => panic!("VastArray block entries must be integers"),
                    };
                ...
                _ => panic!("VastArray blocks must be [value, count] pairs"),
            }
        }
        VastArray {
            values,
