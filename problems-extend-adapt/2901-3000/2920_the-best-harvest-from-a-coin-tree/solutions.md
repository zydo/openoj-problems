# Solutions — The Best Harvest From A Coin Tree

## Halving-state tree DP

Every node is collected exactly once, and the only interaction between
choices is downward: taking the halving move at a node halves the coins of
its whole subtree. Repeated floor-halving composes, so if t ancestors of
a node took the halving move, its coin value is exactly `coins[v] >> t`,
...
over the children y. The first term is the whole-pile move (the k loss is real
and may go negative), the second is the halving move; state 14 absorbs to 0
because every remaining coin is 0 and the halving move always yields at
