function distinctSquareSum(nums: number[]): number {
    ...
    const add = (l: number, r: number, v: number): void => {
    ...
    const last = new Array<number>(100001).fill(-1);
    let answer = 0;
    let running = 0;
    for (let i = 0; i < n; ++i) {
