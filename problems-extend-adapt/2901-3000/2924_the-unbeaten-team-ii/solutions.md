# Solutions — The Unbeaten Team II
...
the unbeaten-team definition onto a local graph fact: a is unbeaten if
and only if no edge points at it.
...
lost to someone, so it cannot be the answer; a team with count zero never lost to
anyone, and by the observation above nobody reaches it at all, so it is
unbeaten. The input guarantees the stronger-than relation is
...
the unique answer, and if there are two or more (Example 2's teams 0
