# Solutions — Rebuild the Sequence from Neighbor Pairs

## Walk the adjacency path

Every value of the lost sequence is a node of a path graph whose edges
are the given pairs: two values have exactly one neighbour (the endpoints of the
...
deterministic — a reversed-but-valid sequence would still satisfy the
problem's own rules but would not match the judged output. The chosen
rule is: start at the first element of `pairs[0]` when that value
