class Solution {
    ...
    public int fewestFloorBoxes(int n) {
        ...
        long target = n;
        long lo = 0,
            hi = 2500; // S(2500) > 2^31 - 1, so hi stands above every n
        while (hi - lo > 1) {
            long mid = (lo + hi) / 2;
        ...
        long k = lo; // largest k with S(k) < n
        long rest = target - (k * (k + 1) * (k + 2)) / 6;
        long jlo = 1,
            jhi = k + 1; // T(k+1) >= rest always holds
        while (jlo < jhi) {
            long mid = (jlo + jhi) / 2;
            if ((mid * (mid + 1)) / 2 >= rest) {
                jhi = mid;
