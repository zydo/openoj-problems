# Solutions — The Best Bookended Flower Row

## Equal endpoints, positive middles

Because flowers may be removed freely, a presentable row is decided by two
...
So for each pair `i < j` with `flowers[i] == flowers[j]`, the best row on
...
`j` closes a row whose sum is `2v + pos - seen[v]`, since the kept
...
needs only that single stored number. The statement promises some presentable
row exists, so some pair always closes and the running answer is set.
