...
// numbers are exact.
function wellConnectedPairs(n: number, edges: number[][], queries: number[]): number[] {
    const deg: number[] = new Array<number>(n + 1).fill(0);
    ...
    sVals.sort((x, y) => x - y);
    tVals.sort((x, y) => x - y);
    const upperBound = (a: number[], k: number): number => {
        let lo = 0;
        let hi = a.length;
        while (lo < hi) {
            const mid = (lo + hi) >> 1;
