...
On `("cabbage", "cabbega")` the scan records 4 and 6, and `a`/`e` match
across the two positions, so one exchange equalizes the words; on
`("abcde", "badce")` a third disagreement ends the scan with `false`.
