...
function minZeroWindowEdits(nums: number[], k: number): number {
    const X = 1024;
    const INF = 1 << 20;
    let dp: number[] = new Array(X).fill(INF);
    dp[0] = 0;
    for (let r = 0; r < k; ++r) {
        const count: number[] = new Array(X).fill(0);
        let size = 0;
        ...
        best += size;
        const nxt: number[] = new Array(X).fill(best);
        for (let v = 0; v < X; ++v) {
            const c = count[v];
            if (c === 0) {
                continue;
