...
function canMouseOutrun(grid: string[], catJump: number, mouseJump: number): boolean {
    const rows = grid.length;
    const cols = grid[0].length;
    const idx = new Array<number>(rows * cols).fill(-1);
    let n = 0;
    let mouse0 = 0;
    let cat0 = 0;
    let food = 0;
    ...
    const MOUSE = 1;
    const CAT = 2;
    const label = new Array<number>(2 * n * n).fill(UNKNOWN);
    const degree = new Array<number>(2 * n * n).fill(0);
    const queue: number[] = [];
    for (let m = 0; m < n; m++) {
        for (let c = 0; c < n; c++) {
            for (let t = 0; t < 2; t++) {
...
function reversed(moves: number[][], n: number): number[][] {
