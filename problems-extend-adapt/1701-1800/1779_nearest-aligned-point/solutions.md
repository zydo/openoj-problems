# Solutions — Nearest Aligned Point

## One pass with a running best

Alignment and distance both come cheap: `points[i]` is aligned exactly when
`ai == x` or `bi == y`, and because an aligned point already agrees with one
...
at your own location scores 0 either way). Nothing but the aligned points ever
needs scoring.
