...
class Solution {
    ...
    public boolean allPathsReachTarget(int n, int[][] edges, int source, int target) {
        ...
        Boolean verdict = leafVerdict(graph, source, target);
        if (verdict != null) {
            return verdict;
        }
        ...
        while (!stack.isEmpty()) {
            ...
            verdict = leafVerdict(graph, neighbor, target);
            if (verdict != null) {
                if (!verdict) {
                    return false;
                }
    ...
    // Returns a decided verdict for a leaf or for the target itself;
    // null means the node needs a full DFS expansion before it is decided.
    private Boolean leafVerdict(List<Integer>[] graph, int node, int target) {
