# Solutions — Splitting Into Clean Blocks

## Dynamic programming over prefix ends

A cut is clean exactly when every block is one of the three good
shapes, so the natural question to ask about a prefix is: can
`nums[:i]` be split cleanly? Call that `ok[i]`. Any clean split of
`nums[:i]` ends with a final block of length two or three — the shapes on
offer — and chopping that block off leaves a shorter prefix whose fitness
